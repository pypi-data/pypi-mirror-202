...
class WrappedTerm(object):
    ...
    def __eq__(self, other):
        return self.term.identifier == other.term.identifier

    def __hash__(self):
        return hash(self.term.identifier)
...
def consolidate_candidate(
    ...
    ordered_labels = [params.kill_label, 'afpdeleted', 'afpperson', 'afplocation', 'afporganization', 'person',
                      'location', 'organization', 'loc_org', 'witness', params.white_label, params.wikidata_label,
    ...
    for ann in sorted_annotations:
        ...
        if (
            ...
            if ann.labelName not in [params.kill_label, params.white_label, 'afpdeleted'] :
            ...
            if target.labelName != params.kill_label and target.labelName != params.white_label:
                if target.start - ann.start == 0 and target.end - ann.end == 0:
                    ...
                    if tlex in ['person', 'location',
                                'organization', 'deleted']:  # target is an AFP entity
                        ...
                        # The AFP entity is ambiguous, keep it and try to disambiguate later thanks to the model
                            ...
                            addit = True
                            if tlex == 'deleted':
                                terms = [t for t in ann.terms if t.identifier != target.terms[0].identifier]
                                if len(terms) > 0:
                                    ann.terms = terms
                                    addit = True
                                else:
        ...
        for a in sorted_group:
            ...
            if a.start in groups[k] and a.end - 1 in groups[k]:
                ...
                if a.start - b.start == 0 and a.end - b.end == 0:
                    terms = set(WrappedTerm(t) for t in b.terms)
