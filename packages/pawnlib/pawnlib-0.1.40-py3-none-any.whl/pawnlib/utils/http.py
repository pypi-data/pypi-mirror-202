...
class IconRpcHelper:
    ...
    def get_step_price(self, ):
        response = self._governance_call(method="getStepPrice")
        return response

    def _get_step_costs(self, url=None):
        response = self._governance_call( method="getStepCosts")
        return response
    ...
    def auto_fill_parameter(self):
        if isinstance(self.request_payload, dict) and self.request_payload.get('params'):
            ...
            # self.request_payload['params']['nonce'] = "0x23"
            ...
            pawn.console.log(f"Invalid payload - {self.request_payload}")
