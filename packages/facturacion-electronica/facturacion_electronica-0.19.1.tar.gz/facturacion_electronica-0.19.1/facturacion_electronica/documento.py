        ...
        if not hasattr(self, '_credec'):
            ...
            for li in self.impuestos:
                if li.tax_id.CredEC:
                    ...
                    li._compute_tax()
                    _credec += li.CredEC
            _credec = util.round0(_credec)
            return self._aplicar_dsc_rcg(_credec)
        return self._credec
 ...
 o servicio(s) prestado(s) ha(n) sido recibido(s).'
    ...
    def Detalle(self):
        ...
        for line in self._lineas_detalle:
            if not line.no_product:
                for imp in line.Impuesto:
                    if imp.TasaImp == 0 and not line.IndExe:
                        raise UserError("Línea {0} Debe indicar razón exento".format(
                            line.NroLinDet
                        ))
...
#                lines['RUTMandante']
    ...
    def Detalle(self, vals=[]):
        ...
        for d in vals:
            ...
            if linea.IndExe in [2, 6]:
                if linea.IndExe == 2:
                    self.MontoNF += linea.MontoItem
                else:
                    self.MontoNF -= linea.MontoItem
            else:
    ...
    def impuestos(self, linea):
        ...
        def set_impuesto(val, base=0, cantidad=0, base_otr_mnda=0):
            imp = False
            ...
            imp.base += base
            imp.base_otr_mnda += base_otr_mnda
            imp._compute_tax()
        ...
        if isinstance(linea, LineaImpuesto):
            ...
            for l in linea:
                for i in l.Impuesto:
                    set_impuesto(i, l.MontoItem, l.QtyItem, l.MontoItemOtrMnda)
        else:
            for i in linea.Impuesto:
                set_impuesto(i, linea.MontoItem, linea.QtyItem,
                             linea.MontoItemOtrMnda)
        ...
        if not hasattr(self, '_imp_reten'):
            ...
            for li in self.impuestos:
                if li.tax_id.es_retencion:
                    _imp_reten -= util.round0(li.MontoReten)
                elif li.tax_id.es_adicional:
                    _imp_reten += util.round0(li.MontoImp)
            return self._aplicar_dsc_rcg(util.round0(_imp_reten))
    ...
    def ImptoRetenOtrMnda(self):
        if not hasattr(self, '_imp_reten_otr_mnda'):
            _imp_reten_otr_mnda = 0
            for li in self.impuestos:
                if li.tax_id.es_retencion:
                    _imp_reten_otr_mnda -= util.round0(li.MontoReten)
                elif li.tax_id.es_adicional:
                    _imp_reten_otr_mnda += util.round0(li.MontoImp)
            return self._aplicar_dsc_rcg(_imp_reten_otr_mnda, OtrMnda=True)
        ...
        if not hasattr(self, '_iva_otr_mnda'):
            ...
            for li in self.impuestos:
                if li.tax_id.TasaImp == self.TasaIVA:
                    if li.base_otr_mnda == 0 and self.resumen:
                        li.base_otr_mnda = self.MntNetoOtrMnda
                    li._compute_tax()
                    _iva_otr_mnda += li.VlrImpOtrMnda
            _iva_otr_mnda = util.round0(_iva_otr_mnda)
            return self._aplicar_dsc_rcg(_iva_otr_mnda, OtrMnda=True)
    ...
    @property
    def MontoPeriodo(self):
        if not hasattr(self, '_monto_periodo'):
            return self.MntTotal + self.MontoNF
        return self._monto_periodo

    @MontoPeriodo.setter
    def MontoPeriodo(self, val):
        self._monto_periodo = float(val)
    ...
    def MntExeOtrMnda(self):
        if not hasattr(self, '_mnt_exe_otr_mnda'):
            _mnt_exe_otr_mnda = 0
            for li in self.impuestos:
                if li.tax_id.TasaImpOtrMnda == 0:
                    _mnt_exe_otr_mnda += li.VlrImpOtrMnda
            return self._aplicar_dsc_rcg(_mnt_exe_otr_mnda, False, True, OtrMnda=True)
    ...
    def MntIVA(self):
        if not hasattr(self, '_mnt_iva'):
            ...
            return self._aplicar_dsc_rcg(util.round0(_mnt_iva))
        return self._mnt_iva

    @MntIVA.setter
    def MntIVA(self, val):
        val = float(val)
        self._mnt_iva = util.round0(val)
    ...
    def MntNeto(self):
        if not hasattr(self, '_mnt_neto'):
            ...
            for li in self.impuestos:
                if li.tax_id.TasaImp == self.TasaIVA:
                    _mnt_neto += li.base
            return self._aplicar_dsc_rcg(util.round0(_mnt_neto))
    ...
    def MntNeto(self, val):
        ...
        if self.resumen and self.MntIVA == 0:
            ...
            self.impuestos = impuesto
        self._mnt_neto = util.round0(val)
    ...
    def MntTotal(self):
        if not hasattr(self, '_mnt_total'):
            return self.MntExe + self.MntNeto + self.MntIVA + self.ImptoReten -\
            self.ValComExe - self.ValComNeto - self.ValComIVA + self.MontoNF - \
            self.CredEC
    ...
    def MntNetoOtrMnda(self):
        if not hasattr(self, '_mnt_neto_otr_mnda'):
            _mnt_neto_otr_mnda = 0
            for li in self.impuestos:
                if li.tax_id.TasaImp == self.TasaIVA:
                    _mnt_neto_otr_mnda += li.base_otr_mnda
            return self._aplicar_dsc_rcg(util.round0(_mnt_neto_otr_mnda), OtrMnda=True)
...
llevar estrictamente las palabras Dice: y debe decir:')
    ...
    def Totales(self):
            ...
            if not self.es_boleta():
                for i in self.impuestos:
                    if i.tax_id.es_adicional:
                        ...
                        mnt_imp = util.round0(i.MontoImp)
                        Totales['ImptoReten']['MontoImp'] = 1 if mnt_imp < 0 else self._aplicar_dsc_rcg(mnt_imp)
        ...
        if self.MontoNF:
            Totales['MontoNF'] = self.MontoNF
            Totales['MontoPeriodo'] = self.MontoPeriodo
        #Totales['SaldoAnterior']
        if not self.es_exportacion:
            Totales['VlrPagar'] = self.VlrPagar
        return Totales
...
, en su ficha de receptor")
    ...
    @property
    def VlrPagar(self):
        if not hasattr(self, '_vlr_pagar'):
            return self.MntTotal
        return self._vlr_pagar

    @VlrPagar.setter
    def VlrPagar(self, val):
        self._vlr_pagar = val
    ...
    def get_agrupados(self, monto, Afectos=True, Exentos=False, OtrMnda=False):
        result = {'D': 0.00, 'R': 0.00}
        for gdr in self._gdrs:
            if (Afectos and not gdr.IndExeDR) or (Exentos and gdr.IndExeDR):
                result[gdr.TpoMov] += gdr.get_monto(monto, OtrMnda)
        return result

    def _aplicar_dsc_rcg(self, monto, Afectos=True, Exentos=False, OtrMnda=False):
        if not hasattr(self, '_gdrs') or monto == 0:
            return monto
        agrupados = self.get_agrupados(monto, Afectos, Exentos, OtrMnda)
        monto_gdr = agrupados['R'] - agrupados['D']
        mn = monto + monto_gdr
        if self.moneda_decimales == 0:
            return util.round0(mn)
        return round(mn, self.moneda_decimales)
