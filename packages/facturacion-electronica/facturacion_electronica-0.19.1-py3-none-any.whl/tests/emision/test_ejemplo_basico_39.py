    ...
    def test_timbrar_y_enviar(self):
        ...
        for dte in result['detalles']:
            ...
            if dte['NroDTE'] == 1:
                ...
                self.assertEqual(dte['MntNeto'], 298355)
                self.assertEqual(Totales.find('MntNeto').text, '298355')
                self.assertEqual(dte['MntIVA'], 56687)
                self.assertEqual(Totales.find('IVA').text, '56687')
                self.assertEqual(dte['MntTotal'], 605042)
                self.assertEqual(Totales.find('MntTotal').text, '605042')
                self.assertEqual(dte['ImptoReten'], 0)
                self.assertIsNone(Totales.find('ImptoReten'))
