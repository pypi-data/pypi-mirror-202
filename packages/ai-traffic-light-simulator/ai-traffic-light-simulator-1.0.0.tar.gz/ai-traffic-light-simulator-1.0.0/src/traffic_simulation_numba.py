...
def update_lane(input_segment, lane, output_segment, simulation_context, max_speed=MAX_SPEED):
    ...
    total_length = len(lane) + input_len + output_len
    full_lane = np.zeros(total_length, np.byte)
    if (input_len > 0 and output_len > 0):
        full_lane[:input_len] = input_segment
        full_lane[input_len:-output_len] = lane
        full_lane[-output_len:] = output_segment
    elif (input_len > 0):
        full_lane[:input_len] = input_segment
        full_lane[input_len:] = lane
    elif (output_len > 0):
        full_lane[:-output_len] = lane
        full_lane[-output_len:] = np.copy(output_segment)  
    else:
        full_lane[:] = lane
