...
# Ignore any warning messages related to the smallest subnormal value for numpy floats
warnings.filterwarnings(
    "ignore", message="The value of the smallest subnormal for <class 'numpy.float64'> type is zero."
)
warnings.filterwarnings(
    "ignore", message="The value of the smallest subnormal for <class 'numpy.float32'> type is zero."
)


class Detect:
    def __init__(self, weights, threshold, iou: float = 0.45, force_reload: bool = False, use_sort: bool = True):
    ...
    def detect(self, img) -> list:
        ...
        for track in tracked_objects:
            ...
            p0 = torch.tensor([image_center_x, image_center_y], device=self.device, dtype=torch.float)
            p1 = torch.tensor([target_center_x, target_center_y], device=self.device, dtype=torch.float)

            detected_rectangles.append({
                "track_id": track_id,
                "distance": torch.linalg.norm(p0 - p1),
