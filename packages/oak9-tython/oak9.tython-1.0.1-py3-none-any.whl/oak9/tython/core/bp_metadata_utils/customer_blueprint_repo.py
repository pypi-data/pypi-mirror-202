...
class CustomerBlueprintRepo(MultiplePoliciesPerFileRepo):
    ...
    def print_blueprint_summary(self):
        ...
        print(f"    {Fore.BLUE}Found {Style.BRIGHT}{len(blueprints)}{Style.NORMAL} blueprints{Style.RESET_ALL}")
        print(f"{Fore.LIGHTBLACK_EX}************************************************************{Style.RESET_ALL}")
        i = 0
        for blueprint in blueprints:
            i += 1
            print(f"\n{Style.BRIGHT}{Fore.LIGHTBLACK_EX}Blueprint #{i}{Style.NORMAL}:{Style.RESET_ALL}")
            print(f"{Style.BRIGHT}Name{Style.RESET_ALL}: {blueprint.name}")
            print(f"{Style.BRIGHT}Description{Style.RESET_ALL}: {blueprint.desc}")
            print(f"{Style.BRIGHT}Author{Style.RESET_ALL}: {blueprint.author}")
            print(f"Found {Style.BRIGHT}{len(blueprint.validations)}{Style.NORMAL} validation checks{Style.RESET_ALL}")
            j = 0
            for validation in blueprint.validations:
                j += 1
                print(f"    {Style.BRIGHT}{j}{Style.NORMAL}: {validation.description}")
