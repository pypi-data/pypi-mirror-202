class SaCExceptions(Exception):
    
    def __init__(self, meta_info: object):
        """_summary_
        SaC exception object will add on the request ID and Caller type to SaC Exceptions. 

        Args:
            resource (object): Resource that is currently being evaluated by SaC
        """
...
class ViolationException(SaCExceptions):

    def __init__(self, exception: Exception):
        """_summary_
        
        SaC exception object meant to handle generic Exceptions and ConfigIdExceptions that occurs during violation object creation.
...
class ConfigIdException(SaCExceptions):

    def __init__(self, resource: object, config_name: str = ""):
        """_summary_
        SaC Exception object for handling errors related to config id generation. This exception will feed into ViolationException when raised.
...
class GrpcPopulationException(SaCExceptions):

    def __init__(self, meta_info: object, exception: Exception):
        """_summary_
        SaC Exception object for handling errors related to GRPC object creation. Currently it covers all GRPC objects that the runner generates.
...
class UnpackingException(SaCExceptions):

    def __init__(self, type_url: str = ""):
        """_summary_
        SaC Exception object for handling errors related to the unpacking of a resource based on the type url.
...
class MessageException(SaCExceptions):
    
    def __init__(self):
        """_summary_
        SaC Exception object for handling errors related to the initial grpc message the runner receives.
        """
        self.error_code = "520"
...
class BlueprintException(SaCExceptions):
    
    def __init__(self, meta: object):
        """_summary_
        SaC Exception object that handles errors related  to blueprint retrieval for validation
