...
class TracerTransport(azt.Transport):
    ...
    def __init__(
        ...
        if use_tornado:
            raise Exception("Tornado is not supported in this vwrsion")
    ...
    def _send(self, record):
        try:
            if self.statsd_is_configured:
                yield from self._send_to_statsd(record)
        except Exception as e:
            logging.exception(e)

        try:
            if self.__tracer == 'zipkin':
                yield from super(TracerTransport, self).send(record)
            else:
                self._queue = []
        except Exception as e:
            logging.exception(e)
    ...
    def send(self, record):
        self.loop.create_task(self._send(record))

    @async_decorator
    def _send_to_statsd(self, rec):
        if self.stats:
            name = rec._name
            tags = rec._tags or {}
            ...
            try:
                ...
                rsp_status = 0
            duration = int(round(rec._duration / STATSD_DURATION_FACTOR))
