...
from rekuest.postmans.utils import ReservationContract, RPCContract, arkiuse, localuse
...
class FlowActor(Actor):
    ...
    async def on_provide(self, provision: Provision):
        ...
        self.contracts = {
            node.id: await self.nodeContractor(node, self) for node in arkitektNodes
        }
