...
class COCOeval2:
    ...
    def accumulate(self, p = None):
        ...
        T           = len(p.iouThrs) # [.5:.05:.95] thresholds for evaluation
        R           = len(p.recThrs) # [0:.01:1] recall thresholds for evaluation
        K           = len(p.catIds) if p.useCats else 1 # [all] K categories to use for evaluation
        A           = len(p.areaRng) # A = object area ranges for evaluation
        M           = len(p.maxDets) # [1 10 100] thresholds of max detections per image
...
##########################################################################################
                ...
                # num_classes = len(p.catIds) # get number of classes
                # avg_ap = 0.0
                # if ap == 1:
...
###########################################################################################
        ...
        def _summarizeDets():
            ...
            info_stats[2], stats[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
            info_stats[3], stats[3] = _summarize(1, areaRng='small', maxDets=self.params.maxDets[2])
            info_stats[4], stats[4] = _summarize(1, areaRng='medium', maxDets=self.params.maxDets[2])
            info_stats[5], stats[5] = _summarize(1, areaRng='large', maxDets=self.params.maxDets[2])
            info_stats[6], stats[6] = _summarize(0, maxDets=self.params.maxDets[0])
            info_stats[7], stats[7] = _summarize(0, maxDets=self.params.maxDets[1])
            info_stats[8], stats[8] = _summarize(0, maxDets=self.params.maxDets[2])
            info_stats[9], stats[9] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2])
            info_stats[10], stats[10] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2])
            info_stats[11], stats[11] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2])
            
            index = 12
           
            for i in range(len(self.params.catIds)):
                info_stats[index + 0], stats[index + 0] = _summarize(1, cat = i)
                info_stats[index + 1], stats[index + 1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2], cat = i)
                info_stats[index + 2], stats[index + 2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2], cat = i)
                info_stats[index + 3], stats[index + 3] = _summarize(1, areaRng='small', maxDets=self.params.maxDets[2], cat = i)
                info_stats[index + 4], stats[index + 4] = _summarize(1, areaRng='medium', maxDets=self.params.maxDets[2], cat = i)
                info_stats[index + 5], stats[index + 5] = _summarize(1, areaRng='large', maxDets=self.params.maxDets[2], cat = i)
                info_stats[index + 6], stats[index + 6] = _summarize(0, maxDets=self.params.maxDets[0], cat = i)
                info_stats[index + 7], stats[index + 7] = _summarize(0, maxDets=self.params.maxDets[1], cat = i)
                info_stats[index + 8], stats[index + 8] = _summarize(0, maxDets=self.params.maxDets[2], cat = i)
        ...
        def _summarizeCustom(ap=None, iouThr=None, labels=None):
            ...
            if ap == 1 and iouThr is None:
                ...
                info_stats[0], stats[0] = _summarize(0, maxDets=self.params.maxDets[0])
                info_stats[1], stats[1] = _summarize(0, maxDets=self.params.maxDets[1])
                info_stats[2], stats[2] = _summarize(0, maxDets=self.params.maxDets[2])
                info_stats[3], stats[3] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2])
                info_stats[4], stats[4] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2])
                info_stats[5], stats[5] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2])
                index = 6

                for i in cat_indexs:
                    info_stats[index + 0], stats[index + 0] = _summarize(0, maxDets=self.params.maxDets[0], cat = i)
                    info_stats[index + 1], stats[index + 1] = _summarize(0, maxDets=self.params.maxDets[1], cat = i)
                    info_stats[index + 2], stats[index + 2] = _summarize(0, maxDets=self.params.maxDets[2], cat = i)
                    info_stats[index + 3], stats[index + 3] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2], cat = i)
                    info_stats[index + 4], stats[index + 4] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2], cat = i)
                    info_stats[index + 5], stats[index + 5] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2], cat = i)
                    index += 6
        ...
        def _summarizeKps():
            ...
            info_stats[0], stats[3] = _summarize(1, maxDets=20, areaRng='medium')
            info_stats[0], stats[4] = _summarize(1, maxDets=20, areaRng='large')
            info_stats[0], stats[5] = _summarize(0, maxDets=20)
            info_stats[0], stats[6] = _summarize(0, maxDets=20, iouThr=.5)
            info_stats[0], stats[7] = _summarize(0, maxDets=20, iouThr=.75)
            info_stats[0], stats[8] = _summarize(0, maxDets=20, areaRng='medium')
        ...
        res = {k: v for k, v in zip(self.stats_info,  self.stats)}
