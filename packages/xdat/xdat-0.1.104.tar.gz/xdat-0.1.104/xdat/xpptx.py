...
class Presentation:
    def __init__(self, theme=DEFAULT_THEME, title=None, print_layout=True, fake=False):
        self.theme = theme
        ...
        if not fake:
            ...
            if title:
                ...
                self.add_slide('title', title=title, subtitle=xsettings.PROJECT_NAME_PRETTY, text=date_str)
    ...
    def save(self, out_path=None):
        if not self.fake:
            if not out_path:
                ...
                out_path = xsettings.OUTPUT_PATH.joinpath(f"{slugify(title)}.pptx")
