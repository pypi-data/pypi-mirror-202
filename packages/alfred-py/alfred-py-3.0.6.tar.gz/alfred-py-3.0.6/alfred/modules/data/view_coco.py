...
try:
    ...
    print("[WARN] view coco need pycocotools installed.")
    # exit(-1)
import os
import sys
import cv2
...
def showAnns(ori_img, anns, draw_bbox=False, cats=None):
    ...
    if datasetType == "instances":
        ...
        for ann in anns:
            c = (
                np.array((np.random.random((1, 3)) * 0.6 + 0.4)[0] * 255)
                .astype(int)
                .tolist()
            )
            ...
            if draw_bbox:
                if "bbox" in ann.keys():
                    ...
                    cv2.rectangle(
                        ori_img, pt1, pt2, color=c, thickness=1, lineType=cv2.LINE_AA
                    )

            if "keypoints" in ann and type(ann["keypoints"]) == list:
                # turn skeleton into zero-based index
                cat_skln = [
                    c["skeleton"] for c in cats if c["id"] == ann["category_id"]
                ]
...
def vis_coco(coco_img_root, ann_f):
    ...
    for img_id in img_ids:
        ...
        if "file_name" in img.keys():
            ...
            print(
                "does not foud a file_name or filename in feild. check your annotation style: ",
                img,
            )
        assert os.path.exists(
            img_f
        ), "{} not found, maybe your filename pattern not right? Pls fire an issue to alfred github repo!".format(
            img_f
        )
        ...
        if (
            len(annos) > 0
            and "segmentation" in annos[0].keys()
            and len(annos[0]["segmentation"]) == 0
        ):
            ...
            for ann in annos:
                ...
                cv2.rectangle(
                    img, (x1, y1), (x2, y2), unique_color, line_thickness, cv2.LINE_AA
                )
                ...
                cv2.rectangle(
                    img,
                    (txt_bottom_left[0] - 4, txt_bottom_left[1] - ret_val[1] - 2),
