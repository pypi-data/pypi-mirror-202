...
class AttentionStep1(Function):
  ...
  def backward(ctx, grad_output):
    ...
    assert q.is_contiguous() and k.is_contiguous() and index0.is_contiguous(
    ) and index1.is_contiguous() and grad_output.is_contiguous()
...
class AttentionStep1_v2(Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_output = grad_output.contiguous()
    # print("grad_output.is_contiguous(): ", grad_output.is_contiguous())
    assert q.is_contiguous() and k.is_contiguous() and index0_offsets.is_contiguous(
    ) and index1.is_contiguous() and grad_output.is_contiguous()
...
class AttentionStep2(Function):
  ...
  def backward(ctx, grad_output):
    ...
    grad_output = grad_output.contiguous()
    # print("grad_output.is_contiguous(): ", grad_output.is_contiguous())
    assert attn.is_contiguous() and v.is_contiguous() and index0.is_contiguous(
    ) and index1.is_contiguous() and grad_output.is_contiguous()
...
class DotProdWithIdx(Function):
  ...
  def backward(ctx, grad_output):
    ...
    ) and rel_idx.is_contiguous() and grad_output.is_contiguous()
...
class DotProdWithIdx_v2(Function):
  ...
  def forward(ctx, q, index_q, k, index_k, table_q, table_k, rel_idx):
    ...
    output: output: [M, h]
    """
    assert q.is_contiguous() and index_q.is_contiguous() and k.is_contiguous() and index_k.is_contiguous(
    ) and table_q.is_contiguous() and table_k.is_contiguous() and rel_idx.is_contiguous()
  ...
  def backward(ctx, grad_output):
    ...
    grad_output = grad_output.contiguous()
    assert q.is_contiguous() and index_q.is_contiguous() and k.is_contiguous() and index_k.is_contiguous() and table_q.is_contiguous() and table_k.is_contiguous(
    ) and rel_idx.is_contiguous() and rel_idx_offsets.is_contiguous() and sort_indices.is_contiguous() and grad_output.is_contiguous()
...
class DotProdWithIdx_v3(Function):
  ...
  def forward(ctx, q, index_q_offsets, n_max, k, index_k, table_q, table_k, rel_idx):
    ...
    output: output: [M, h]
    """
    assert q.is_contiguous() and index_q_offsets.is_contiguous() and k.is_contiguous() and index_k.is_contiguous(
    ) and table_q.is_contiguous() and table_k.is_contiguous() and rel_idx.is_contiguous()
  ...
  def backward(ctx, grad_output):
    ...
    grad_output = grad_output.contiguous()
    assert q.is_contiguous() and index_q_offsets.is_contiguous() and k.is_contiguous() and index_k.is_contiguous(
    ) and table_q.is_contiguous() and table_k.is_contiguous() and rel_idx.is_contiguous() and grad_output.is_contiguous()
...
class AttentionStep2WithRelPosValue(Function):
  ...
  def forward(ctx, attn, v, index0, index1, table, rel_idx):
    ...
    """
    assert attn.is_contiguous() and v.is_contiguous() and index0.is_contiguous(
    ) and index1.is_contiguous() and table.is_contiguous() and rel_idx.is_contiguous()
  ...
  def backward(ctx, grad_output):
    ...
    # print("grad_output.is_contiguous(): ", grad_output.is_contiguous())
    assert attn.is_contiguous() and v.is_contiguous() and index0.is_contiguous() and index1.is_contiguous(
    ) and grad_output.is_contiguous() and table.is_contiguous() and rel_idx.is_contiguous()
