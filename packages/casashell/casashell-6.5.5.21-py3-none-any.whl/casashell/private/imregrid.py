...
##################### ec3b2b50981cea05bc279bf990506942 ##############################
...
class _imregrid:
    ...
    * the name of an image from which to get the coordinate system and shape.
    ...
    def __shape_dflt( self, glb ):
        if self.__template( glb ) != "get": return [int(-1)]
        return None
    ...
    def __template_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__template_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return 'get'
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'template': value},{'template': self.__schema['template']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('\x1B[1m\x1B[47m%-13.13s =\x1B[0m %s%-23s%s' % ('template',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __output_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('output',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __asvelocity_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__asvelocity_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return True
        description = 'Regrid spectral axis in velocity space rather than frequency space?'
        value = self.__asvelocity( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'asvelocity': value},{'asvelocity': self.__schema['asvelocity']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('asvelocity',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __axes_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('axes',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __shape_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__shape_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ int(-1) ]
        if self.__shape_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'shape': value},{'shape': self.__schema['shape']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('shape',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __interpolation_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('interpolation',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __decimate_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__decimate_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(10)
        description = 'Decimation factor for coordinate grid computation'
        value = self.__decimate( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'decimate': value},{'decimate': self.__schema['decimate']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('decimate',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __replicate_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__replicate_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        description = 'Replicate image rather than regrid?'
        value = self.__replicate( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'replicate': value},{'replicate': self.__schema['replicate']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('replicate',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# imregrid -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "imregrid.last" if os.path.isfile("imregrid.last") else "imregrid.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('imregrid.last')
