...
##################### ec19dbdd3c2dd6df7f47f3edfab28dfd ##############################
...
class _imhead:
    ...
    _info_group_ = """information, analysis, manipulation"""
    _info_desc_ = """List, get and put image header parameters"""

    __schema = {'imagename': {'type': 'cReqPath', 'coerce': _coerce.expand_path}, 'mode': {'type': 'cStr', 'coerce': _coerce.to_str, 'allowed': [ 'add', 'put', 'history', 'get', 'summary', 'list', 'del' ]}, 'hdkey': {'type': 'cStr', 'coerce': _coerce.to_str}, 'hdvalue': {'type': 'cVariant', 'coerce': [_coerce.to_variant]}, 'verbose': {'type': 'cBool'}}
    ...
    def __hdkey_inp(self):
        ...
        if self.__hdkey_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-7.7s =\x1B[0m %s%-23s%s' % ('hdkey',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __hdvalue_inp(self):
        ...
        if self.__hdvalue_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-7.7s =\x1B[0m %s%-23s%s' % ('hdvalue',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# imhead -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "imhead.last" if os.path.isfile("imhead.last") else "imhead.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('imhead.last')
