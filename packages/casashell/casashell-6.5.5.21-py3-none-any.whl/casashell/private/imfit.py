...
##################### e3febe6789df532aa118067b3a2ac230 ##############################
...
class _imfit:
    ...
    * If the image being analyzed is an interferometric image with poor uv sampling, the parameter
    ...
    def __includepix_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('includepix',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __excludepix_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('excludepix',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __residual_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('residual',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __model_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('model',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __estimates_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('estimates',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    def __newestimates_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__newestimates_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'newestimates': value},{'newestimates': self.__schema['newestimates']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('newestimates',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __complist_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-12.12s =\x1B[0m %s%-23s%s' % ('complist',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    ...
    def __dooff_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__dooff_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        description = 'Also fit a zero level offset? Default is False'
        value = self.__dooff( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'dooff': value},{'dooff': self.__schema['dooff']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('\x1B[1m\x1B[47m%-12.12s =\x1B[0m %s%-23s%s' % ('dooff',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __offset_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__offset_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(0.0)
        if self.__offset_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'offset': value},{'offset': self.__schema['offset']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-9.9s =\x1B[0m %s%-23s%s' % ('offset',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __fixoffset_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__fixoffset_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        if self.__fixoffset_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'fixoffset': value},{'fixoffset': self.__schema['fixoffset']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-9.9s =\x1B[0m %s%-23s%s' % ('fixoffset',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    ...
    def __rms_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__rms_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(0)
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'rms': value},{'rms': self.__schema['rms']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('rms',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __noisefwhm_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__noisefwhm_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'noisefwhm': value},{'noisefwhm': self.__schema['noisefwhm']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('noisefwhm',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __summary_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__summary_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        description = 'File name to which to write table of fit parameters.'
        value = self.__summary( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'summary': value},{'summary': self.__schema['summary']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('summary',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# imfit -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "imfit.last" if os.path.isfile("imfit.last") else "imfit.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('imfit.last')
