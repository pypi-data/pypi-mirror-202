...
##################### a81d50501849096b05583002a039cc1e ##############################
...
class _specfit:
    ...
    def __gmampest_dflt( self, glb ):
        if self.__gmncomps( glb ) != 0: return [float(0.0)]
        return None
    ...
    def __gmampcon_dflt( self, glb ):
        if self.__gmncomps( glb ) != 0: return { }
        return None
    ...
    def __box_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('box',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __region_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('region',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __chans_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('chans',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __stokes_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('stokes',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __axis_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('axis',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __mask_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('mask',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __ngauss_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__ngauss_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(1)
        if self.__ngauss_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'ngauss': value},{'ngauss': self.__schema['ngauss']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('ngauss',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __poly_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__poly_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(-1)
        description = 'Order of polynomial element.  Default: do not fit a polynomial (<0).'
        value = self.__poly( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'poly': value},{'poly': self.__schema['poly']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('poly',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __estimates_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__estimates_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        description = 'Name of file containing initial estimates.  Default: No initial estimates ("").'
        value = self.__estimates( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'estimates': value},{'estimates': self.__schema['estimates']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('estimates',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __minpts_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('minpts',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __multifit_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('multifit',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __model_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('model',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __residual_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('residual',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __amp_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__amp_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__amp_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'amp': value},{'amp': self.__schema['amp']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('amp',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __amperr_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__amperr_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__amperr_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'amperr': value},{'amperr': self.__schema['amperr']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('amperr',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __center_inp(self):
        ...
        if self.__center_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('center',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __centererr_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__centererr_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__centererr_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'centererr': value},{'centererr': self.__schema['centererr']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('centererr',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __fwhm_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__fwhm_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__fwhm_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'fwhm': value},{'fwhm': self.__schema['fwhm']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('fwhm',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __fwhmerr_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__fwhmerr_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__fwhmerr_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'fwhmerr': value},{'fwhmerr': self.__schema['fwhmerr']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('fwhmerr',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __integral_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__integral_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__integral_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'integral': value},{'integral': self.__schema['integral']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('integral',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __integralerr_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__integralerr_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__integralerr_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'integralerr': value},{'integralerr': self.__schema['integralerr']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('integralerr',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __wantreturn_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('wantreturn',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __stretch_inp(self):
        ...
        if self.__stretch_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('stretch',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __logresults_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('logresults',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __pampest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pampest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        if self.__pampest_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pampest': value},{'pampest': self.__schema['pampest']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('pampest',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pcenterest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pcenterest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        if self.__pcenterest_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pcenterest': value},{'pcenterest': self.__schema['pcenterest']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('pcenterest',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pfwhmest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pfwhmest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        if self.__pfwhmest_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pfwhmest': value},{'pfwhmest': self.__schema['pfwhmest']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('pfwhmest',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pfix_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pfix_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        if self.__pfix_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pfix': value},{'pfix': self.__schema['pfix']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('pfix',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __gmncomps_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmncomps_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(0)
        description = 'Number of components in each gaussian multiplet to fit'
        value = self.__gmncomps( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmncomps': value},{'gmncomps': self.__schema['gmncomps']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('gmncomps',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __gmampcon_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmampcon_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmampcon': value},{'gmampcon': self.__schema['gmampcon']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmampcon',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmcentercon_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmcentercon_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmcentercon': value},{'gmcentercon': self.__schema['gmcentercon']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmcentercon',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmfwhmcon_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmfwhmcon_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmfwhmcon': value},{'gmfwhmcon': self.__schema['gmfwhmcon']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmfwhmcon',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmampest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmampest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmampest': value},{'gmampest': self.__schema['gmampest']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmampest',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmcenterest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmcenterest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmcenterest': value},{'gmcenterest': self.__schema['gmcenterest']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmcenterest',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmfwhmest_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmfwhmest_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmfwhmest': value},{'gmfwhmest': self.__schema['gmfwhmest']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmfwhmest',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gmfix_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gmfix_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gmfix': value},{'gmfix': self.__schema['gmfix']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('gmfix',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __logfile_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('logfile',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __append_inp(self):
        ...
        if self.__append_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('append',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pfunc_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pfunc_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__pfunc_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pfunc': value},{'pfunc': self.__schema['pfunc']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('pfunc',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __goodamprange_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__goodamprange_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'goodamprange': value},{'goodamprange': self.__schema['goodamprange']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('goodamprange',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __goodcenterrange_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__goodcenterrange_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'goodcenterrange': value},{'goodcenterrange': self.__schema['goodcenterrange']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('goodcenterrange',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __goodfwhmrange_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__goodfwhmrange_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float(0.0) ]
        ...
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'goodfwhmrange': value},{'goodfwhmrange': self.__schema['goodfwhmrange']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('goodfwhmrange',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __sigma_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('sigma',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __outsigma_inp(self):
        ...
        if self.__outsigma_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('outsigma',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# specfit -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "specfit.last" if os.path.isfile("specfit.last") else "specfit.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('specfit.last')
