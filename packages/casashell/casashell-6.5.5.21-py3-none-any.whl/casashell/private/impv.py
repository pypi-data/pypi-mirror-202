...
##################### 9ab21b989585436ac77e0cb1c221919a ##############################
...
class _impv:
    ...
    def __start_inp(self):
        ...
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('start',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __end_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__end_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        description = 'The ending pixel in the direction plane (array of two values).'
        value = self.__end( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'end': value},{'end': self.__schema['end']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-12.12s = %s%-23s%s' % ('end',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __center_inp(self):
        ...
        if self.__center_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-9.9s =\x1B[0m %s%-23s%s' % ('center',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __length_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__length_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        if self.__length_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'length': value},{'length': self.__schema['length']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-9.9s =\x1B[0m %s%-23s%s' % ('length',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pa_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
    ...
    def __region_inp(self):
        ...
        if self.__region_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-9.9s =\x1B[0m %s%-23s%s' % ('region',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __chans_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-12.12s =\x1B[0m %s%-23s%s' % ('chans',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "impv.last" if os.path.isfile("impv.last") else "impv.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('impv.last')
