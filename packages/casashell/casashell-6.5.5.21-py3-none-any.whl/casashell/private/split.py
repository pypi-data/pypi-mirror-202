...
##################### 153f85b9b5e0c34f95358068857257ea ##############################
...
class _split:
    ...
    def __keepflags_inp(self):
        ...
        self.__do_inp_output('%-11.11s = %s%-23s%s' % ('keepflags',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __width_inp(self):
        ...
        self.__do_inp_output('%-11.11s = %s%-23s%s' % ('width',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __timebin_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-11.11s =\x1B[0m %s%-23s%s' % ('timebin',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __combine_inp(self):
        ...
        if self.__combine_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-8.8s =\x1B[0m %s%-23s%s' % ('combine',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# split -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "split.last" if os.path.isfile("split.last") else "split.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('split.last')
