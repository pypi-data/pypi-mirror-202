...
##################### 0547fb425e17ddf38c97521fa4ec472a ##############################
...
class _cvel:
    ...
    def __passall_inp(self):
        ...
        self.__do_inp_output('%-16.16s = %s%-23s%s' % ('passall',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    def __array_inp(self):
        ...
        if self.__array_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-13.13s =\x1B[0m %s%-23s%s' % ('array',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __mode_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "cvel.last" if os.path.isfile("cvel.last") else "cvel.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('cvel.last')
