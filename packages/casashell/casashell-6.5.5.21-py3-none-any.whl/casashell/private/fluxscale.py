...
##################### d755b0baa9e1470fc32bae479b20569e ##############################
...
class _fluxscale:
    ...
    refspwmap     Vector of spectral windows enabling scaling across
                  ...
                  Default: [-1] (none)
                     ...
                     only in spw 1 or 3.
    gainthreshold Threshold in the input gain solutions to be used in fractional deviation from median values.
    ...
    def __fluxtable_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__fluxtable_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return ''
        description = 'Name of output, flux-scaled calibration table (required)'
        value = self.__fluxtable( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'fluxtable': value},{'fluxtable': self.__schema['fluxtable']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('fluxtable',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __reference_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__reference_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        description = 'Reference field name(s) (transfer flux scale FROM)'
        value = self.__reference( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'reference': value},{'reference': self.__schema['reference']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('reference',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __transfer_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__transfer_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [  ]
        description = 'Transfer field name(s) (transfer flux scale TO), \'\' -> all'
        value = self.__transfer( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'transfer': value},{'transfer': self.__schema['transfer']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('transfer',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __listfile_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('listfile',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __append_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('append',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __refspwmap_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__refspwmap_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ int(-1) ]
        description = 'Scale across spectral window boundaries'
        value = self.__refspwmap( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'refspwmap': value},{'refspwmap': self.__schema['refspwmap']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('refspwmap',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __gainthreshold_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__gainthreshold_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(-1.0)
        description = 'Threshold (fractional deviation from the median) on gain amplitudes to be used in the flux scale calculation'
        value = self.__gainthreshold( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'gainthreshold': value},{'gainthreshold': self.__schema['gainthreshold']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('gainthreshold',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __antenna_inp(self):
        ...
        self.__do_inp_output('\x1B[1m\x1B[47m%-13.13s =\x1B[0m %s%-23s%s' % ('antenna',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __timerange_inp(self):
        ...
        if self.__timerange_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('timerange',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __scan_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
        ...
        if self.__scan_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('scan',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __incremental_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('incremental',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __fitorder_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('fitorder',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __display_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('display',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# fluxscale -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "fluxscale.last" if os.path.isfile("fluxscale.last") else "fluxscale.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('fluxscale.last')
