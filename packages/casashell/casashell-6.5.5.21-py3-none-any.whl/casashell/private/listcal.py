##################### generated by xml-casa (v2) from listcal.xml ###################
##################### b343a7988fdaf2c3b471d36e6403326b ##############################
...
class _listcal:
    ...
    _info_desc_ = """List antenna gain solutions"""

    __schema = {'vis': {'type': 'cReqPath', 'coerce': _coerce.expand_path}, 'caltable': {'type': 'cReqPath', 'coerce': _coerce.expand_path}, 'field': {'type': 'cStr', 'coerce': _coerce.to_str}, 'antenna': {'type': 'cStr', 'coerce': _coerce.to_str}, 'spw': {'type': 'cStr', 'coerce': _coerce.to_str}, 'listfile': {'type': 'cStr', 'coerce': _coerce.to_str}, 'pagerows': {'type': 'cInt'}}
    ...
    def __antenna_inp(self):
        ...
        self.__do_inp_output('%-8.8s = %s%-23s%s' % ('antenna',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __spw_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
    ...
    def __listfile_inp(self):
        ...
        self.__do_inp_output('%-8.8s = %s%-23s%s' % ('listfile',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __pagerows_inp(self):
        ...
        self.__do_inp_output('%-8.8s = %s%-23s%s' % ('pagerows',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# listcal -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "listcal.last" if os.path.isfile("listcal.last") else "listcal.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('listcal.last')
