...
class _exportuvfits:
    ...
    def __fitsfile_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('fitsfile',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    def __antenna_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('antenna',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __timerange_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('timerange',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __writesyscal_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__writesyscal_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        description = 'Write GC and TY tables (not yet available)'
        value = self.__writesyscal( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'writesyscal': value},{'writesyscal': self.__schema['writesyscal']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('writesyscal',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __multisource_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__multisource_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return True
        description = 'Write in multi-source format?'
        value = self.__multisource( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'multisource': value},{'multisource': self.__schema['multisource']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('multisource',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __combinespw_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__combinespw_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return True
        description = 'Export the spectral windows as IFs'
        value = self.__combinespw( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'combinespw': value},{'combinespw': self.__schema['combinespw']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('\x1B[1m\x1B[47m%-15.15s =\x1B[0m %s%-23s%s' % ('combinespw',pre,self.__to_string_(value),post),description,13+len(pre)+len(post))
    def __writestation_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__writestation_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return True
        description = 'Write station name instead of antenna name'
        value = self.__writestation( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'writestation': value},{'writestation': self.__schema['writestation']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('writestation',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __padwithflags_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__padwithflags_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        if self.__padwithflags_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'padwithflags': value},{'padwithflags': self.__schema['padwithflags']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-12.12s =\x1B[0m %s%-23s%s' % ('padwithflags',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __overwrite_inp(self):
        ...
        self.__do_inp_output('%-15.15s = %s%-23s%s' % ('overwrite',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# exportuvfits -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "exportuvfits.last" if os.path.isfile("exportuvfits.last") else "exportuvfits.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('exportuvfits.last')
