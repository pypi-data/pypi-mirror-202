...
class _sdfixscan:
    ...
    def __infiles_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('infiles',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __mode_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
    ...
    def __numpoly_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__numpoly_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return int(2)
        if self.__numpoly_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'numpoly': value},{'numpoly': self.__schema['numpoly']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('numpoly',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __beamsize_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__beamsize_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(0.0)
        if self.__beamsize_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'beamsize': value},{'beamsize': self.__schema['beamsize']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('beamsize',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __smoothsize_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__smoothsize_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(2.0)
        if self.__smoothsize_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'smoothsize': value},{'smoothsize': self.__schema['smoothsize']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('smoothsize',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __direction_inp(self):
        ...
        if self.__direction_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('direction',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __maskwidth_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__maskwidth_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(1.0)
        if self.__maskwidth_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'maskwidth': value},{'maskwidth': self.__schema['maskwidth']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('maskwidth',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __tmax_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__tmax_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(0.0)
        description = 'maximum threshold value for processing'
        value = self.__tmax( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'tmax': value},{'tmax': self.__schema['tmax']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('tmax',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __tmin_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__tmin_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(0.0)
        description = 'minimum threshold value for processing'
        value = self.__tmin( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'tmin': value},{'tmin': self.__schema['tmin']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('tmin',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# sdfixscan -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "sdfixscan.last" if os.path.isfile("sdfixscan.last") else "sdfixscan.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('sdfixscan.last')
