...
##################### fccb7fde89f14ceba514bdd8be54ae42 ##############################
...
class _virtualconcat:
    ...
    def __concatvis_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('concatvis',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __freqtol_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('freqtol',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __dirtol_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('dirtol',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __respectname_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('respectname',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __visweightscale_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('visweightscale',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __keepcopy_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__keepcopy_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return False
        description = 'If true, a copy of the input MSs is kept in their original place.'
        value = self.__keepcopy( self.__globals_( ) )
        (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'keepcopy': value},{'keepcopy': self.__schema['keepcopy']}) else ('\x1B[91m','\x1B[0m')
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('keepcopy',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __copypointing_inp(self):
        ...
        self.__do_inp_output('%-14.14s = %s%-23s%s' % ('copypointing',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# virtualconcat -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "virtualconcat.last" if os.path.isfile("virtualconcat.last") else "virtualconcat.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('virtualconcat.last')
