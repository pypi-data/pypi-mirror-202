...
class _widebandpbcor:
    ...
    def __nterms_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('nterms',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    def __threshold_inp(self):
        ...
        self.__do_inp_output('%-13.13s = %s%-23s%s' % ('threshold',pre,self.__to_string_(value),post),description,0+len(pre)+len(post))
    ...
    def __reffreq_inp(self):
        ...
        if self.__reffreq_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('reffreq',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __pbmin_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__pbmin_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return float(0.2)
        if self.__pbmin_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'pbmin': value},{'pbmin': self.__schema['pbmin']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('pbmin',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __field_inp(self):
        ...
        if self.__field_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('field',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __spwlist_inp(self):
        ...
        if self.__spwlist_dflt( self.__globals_( ) ) is not None:
             ...
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('spwlist',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __chanlist_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__chanlist_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ int() ]
        if self.__chanlist_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'chanlist': value},{'chanlist': self.__schema['chanlist']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('chanlist',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    def __weightlist_inp(self):
        def xml_default( ):
            ## play the crazy subparameter shell game
            dflt = self.__weightlist_dflt( self.__globals_( ) )
            if dflt is not None: return dflt
            return [ float() ]
        if self.__weightlist_dflt( self.__globals_( ) ) is not None:
             ...
             (pre,post) = (('','') if value == xml_default( ) else ('\x1B[34m','\x1B[0m')) if self.__validate_({'weightlist': value},{'weightlist': self.__schema['weightlist']}) else ('\x1B[91m','\x1B[0m')
             self.__do_inp_output('   \x1B[92m%-10.10s =\x1B[0m %s%-23s%s' % ('weightlist',pre,self.__to_string_(value),post),description,9+len(pre)+len(post))
    ...
    #--------- inp function -----------------------------------------------------------
    def inp(self):
        print("# widebandpbcor -- %s" % self._info_desc_)
    ...
    def tget(self,savefile=None):
        ...
        filename = savefile
        if filename is None:
            filename = "widebandpbcor.last" if os.path.isfile("widebandpbcor.last") else "widebandpbcor.saved"
    ...
    def tput(self,outfile=None):
        ...
        _postfile = outfile if outfile is not None else os.path.realpath('widebandpbcor.last')
