...
class TargetConfigurationMissingError(Exception):
    """Raised when there's no configuration for a given target."""

    def __init__(self, target: str) -> None:
        """Construct class."""
        super().__init__(f"No configuration for target `{target}`")
