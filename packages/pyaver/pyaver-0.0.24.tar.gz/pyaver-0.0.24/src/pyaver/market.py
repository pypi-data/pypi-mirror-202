...
class AverMarket():
    ...
    async def make_sweep_fees_instruction(self):
        ...
        print('ABOUTO TO SWEEP FEES')
        print("market",  self.market_pubkey)
        print("quote_vault", self.market_state.quote_vault)
        print("vault_authority", self.market_state.vault_authority)
        print("third_party_token_vault", third_party_vault_token_account)
        print("aver_quote_token_account", aver_quote_token_account)
        print("spl_token_program", TOKEN_PROGRAM_ID)
    ...
    async def sweep_fees(self, fee_payer: Keypair = None, send_options: TxOpts = None,):
        ...
        print('MADE the IX', ix)
        print('Sending the TX')

        tx = Transaction()
        tx.add(ix)
        if(send_options == None):
            send_options = self.aver_client.provider.opts
        print('Sending the TX')
        tx_result = await self.aver_client.provider.connection.send_transaction(tx, [fee_payer], opts=send_options)
        print('Got the result', tx_result)
