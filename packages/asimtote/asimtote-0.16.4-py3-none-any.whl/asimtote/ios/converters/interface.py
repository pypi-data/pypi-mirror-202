...
class Cvt_Int_IPIGMPVer(DiffConvert_Int):
    ...
    def update(self, old, upd, new, int_name):
        return self.enter(int_name) + [" ip igmp version %d" % new]
...
class Cvt_Int_IPPIMMode(DiffConvert_Int):
    ...
    def update(self, old, upd, new, int_name):
        return self.enter(int_name) + [" ip pim %s-mode" % new]
...
class Cvt_Int_IPProxyARP(DiffConvert_Int):
    ...
    def remove(self, old, int_name):
        ...
        if not old:
            return self.enter(int_name) + [" ip proxy-arp"]
...
class Cvt_Int_IPv6MultBdry(DiffConvert_Int):
    ...
    def update(self, old, upd, new, int_name):
        return self.enter(int_name) + [
            " ipv6 multicast boundary scope %d" % new]
...
class Cvt_Int_IPv6NDPfx(DiffConvert_Int):
    ...
    def remove(self, old, int_name, pfx):
        return self.enter(int_name) + [" no ipv6 nd prefix " + pfx]

    def update(self, old, upd, new, int_name, pfx):
        ...
        return self.enter(int_name) + [" ipv6 nd prefix %s %s" % (pfx, e)]
