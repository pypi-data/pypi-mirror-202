...
class DiffConvert:
    ...
    * filter() - by default, the above method calls the same filter()
    ...
    split_args_offset -- this is used by the split_args() method: see
    that for more information; it defaults to 0 (causing split_args() to
    use the natural boundary between the context and cmd/ext)
    """
    ...
    def __init__(self):
        ...
        # and used by split_args() method
        #
        # by default, this counts the number of wildcard arguments in
        # the context but can be adjusted with split_args_offset
        #
        ...
        self._context_args = (
            len([ i for i, v in enumerate(self.context)
                     if (v is None) or isinstance(v, set) ])
            + self.split_args_offset)


    def split_args(self, args):
        """Split a list of wildcard arguments into a tuple - those used
        in the context vs those used afterwards (by the cmd and ext
        parts).

        This is useful when writing converters which can work in several
        different contexts (such as those for commands in BGP
        configuration).
        ...
        The boundary between these can be adjusted from what would
        automatically be inferred by setting split_args_offset.
        """

        return args[:self._context_args], args[self._context_args:]
    ...
    def filter(self, *args):
        ...
        call by default, if all types of action are to be equially
        filtered.
        """
    ...
    def delete(self, old, rem, new, *args):
        ...
        for cvt in chain.from_iterable(
            ...
            for match in all_matches:
                ...
                if match in remove_matches:
                    ...
                    try:
                        filtered = cvt.filter_delete(*args)
                    ...
                    # call the delete converter action method using the
                ...
                if match in update_matches:
                    ...
                    try:
                        filtered = cvt.filter_update(*args)
                ...
                if ((match in triggered_matches)
                    ...
                    try:
                        filtered = cvt.filter_trigger(*args)
