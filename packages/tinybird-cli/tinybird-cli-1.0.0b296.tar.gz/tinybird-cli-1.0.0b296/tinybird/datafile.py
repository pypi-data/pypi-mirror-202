...
class PipeCheckerRunner:
    ...
    def _delta_percentage(self, checker: float, current: float) -> float:
        try:
            ...
            logging.exception(f"Error calculating delta: {exc}")
            return 0.0
...
async def build_graph(filenames, tb_client: TinyB, dir_path=None, tag='', resource_versions=None, workspace_map=None, process_dependencies=False, verbose=False, skip_connectors=False, workspace_lib_paths=None, current_ws=None, changed=None, only_changes=False):  # noqa: C901 B006
    """process files"""
    to_run: Dict[str, Any] = {}
    deps: List[str] = []
    ...
    if process_dependencies:
        if only_changes:
            for key in dict(to_run):
                ...
                if target_datasource := get_target_materialized_data_source_name(to_run[key]):
...
async def get_changes_from_main(only_changes: bool, client: TinyB, config: Optional[Dict[str, Any]] = None, current_ws: Optional[Dict[str, Any]] = None):
    changed = None
    if not only_changes:
        return changed

    if current_ws and not current_ws.get('is_branch'):
        changed = await diff_command('.', True, client, no_color=True, with_print=False)
...
async def folder_push(
    ...
    changed = await get_changes_from_main(only_changes, tb_client, config, current_ws) if config else None
...
def is_endpoint_with_no_dependencies(resource, dep_map, to_run):
    if not resource or len(resource.get('tokens', [])) == 0 or resource.get('resource') == 'datasources':
        return False
