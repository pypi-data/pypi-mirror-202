...
def get_html(X):
    ...
    tr = '<tr bgcolor="white"><th colspan="3" style="center"> Predicted origin:  ' + str(place) + '</th><tr>'  # <th> Value </th><th> Details </th>

    html += tr

    tr = '<tr><td colspan="3"> ' + str('Inner Mongolia wild :' + str(round(pro[0]*100,2))+str('%')+ ', '+'Inner Mongolia cultivated :'+str(round(pro[1]*100,2))+str('%')+', '+'Inner Mongolia black bupleurum:'+str(round(pro[2]*100,2))+str('%')+', '+'Gansu :'+str(round(pro[3]*100,2))+str('%')+', '+'Shaanxi :'+str(round(pro[4]*100,2)))+str('%')+', ' + '</td>' '<tr>'
    html += tr

    tr = '<tr><th> Saikosaponin A, SSa (mg/L)</th>' '<th> Saikosaponin C, SSc (mg/L)</th><th> Saikosaponin D, SSd (mg/L)</th><tr>'  # <th> Value </th><th> Details </th>
    html += tr
