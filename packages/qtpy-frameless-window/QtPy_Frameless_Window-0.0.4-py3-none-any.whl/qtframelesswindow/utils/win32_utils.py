...
def isGreaterEqualVersion(version):
    ...
    def get_version(version:str):
        print(version)
        start=version.index(', ')+2
        end=version.index(")")
        return [int(x) for x in version[start:end].split('.')]
    def compare(a,b):
        index=0
        while(index<3):
            if a[index]!=b[index]:
    ...
    try:
        ...
        current_version=get_version(str(QOperatingSystemVersion.current()))
        version=get_version(str(version))
        return compare(current_version,version)
