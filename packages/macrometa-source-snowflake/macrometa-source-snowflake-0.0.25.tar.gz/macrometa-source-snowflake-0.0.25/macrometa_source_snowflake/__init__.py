...
def do_sync_cdc(snowflake_conn, catalog_entry, state, columns, replication_method, stream_name):
    LOGGER.info('Stream %s is using CDC replication', catalog_entry.stream)

    write_schema_message(catalog_entry)

    cdc.sync_table(snowflake_conn, catalog_entry, state,
                   columns, replication_method, stream_name)

    singer.write_message(singer.StateMessage(value=copy.deepcopy(state)))


def sync_streams(snowflake_conn, catalog, state, integration, replication_method):
    stream_name = integration['table']
    ...
    for catalog_entry in catalog.streams:
        ...
        with metrics.job_timer('sync_table') as timer:
            ...
            if replication_method == 'INCREMENTAL':
                ...
                do_sync_cdc(
                    snowflake_conn, catalog_entry, state, columns, replication_method, stream_name)
            else:
                raise Exception(
...
def do_sync(snowflake_conn, config, catalog, state, replication_method, integration):
    catalog = get_streams(snowflake_conn, catalog, config, state)
    sync_streams(snowflake_conn, catalog, state,
                 integration, replication_method)
    ...
    try:
        ...
        if args.discover:
            ...
            do_sync(snowflake_conn, config,
                    args.catalog, state, replication_method, config['integration'])
        elif args.properties:
            catalog = Catalog.from_dict(args.properties)
            state = args.state or {}
            do_sync(snowflake_conn, config,
                    catalog, state, replication_method, config['integration'])
        else:
