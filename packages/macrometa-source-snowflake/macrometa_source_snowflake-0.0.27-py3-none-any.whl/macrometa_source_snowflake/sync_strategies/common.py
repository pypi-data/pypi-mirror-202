...
def create_stream(catalog_entry):
    ...
    query = f"""
    CREATE STREAM {database_name}.{schema_name}.{stream_name}
    ...
    with metrics.record_counter(None) as counter:
        ...
        while row:
            ...
            if replication_method == 'CDC':
                metadata_action = record_message.record.get(
                    'METADATA$ACTION', None)
                metadata_update = record_message.record.get(
                ...
                if metadata_action == 'DELETE':
                    if metadata_update:
                        ...
                        record_message['_sdc_deleted_at'] = singer.utils.strftime(
                            utils.now())
