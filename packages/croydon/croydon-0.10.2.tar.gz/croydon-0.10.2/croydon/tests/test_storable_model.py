...
class TestStorableModel(MongoMockTest):
    ...
    @patch("croydon.context.ctx._cache_l2", spec=True)
    @patch("croydon.context.ctx._cache_l1", spec=True)
    async def test_invalidate_cached_method(self, l1c, l2c):
        ...
        l1c.delete.assert_called_once_with("user.Dilan")

        _ = await user.full_name()
        l1c.get.assert_called_once_with(f"user.{user.id}.full_name")

        await user.save()
        l1c.delete.assert_any_call(f"user.Dilan")
        l1c.delete.assert_any_call(f"user.{user.id}")
        l1c.delete.assert_any_call(f"user.{user.id}.full_name")
