...
class Client(Socket, Service):
  ...
  def soborol_login(self, email: str, mojo_id_token: str):
    ...
    return SoborolUser(self.postRequest("https://api.soborol.com/parse/users", data, headers=headers.Headers().soboro))
...
{"friendsOnly": friendsOnly})
  ...
  def get_meshes(self, limit: int = 1, public: bool = False, friends: bool = False, local: bool = False, invited: bool = False, lang: str = "en"):
    return MeshesList(self.get("/meshes/self", {"deviceId": self.deviceId,"public": public,"local": local, "invited": invited, "limit": limit, "lang": lang}))

  def get_mesh_info(self, meshId: str):
      return MeshInfo(self.get(f"/meshes/{meshId}"))

  def invite_to_mesh(self, meshId: str, usersId: list):
    return self.post(f"/meshes/{meshId}/invites", {"deviceId": self.deviceId, "ids": usersId})
  ...
  def like_mesh_video(self, meshId: str, video_url):
    return self.put(f"/meshes/{meshId}/likeskip", {"opinion":"LIKE","url": video_url,"videoInstanceId": nonce()})

  def dislike_mesh_video(self, meshId: str, video_url):
    return self.delete(f"/meshes/{meshId}/likeskip", {"opinion":"LIKE","url": video_url,"videoInstanceId": nonce()})
    
  #def get_mesh_videos_queue(self, meshId: str):

  def edit_mesh(self, meshId: str, voipMode: str = "ALL", isFriend: bool = False, isLocal: bool = False, isPublic: bool = True, lat: float  = 0.0, lng: float = 0.0, radius: float = 0.0):
    data = {
      "isFriend": isFriend,
      "isLocal": isLocal,
      "isPublic": isPublic,
      ...
      "playMode": "VOTE",
      "radius": radius,
      "voipMode": voipMode # ALL: Can Every one join the mice chat, OFF: no one can join the mice chat
    }
    data = dumps(data)
