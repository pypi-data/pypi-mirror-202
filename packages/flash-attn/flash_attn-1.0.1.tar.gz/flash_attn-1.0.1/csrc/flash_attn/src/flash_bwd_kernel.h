...
inline __device__ void compute_dot_do_o(const Params &params) {
    ...
    // TODO: for hdim not power of 2, we'll need smem
    // // Shared memory.
...
inline __device__ void compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    const index_t row_offset_k = binfo.k_offset(params.k_batch_stride, params.k_row_stride, bidb)
        + n_block * kBlockN * params.k_row_stride + bidh * params.k_head_stride;
    ...
    Tensor sdO = make_tensor(sQ.data() + 2 * size(sQ), typename Kernel_traits::SmemLayoutQdO{});
