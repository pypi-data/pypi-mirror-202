...
from tala.backend.dependencies.for_running import BackendDependenciesForRunning
...
def create_orchestrated_domain_bundle(args):
    logger = configure_structlog_and_create_logger(args)
    dependencies = BackendDependenciesForRunning(args, logger)
    _store_backend_dependencies(dependencies, args.output)


def serialize_ods(dependencies):
    zipped_ods = zlib.compress(dill.dumps(dependencies))
    return base64.b64encode(zipped_ods).decode()


def _store_backend_dependencies(dependencies, output_dir):
    print(f"storing orchestrated domain bundle in file '{output_dir}'")
    file = open(output_dir, 'w')
    file.write(serialize_ods(dependencies))
    file.close()
