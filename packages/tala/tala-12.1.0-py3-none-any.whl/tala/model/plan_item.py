...
class PlanItem(SemanticObject, AsSemanticExpressionMixin):
    ...
    def is_respond_plan_item(self):
        return self._type == self.TYPE_RESPOND

    def isRespondPlanItem(self):
        return self.is_respond_plan_item()
    ...
    def is_forget_shared_plan_item(self):
        return self._type == self.TYPE_FORGET_SHARED
    ...
    def is_invoke_domain_query_plan_item(self):
        return self._type == self.TYPE_INVOKE_DOMAIN_QUERY
...
class RespondToInsultPlanItem(PlanItem):
    def __init__(self):
        PlanItem.__init__(self, PlanItem.TYPE_RESPOND_TO_INSULT)
...
class ForgetSharedPlanItem(PlanItemWithSemanticContent):
    def __init__(self, predicate_or_proposition):
        PlanItemWithSemanticContent.__init__(self, PlanItem.TYPE_FORGET_SHARED, predicate_or_proposition)
...
class InvokeQueryPlanItem(PlanItemWithSemanticContent):
...
class InvokeServiceQueryPlanItem(InvokeQueryPlanItem):
...
class InvokeDomainQueryPlanItem(InvokeQueryPlanItem):
    def __init__(self, issue, min_results=None, max_results=None):
        min_results = min_results or 0
        if min_results < 0:
            raise MinResultsNotSupportedException("Expected 'min_results' to be 0 or above but got %r." % min_results)
        ...
        PlanItemWithSemanticContent.__init__(self, PlanItem.TYPE_INVOKE_DOMAIN_QUERY, issue)
        self._min_results = min_results
        self._max_results = max_results
...
class EndTurnPlanItem(PlanItem):
    def __init__(self, timeout):
        PlanItem.__init__(self, PlanItem.TYPE_END_TURN)
...
class ResetDomainQueryPlanItem(PlanItem):
    def __init__(self, query):
        PlanItem.__init__(self, PlanItem.TYPE_RESET_DOMAIN_QUERY)
        self._query = query
...
class IteratePlanItem(PlanItem):
    def __init__(self, iterator):
        PlanItem.__init__(self, PlanItem.TYPE_ITERATE)
...
class ChangeDDDPlanItem(PlanItem):
    def __init__(self, ddd):
        PlanItem.__init__(self, PlanItem.TYPE_CHANGE_DDD)
        self._ddd = ddd
