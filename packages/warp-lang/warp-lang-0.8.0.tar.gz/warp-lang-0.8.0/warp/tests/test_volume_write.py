...
@wp.kernel
def test_volume_readback_f(volume: wp.uint64,
                           points: wp.array(dtype=wp.vec3),
...
@wp.kernel
def test_get_list_of_tiles(volume: wp.uint64,
...
@wp.kernel
def test_volume_tile_store_f(volume: wp.uint64,
...
@wp.kernel
def test_volume_tile_store_ws_f(volume: wp.uint64,
                                tiles: wp.array(dtype=wp.vec3)):
...
@wp.kernel
def test_volume_tile_readback_f(volume: wp.uint64,
                                tiles: wp.array2d(dtype=wp.int32),
                                values: wp.array(dtype=wp.float32)):
...
@wp.kernel
def test_volume_tile_store_v(volume: wp.uint64,
                             tiles: wp.array2d(dtype=wp.int32)):
...
@wp.kernel
def test_volume_tile_readback_v(volume: wp.uint64,
                                tiles: wp.array2d(dtype=wp.int32),
    ...
    class TestVolumes(parent):
        ...
        def test_volume_allocation(self):
            ...
            for device in devices:
                ...
                volume_a = wp.Volume.allocate(-bb_max, bb_max, voxel_size=voxel_size, bg_value=background_value, translation=translation, device=device)
                volume_b = wp.Volume.allocate(-bb_max * voxel_size + translation, bb_max * voxel_size + translation,
                                              voxel_size=voxel_size, bg_value=background_value, translation=translation,
                                              points_in_world_space=True, device=device)
