...
def test_constructors(test, device, dtype, register_kernels=False):
    ...
    val = input.numpy()[0]
    assert_np_equal(v2.numpy()[0], np.array([val,val]), tol=1.e-6)
    assert_np_equal(v3.numpy()[0], np.array([val,val,val]), tol=1.e-6)
    assert_np_equal(v4.numpy()[0], np.array([val,val,val,val]), tol=1.e-6)
    assert_np_equal(v5.numpy()[0], np.array([val,val,val,val,val]), tol=1.e-6)
    
    assert_np_equal(v20.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v21.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v30.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v31.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v32.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v40.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v41.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v42.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v43.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v50.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v51.numpy()[0], 2 * val, tol=1.e-6)
    assert_np_equal(v52.numpy()[0], 2 * val, tol=1.e-6)
...
def test_crossproduct(test,device,dtype, register_kernels=False):
    ...
    if dtype in np_float_types:
        ...
        assert_np_equal(tape.gradients[s3].numpy(), 2.0 * np.array([0,v3.numpy()[0,2],-v3.numpy()[0,1]]), tol=10*tol)
        assert_np_equal(tape.gradients[v3].numpy(), 2.0 * np.array([0,-s3.numpy()[0,2],s3.numpy()[0,1]]), tol=10*tol)
        tape.zero()
        
        tape.backward(loss=c1)
        assert_np_equal(tape.gradients[s3].numpy(), 2.0 * np.array([-v3.numpy()[0,2],0,v3.numpy()[0,0]]), tol=10*tol)
        assert_np_equal(tape.gradients[v3].numpy(), 2.0 * np.array([s3.numpy()[0,2],0,-s3.numpy()[0,0]]), tol=10*tol)
        tape.zero()
        
        tape.backward(loss=c2)
        assert_np_equal(tape.gradients[s3].numpy(), 2.0 * np.array([v3.numpy()[0,1],-v3.numpy()[0,0],0]), tol=10*tol)
