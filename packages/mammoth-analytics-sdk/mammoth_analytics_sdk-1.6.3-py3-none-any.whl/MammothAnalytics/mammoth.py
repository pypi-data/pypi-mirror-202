...
from MammothAnalytics.const import REQUEST_STATUS, USER_PERMISSIONS, RESERVED_BATCH_COLUMN_INTERNAL_NAMES_AND_KEYS
...
class MammothConnector(object):
    def __init__(self, workspace_id=None, api_url=None, api_key=None, api_secret=None):
        """
    ...
    def list_users(self):
        ...
        :return: List of dictionaries. Each dictionary contains id, name and other properties associated with an workspace
        """
        response = self._make_signed_request('get', '/workspaces/{workspace_id}/users'.format(workspace_id=self.__workspace_id))
        return response.json()['users']

    @handleError
    def add_user_to_workspace(self, email, full_name, user_permission=USER_PERMISSIONS.ANALYST,
    ...
    def remove_user_from_workspace(self, user_id):
        self._make_signed_request('delete', '/workspaces/{workspace_id}/users/{user_id}'.format(
    ...
    def get_task(self, view_id):
        ...
        response = self._make_signed_request('get', '/dataviews/{0}/tasks'.format(view_id))
        future_id = response.json()['future_id']
    ...
    def add_task(self, view_id, params, wait_till_ready=True):
        ...
        r = self._make_signed_request('post', '/dataviews/{0}/tasks'.format(view_id), json={'param': params})
        response = r.json()
    ...
    def update_task(self, view_id, task_id, params, wait_till_ready=True):
        ...
        r = self._make_signed_request('patch', '/dataviews/{0}/tasks/{1}'.format(view_id, task_id),
    ...
    def delete_task(self, view_id, task_id, wait_till_ready=True, skip_validation=False):
        ...
        r = self._make_signed_request('delete', '/dataviews/{0}/tasks/{1}?skip_validation={2}'.format(view_id, task_id,
                                                                                                       skip_validation_arg))
        response = r.json()
        if wait_till_ready and response['information']['status'] == 'processing':
            response = self.track_future_id(response['information']['future_id'])
    ...
    def get_task_statuses(self, view_id):
        response = self._make_signed_request('get', 'dataviews/{0}/task-statuses'.format(view_id))
    ...
    def append_datasets(self, source_dataset_id, target_dataset_id, column_mapping, change_map=None,
        ...
        for dataview in dataviews:
            ...
            dataview_data.append({'view_id': view_id, 'last_updated_at': last_updated_at})
    ...
    def list_views(self, ds_id):
        ...
        """
        response = self._make_signed_request('get', '/datasets/{0}/dataviews'.format(ds_id))
    ...
    def create_view(self, ds_id, duplicate_from_view_id=None):
        ...
        try:
            ...
            response = self._make_signed_request('post', '/datasets/{0}/dataviews'.format(ds_id), json=params)
    ...
    def delete_view(self, view_id, wait_till_ready=True):
        ...
        """
        params = {}
        r = self._make_signed_request('post', '/dataviews/{}/safe-delete-request'.format(view_id), json=params)
        response = r.json()
        if wait_till_ready and 'future_id' in response and response["STATUS"] == REQUEST_STATUS.PROCESSING:
    ...
    def reset_view(self, view_id, wait_till_ready=True):
        ...
        :return: response
        """
        r = self._make_signed_request('post', '/dataviews/{0}/reset'.format(view_id), json={})
        response = r.json()
        if wait_till_ready and 'future_id' in response and response["STATUS"] == REQUEST_STATUS.PROCESSING:
            response = self.track_future_id(response['future_id'])
        return response
    ...
    def rerun_pipeline(self, view_id, force_run_pipeline=False):
        ...
        response = self._make_signed_request('post', '/dataviews/{}/rerun'.format(view_id), json=params)
        return response.json()

    def reorder_tasks(self, view_id, param, wait_till_ready=True):
        log.debug("Reordering tasks for view_id: {0}".format(view_id))
        r = self._make_signed_request('patch', '/dataviews/{0}/tasks'.format(view_id),
    ...
    def get_view_data(self, view_id, columns=None, condition=None, limit=None, offset=None):
        ...
        response = self._make_signed_request('post', 'dataviews/{dataview_id}/data'.format(dataview_id=view_id),
                                             json=paging_params)
        future_id = response.json()['future_id']
        future_tracking_response = self.track_future_id(future_id)
        future_response = future_tracking_response['response']
    ...
    def run_transform(self, view_id, param):
        r = self._make_signed_request('post', '/dataviews/{0}/tasks'.format(view_id), json={'param': param})
        return r.json()
    ...
    def set_view_properties(self, view_id, properties):
        ...
        r = self._make_signed_request('post', '/dataviews/{0}'.format(view_id), json=properties)
        future_id = r.json()['future_id']
        future_tracking_response = self.track_future_id(future_id)
    ...
    def add_action(self, view_id, param, wait_till_ready=False):
        ...
        """
        r = self._make_signed_request('post', '/dataviews/{0}/actions'.format(view_id), json={'param': param})
        response = r.json()
        if wait_till_ready and 'future_id' in response and response["STATUS"] == REQUEST_STATUS.PROCESSING:
            response = self.track_future_id(response['future_id'])
        return response
    ...
    def run_action(self, view_id, action_id, param, wait_till_ready=False):
        ...
        :return: Request Id for the action
        """
        r = self._make_signed_request('post', '/dataviews/{0}/actions/{1}'.format(view_id, action_id),
                                      json={'param': param})
        response = r.json()
        if wait_till_ready and 'future_id' in response and response["STATUS"] == REQUEST_STATUS.PROCESSING:
            response = self.track_future_id(response['future_id'])
    ...
    def edit_action(self, view_id, action_id, param, wait_till_ready=False):
        ...
        r = self._make_signed_request('patch', '/dataviews/{0}/actions/{1}'.format(view_id, action_id),
    ...
    def list_actions(self, view_id):
        ...
        response = self._make_signed_request('get', '/dataviews/{0}/actions'.format(view_id))
        future_id = response.json()['future_id']
        future_tracking_response = self.track_future_id(future_id)
        future_response = future_tracking_response['response']
    ...
    def delete_action(self, view_id, action_id):
        ...
        :return:
        """
        r = self._make_signed_request('delete', '/dataviews/{0}/actions/{1}'.format(view_id, action_id))
        return r.json()
    ...
    def add_publish(self, view_id, params, wait_till_ready=True):
        r = self._make_signed_request('post', '/dataviews/{0}/publish'.format(view_id), json={'param': params})
    ...
    def delete_publish(self, view_id, publish_trigger_id):
        ...
        response = self._make_signed_request('delete', f'/dataviews/{view_id}/publish/{publish_trigger_id}')
    ...
    def rename_view(self, dataview_id, dataview_name):
        ...
        """
        res = self._make_signed_request('patch', '/dataviews/{0}'.format(dataview_id),
                                        json={"patch": [{"op": "replace", "path": "name",
                                                         "value": dataview_name}]})
        response = res.json()
        return response
    ...
    def apply_template(self, view_id, params):
        ...
        """
        res = self._make_signed_request('post', 'dataviews/{0}/exportable-config'.format(view_id),
                                        json={'config': params})
        response = res.json()
        return response
    ...
    @handleError
    def wait_till_batch_is_updated(self, ds_id, batch_count=None, batch_creation_timestamp=None):
        """
        ...
        if not batch_count and not batch_creation_timestamp:
            raise RuntimeError(f"Either of batch_count or batch_creation_timestamp should be passed")

        retry_count = 0
        if batch_creation_timestamp:
            batch_creation_timestamp = datetime.strptime(batch_creation_timestamp, DATE_FORMAT)
    ...
    def get_explore_data(self, view_id, params, wait_till_ready=True):
        ...
        """
        r = self._make_signed_request('post', '/dataviews/{0}/data/query'.format(view_id), json=params)
        response = r.json()
        if wait_till_ready:
    ...
    def change_user_role(self, workspace_id, user_id, user_role):
        ...
        """
        response = self._make_signed_request('patch', '/workspaces/{0}/users/{1}'.format(workspace_id, user_id), json={
    ...
    def add_user(self, add_user_param):
        ...
        """
        response = self._make_signed_request('post', '/workspaces/{0}/users'.format(self.__workspace_id),
                                             json=add_user_param)
        return response.json()
    ...
    def remove_user(self, user_id):
        ...
        """
        response = self._make_signed_request('delete', '/workspaces/{0}/users/{1}'.format(self.__workspace_id, user_id))
        return response.json()
    ...
    def get_current_plan_details(self):
        ...
        response = self._make_signed_request('get', '/workspaces/{0}/sms-details'.format(self.__workspace_id))
    ...
    def get_user_app_usage(self, user_id):
        ...
        Args:
            user_id: user id
        """
        response = self._make_signed_request('get', '/workspaces/{0}/app-usage/{1}'.format(self.__workspace_id, user_id))
        return response.json()
    ...
    def edit_organization_name(self, organization_name):
        ...
        """
        response = self._make_signed_request('patch', '/workspace/{0}'.format(self.__workspace_id), json={
    ...
    def update_pipeline_inbound_updates(self, view_id, disable=False):
        ...
        """
        response = self._make_signed_request('post', 'dataviews/{0}/inbound-updates'.format(view_id),
    ...
    def add_metric(self, view_id, params, condition=None, limit=None, wait_till_ready=True):
        ...
        """
        res = self._make_signed_request('post', '/dataviews/{0}/derivatives'.format(view_id), json=params)
        response = res.json()
        if wait_till_ready:
    ...
    def delete_metric(self, view_id, metric_id, wait_till_ready=True):
        ...
        """
        res = self._make_signed_request('delete', '/dataviews/{0}/derivatives/{1}'.format(view_id, metric_id))
        response = res.json()
        if wait_till_ready and 'future_id' in response and response["STATUS"] == REQUEST_STATUS.PROCESSING:
            response = self.track_future_id(response['future_id'])
        return response
    ...
    def list_metrics(self, view_id):
        ...
        Args:
            view_id: dataview id
        """
        res = self._make_signed_request('get', '/dataviews/{0}/derivatives'.format(view_id))
    ...
    def wait_till_metric_is_ready(self, view_id, metric_id, condition=None, limit=None):
        ...
        while retry_count < MAX_RETRIES:
            if retry_count != 0:
                time.sleep(RETRY_DELAY_IN_SEC)
            metric_res = self._make_signed_request('post', '/dataviews/{0}/derivatives/{1}'.format(view_id, metric_id),
    ...
    def toggle_draft_mode(self, view_id, mode):
        ...
        """
        res = self._make_signed_request('post', '/dataviews/{0}/draft-mode'.format(view_id),
    ...
    def get_workspace_details(self):
        ...
        """
        r = self._make_signed_request('get', '/workspace/{workspace_id}'.format(workspace_id=self.__workspace_id))
        response = r.json()
        return response
    ...
    def download_template(self, view_id):
        ...
        """
        r = self._make_signed_request('get', '/dataviews/{view_id}/exportable-config'.format(view_id=view_id))
        response = r.json()
    ...
    def get_pending_pipeline_changes(self):
        ...
        """
        res = self._make_signed_request('get', '/pipeline_changes')
    ...
    def submit_pending_pipeline_changes(self, view_ids):
        ...
        """
        res = self._make_signed_request('post', '/pipeline_changes',
                                        json={"dataview_ids": view_ids})
        future_id = res.json()['future_id']
        future_tracking_response = self.track_future_id(future_id)
        future_response = future_tracking_response['response']
        return future_response
