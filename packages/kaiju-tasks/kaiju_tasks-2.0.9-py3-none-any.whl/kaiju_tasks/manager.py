...
class TaskManager(ContextableService, AbstractRPCCompatible):
    ...
    async def suspend_executor(self, executor_id: uuid.UUID) -> None:
        ...
        self.logger.info('Suspending executor "%s".', executor_id)
        sql = (
            self.table.update()
    ...
    async def execute_stage(self, id: str, executor_id: uuid.UUID, stage: int):  # noqa id
        ...
        self.logger.info('Stage "%s" of task "%s": being executed by "%s".', stage, id, executor_id)
        sql = (
            self.table.update()
    ...
    async def write_stage(
        ...
        if error:
            self.logger.info(
                'Stage "%s" of task "%s" from "%s": finished with an error: %s', stage, id, executor_id, result
            )
            sql = sql.values(
                {
            ...
            self.logger.info('Stage "%s" of task "%s" from "%s": task finished.', stage, id, executor_id)
            sql = sql.values(
                {
            ...
            self.logger.info('Stage "%s" of task "%s" from "%s".', stage, id, executor_id)
            sql = sql.values(
