...
def _validate_keys(
    ...
    return_annotation = get_type_hints(stream_fn).get("return")
...
def parse_and_register_stream_resolver(
    *,
    ...
    elif keys and mode != "continuous":
        raise ValueError(
            f"Stream resolver '{fqn}' takes in a 'keys' argument in the decorator "
            f"but is not in continuous mode. Only continuous streaming resolvers take in 'keys'."
        )
