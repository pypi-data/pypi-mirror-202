...
def func_ambank(content: Union[io.BytesIO, str]) -> tuple:
    ...
    # add close file before running next unit test
    pdf.close()
...
def func_cimb(content: Union[io.BytesIO, str]) -> tuple:
    headers = flatten_list([bank['headers'] for bank in SCHEMA
                            if bank['name'] == 'CIMB'])
    ...
    if pd.to_numeric(transformed_df['debit'].iloc[0]) > 0:
        opening_balance = pd.to_numeric(transformed_df['balance'].iloc[0]) + pd.to_numeric(transformed_df['debit'].iloc[0])
    elif pd.to_numeric(transformed_df['credit'].iloc[0]) > 0:
        opening_balance = pd.to_numeric(transformed_df['balance'].iloc[0]) - pd.to_numeric(transformed_df['credit'].iloc[0])
    else:
