...
class ListenerApiModelV1(object):
    ...
    def __get_state(self, data: dict = {}) -> dict:
        ...
        if arikuri_from_db in (None, {}):
            ...
            logging.info(f"arikuri_from_db is None or empty")
            return response
        # check if the topic_id is in the arikuri
        if topic_id not in arikuri_from_db:
            # if it does not exist, return response
            logging.info(f"topic_id {topic_id} not in arikuri_from_db")
            return response
        ...
        if filename == None:
            # if it does not exist, return response
            logging.info(f"filename is None")
            return response
        # check if the filename is not an empty string
        if filename == "":
            # if it does not exist, return response
            logging.info(f"filename is an empty string")
            return response
        # check if the filename exists as a file
        file_path = os.path.join(
            self.__data_files_path, data["chain_spec_hash"], topic_id, filename)
        if not os.path.isfile(file_path):
            # if it does not exist, return response
            logging.info(f"file_path {file_path} does not exist")
            return response
        # update the response
        response["found"] = True
