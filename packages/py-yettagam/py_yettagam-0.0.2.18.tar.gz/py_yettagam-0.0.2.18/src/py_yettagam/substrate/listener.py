...
class SubstrateListener(object):
    ...
    def __subscription_handler(self, obj, update_nr, subscription_id):
        ...
        if update_nr > math.inf:
            return {'message': 'Listener has been stopped', 'updates_processed': update_nr}
    ...
    def __validate_state_payload(self, payload: any = None) -> dict:
        ...
        if data["topic_id"] not in self.__topics_from_chain():
            error_message = f"topic_id {data['topic_id']} is not in the chain!"
            logging.error(error_message)
            result["errors"].append(error_message)
        # check if the kuri is in the chain
        if data["kuri"] not in self.__writers_from_db():
            error_message = f"kuri {data['kuri']} is not in the chain!"
            logging.error(error_message)
            result["errors"].append(error_message)
        # check if the chain_spec_hash is in the chain
        if data["chain_spec_hash"] not in self.__chain_spec_hashes_from_db():
            error_message = f"chain_spec_hash {data['chain_spec_hash']} is not in the chain!"
            logging.error(error_message)
            result["errors"].append(error_message)
        # return the result
        return result
    ...
    def state(self, payload: any = None):
        ...
        for kuri in payload["kuris"]:
            ...
            response["data"][kuri] = True if state["found"] else False
    ...
    def _copy_files(self, source_folder: str = None, destination_folder: str = None, filenames: list = None) -> None:
