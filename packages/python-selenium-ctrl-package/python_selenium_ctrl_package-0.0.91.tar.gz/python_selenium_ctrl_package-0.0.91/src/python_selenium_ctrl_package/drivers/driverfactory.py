...
from ..utils import remote_credentials, screenshot_conf
from ..drivers.browsers.remote_options import RemoteOptions
from ..drivers.browsers.local_browsers import LocalBrowsers

from ..conf.app import ports_conf     # use for local mobile tests
...
class DriverFactory(RemoteOptions, LocalBrowsers):
    ...
    def __init__(self, browser="chrome", browser_version=None, os_name=None):               # browser should be already default set in browser fixture
    ...
    def get_browser(self, browser, browser_version):
        ...
        # TODO: add other browsers, such as edge
    ...
    def run_browserstack(self, os_name, os_version, browser, browser_version,
        ...
        web_driver = webdriver.Remote(RemoteConnection("http://%s:%s@hub-cloud.browserstack.com/wd/hub"
                                                       % (username, password), resolve_ip=False),
    ...
    def run_mobile(self, mobile_os_name, mobile_os_version, device_name, app_package,
        ...
        if app_package == 'com.android.chrome':
            desired_capabilities = self.set_mobile_chrome(desired_capabilities)

        # Check whether the OS is android or iOS and get the mobile driver
        mobile_driver = None
        if mobile_os_name in 'Android':
            mobile_driver = self.android(remote_flag, desired_capabilities, app_path, app_name,
                                         ...
                                         password, device_flag, no_reset_flag)
    ...
    def android(self, remote_flag, desired_capabilities, app_path, app_name, appium_version,
                app_package, app_activity, username, password, device_flag, no_reset_flag):
        ...
        # Get the driver when test is run on local setup
            ...
            try:
                ...
                desired_capabilities['noReset'] = True if no_reset_flag.lower() == 'true' else False
    ...
    def saucelab_mobile(self, app_path, app_name, desired_capabilities, username, password):
        ...
        self.sauce_upload(app_path,
                          app_name)  # Saucelabs expects the app to be uploaded to Sauce storage everytime the test is run

        # Checking if the app_name is having spaces and replacing it with blank
        if ' ' in app_name:
            app_name = app_name.replace(' ', '')
            print(
                "The app file name is having spaces, hence replaced the white spaces with blank in the file name:%s" % app_name)
    ...
    def browserstack_mobile(self, app_path, app_name, desired_capabilities, username, password,
        ...
        desired_capabilities['app'] = self.browser_stack_upload(app_name,
                                                                app_path)  # upload the application to the Browserstack Storage
