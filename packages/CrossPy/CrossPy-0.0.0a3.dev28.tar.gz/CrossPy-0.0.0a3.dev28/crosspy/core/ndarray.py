...
def _cross_apply(func, arg0, *args, **kwargs):
    if hasattr(arg0, 'device'):
        with arg0.device:
            return func(arg0, *(cupy.asarray(x) for x in args), **kwargs)
...
class CrossPyArray(numpy.lib.mixins.NDArrayOperatorsMixin):
    ...
    def __init__(self, obj, axis: Optional[int] = None, **kwargs) -> None:
        ...
        if not isinstance(obj, (list, tuple)):
            ...
            if len(list_obj) == 0:
                ...
                self._concat_axis = (axis is not None) and _check_concat_axis(shapes, axis)
        ...
        if self._concat_axis == 0 and len(index) == 1:
            if isinstance(index[0], slice):
                ...
                if len(relevant_blocks) == 1:
                    if hasattr(relevant_blocks[0], '__verify__'):
                        relevant_blocks[0].__verify__()
            ...
            if isinstance(index[0], list) and all(isinstance(i, int) for i in index[0]) or is_array(index[0]):  # and index[0].dtype != bool:
                ints = index[0]
                block_ids = self._global_index_to_block_id(ints)
                assert len(block_ids) == len(ints)
                ...
                for right in bounds:
                    ...
                    with getattr(block, 'device', nullcontext()):
                        obj.append(block[ints[left:right]])
                    left = right
                if len(obj) == 1:
    ...
    def block_view(self, apply=None):
        ...
        assert self._concat_axis is not None, "unknown axis to divide blocks" # TODO one block singleton
