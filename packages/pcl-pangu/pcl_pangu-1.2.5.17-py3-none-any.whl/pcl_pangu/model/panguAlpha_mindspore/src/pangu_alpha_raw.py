...
class EmbeddingLookup(nn.Cell):
    ...
    def __init__(self, config):
        ...
        try:
            e_table = np.load('{}/word_embedding.npy'.format(config.load_ckpt_local_path))
...
class PanguAlpha_Model(nn.Cell):
    ...
    def __init__(self, config):
        ...
        try:
            p_table = np.load('{}/position_embedding.npy'.format(config.load_ckpt_local_path))
        ...
        if config.use_top_query_attention:
            ...
            try:
                query_table = np.load('{}/top_query_embedding.npy'.format(config.load_ckpt_local_path))
