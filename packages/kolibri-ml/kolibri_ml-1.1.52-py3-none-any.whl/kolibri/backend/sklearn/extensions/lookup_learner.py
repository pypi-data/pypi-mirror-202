...
class LazyLookupRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, lookup_keys=[], n_neighbors:int=2, algorithm='auto',  distance='heom', leaf_size:int=10,
    ...
    def fit(self, X, y):
        ...
        * `n_nodes_`: number of nodes
        ...
        X_groups=self.X[self.lookup_keys].drop_duplicates().reset_index(drop=True)
    ...
    def _predict_one(self, X):
        ...
        logger.debug("model_key: "+str(to_pred))
        if model_key not in self.models:
            logger.debug("model_key not in models: " + str(to_pred))
    ...
    def _fit_linear_model(self, X_train, y_train, estimator):
        ...
        regressor = Pipeline(
            steps=[("infer_type", infer_type), ("dummy",DummyConverter(config={})), ("zero_var", zero),("colinear", colinear), ("regressor", estimator)]
        )

        regressor.fit(X_train, y_train)
        logger.debug("estimator: "+ str(regressor['regressor'].estimator_))
        return regressor
