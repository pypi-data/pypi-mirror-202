...
class GroupAPIView(APIView):
    ...
    def in_group_users(self, *args, request_params=None, **kwargs) -> JsonResponse:
        ...
        if request_params is None:
            request_params = {}
        need_child = request_params.get("need_child") == 1 or request_params.get("need_child", "0") == True
        request_params = format_params_handle(
            param_dict=request_params,
            filter_filed_list=["phone", "user_name", "full_name", "email", "group_id", "user_id", "user_group_id"],
    ...
    def user_group_list(self, *args, request_params=None, **kwargs):
        # 用户组 列表接口
        data, err = UserGroupService.group_list(request_params)
        if err:
            return util_response(err=1000, msg=err)
        return util_response(data=data)
