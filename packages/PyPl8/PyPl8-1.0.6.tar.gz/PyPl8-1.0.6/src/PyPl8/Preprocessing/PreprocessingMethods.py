...
def AutoCrop(file, sourcefolderpath, rotate = False, 
    ...
    thresholdedx = [c[0] for c in np.argwhere(x > s*1700/200)] #rough estimate of agar height
    ...
    bufferr = s*250/200 # buffer away from plate edges
    bufferc = s*175/200
...
def ClickCrop(file, sourcefolder, s=200, array_dimensions = (8,12), rotate = False, adjust = True):
    ...
    sf = height/800
    image = np.array(im.resize((int(height/sf),int(width/sf))))
    
    #Define click and crop event on image
    def click_and_crop(event, x, y, flags, s):
        ...
        # check to see if the left mouse button was released
            ...
            for r in rps:
                for c in cps:
                    corners[j] = (int((r-n)*sf),int((c-n)*sf))
    ...
    cv2.setMouseCallback("image", click_and_crop,s)
...
def AutoCropTest(image, s = 200, array_dimensions = (8,12),adjust=True, display = False):
    ...
    # -- Identify top and bottom
    bw = img>.5
    bw = remove_small_objects(bw, min_size = 0.025*1000/s**2)
    mask = convex_hull_image(bw)
