...
from clean_graph.clean_graph       import GeneralGraph 
from clean_graph.general_functions import plot_line_accross_axes, plot_line_within_ax, prepare_title, formatstring, optimize_data
from clean_graph.multiplier        import Multiplier


class ColumnWithWaterfall(GeneralGraph):
    """
    Produces a column graph with a waterfall delta graph based on data of one year divided in 12 months
    
    Minimal requirements to make the graph
    --------------------------------------
    import clean_graph as cg
    dataset = {'PY': [14, 16, 14, 17, 19, 17, 19, 22, 16, 17, 16, 22],      # 12 months of Previous Year data (optional)
               ...
               'Year': 2022}                                                # the year to be mentioned in the graph
    cg.ColumnWithWaterfall(data=dataset)
    ...
    def __init__(self, data=None, positive_is_good=True, preferred_base_scenario='PL', title=None, measure=True, multiplier='1', filename=None, 
        ...
        # We are exporting the graph if the filename has a value
    ...
    def _check_data(self, data=None):
        ...
        Self variables
        --------------
        self.barshift           : The barshift for the main part of the graph. This value will be overruled with barshift_value if there are other relevant scenario's available
        
        self.barshift_leftside  : The barshift for the left side part of the graph. This value will be overruled with barshift_value if there are relevant scenario's available
        ...
        # Determine barshift for leftside graph. self.barshift_leftside is already initialized in the superclass
       ...
       """
        ...
        for element_list in data_list:
            ...
            if headers is None:
                ...
                # The element_list usable as header must contain Year and Month information or else we can not prepare the data for the graphs later
    ...
    def _fill_main_ax_bar(self, scenario):
        ...
        self.barshift      : the barshift for the main part of the graph.

        self.barwidth      : a float with the width of the bars for measure of ratio

        self.colors        : a dictionary of colors needed for the consistent look of the graphs
    ...
    def _fill_main_ax_text(self):
        ...
        self.font        : All text in a graph has the same font
        
        self.fontsize    : All text in a graph has the same height
    ...
    def _fill_main_ax_label(self): 
        ...
        self.barshift       : Information about how much we need to shift the bar to fit right on top of the other graphs
        """
        ax = self.ax["main"]
        ax.tick_params(top=False, bottom=False, left=False, right=False, labelleft=False, labelbottom=True)
        
        # Make a list of coordinates to shift the names of the months a bit to the right (if there is a comparison in the graph)
    ...
    def _fill_leftside_ax(self): 
        ...
        self.delta_base_value : Base_values for the delta graphs. Delta_name is the key for the value
    ...
    def _show_delta(self, delta_name='main', last_line=True):
        """
        Plots a delta bar on the main ax subplot of the graph
    ...
    def _fill_delta_ax_text(self, delta_name='main'):
        """
        Puts the text on the delta graphs
    ...
    def _title(self, title=None):
        """
        Puts a title in the upper left corner of the graph
