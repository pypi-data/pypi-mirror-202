...
from flux.enum_dimensoes_agrupamento import DimensoesAgrupamento
...
class BodyFluxPersonalizado:
    ...
    def set_option_cenarios_vpercentil(self, lista_cenarios: list):
        ...
        for p in lista_cenarios:
            if type(p) != int or p < 0 or p > 100:
                raise Exception("[EE BodyFluxPersonalizado] - O argumento da função {} deve ser "
                                "uma lista de números inteiros válida.".format(lista_cenarios))
        self.__option_cenarios_vpercentil = lista_cenarios
    ...
    def set_option_dimensoes_para_agrupamento(self, dim: list):
        check1 = dim == DimensoesAgrupamento.SE_NE_vs_S_N
        check2 = dim == DimensoesAgrupamento.SE_S_vs_NE_N
        check3 = dim == DimensoesAgrupamento.SE_N_vs_S_NE
        check4 = dim == DimensoesAgrupamento.SE_vs_S_NE_N
        check5 = dim == DimensoesAgrupamento.SE_vs_S_vs_NE_vs_N
