...
class And(tc.Formula):
    ...
    def _to_sql(self) -> str:
        ...
        if isinstance(self.children[0], And):
            left = f"{self.children[0].children[0]._to_sql()} AND {self.children[0].children[1]._to_sql()}"
        elif not isinstance(self.children[0], tc.Not | tc.ATOM_TYPES):
            left = f"({left})"

        if isinstance(self.children[1], And):
            right = f"{self.children[1].children[0]._to_sql()} AND {self.children[1].children[1]._to_sql()}"
        elif not isinstance(self.children[1], tc.Not | tc.ATOM_TYPES):
            right = f"({right})"
