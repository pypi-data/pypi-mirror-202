...
def get_prediction_header(
    ...
    return (
        ...
        + time_groups
        + [PREDICTION_COLNAME]
        + ([target_name] if has_target else [])
        + keep_columns
    )
...
class ModelStore(BaseModel):
    ...
    def load_and_unpickle(
        ...
        meta, model_bytes, stuff_dic = self.load(model_id)
        # Some algorithm didn't had model object
        model = None
        if model_bytes:
            model = pickle.loads(model_bytes)
        stuff = {key: pickle.loads(stuff_dic[key]) for key in stuff_dic}
        return meta, model, stuff

    def load(
        self, model_id: str, model_not_found_ignored: bool=False
        ...
        try:
            ...
            if not model_not_found_ignored:
                raise e
