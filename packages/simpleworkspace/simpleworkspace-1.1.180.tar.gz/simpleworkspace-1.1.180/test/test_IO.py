...
class IO_FileTests(BaseTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.testPath_samples = cls.testPath + "/tmp"
    ...
    def test_File_Reading_ReadsCorrect(self):
        ...
        f1 = self.tmpFile("file1.txt", fileContent)
        data = sw.io.file.Read(f1, readLimit=len(fileContent), getBytes=False)
        self.assertEqual(data,  fileContent)

        #
        tmpList = []
        sw.io.file.Read(f1, callback=tmpList.append, readSize=2, readLimit=6, getBytes=True)
        self.assertEqual(len(tmpList), 3)
        self.assertEqual(len(tmpList[0]), 2)

        #
        tmpList = []
        sw.io.file.Read(f1, callback=lambda x: tmpList.append(x), readSize=2, getBytes=True)
        self.assertEqual(len(tmpList[0]),  2)

        #
        tmpList = []
        sw.io.file.Read(f1, callback=tmpList.append, readSize=5, readLimit=2, getBytes=True)
        self.assertEqual(len(tmpList), 1)
        self.assertEqual(len(tmpList[0]), 2)

        #
        tmpList = []
        sw.io.file.Read(f1, callback=tmpList.append, readSize=-1, readLimit=4, getBytes=True)
