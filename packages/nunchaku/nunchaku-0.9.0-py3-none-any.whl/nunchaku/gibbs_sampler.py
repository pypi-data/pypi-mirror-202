...
class GibbsSampler:
    ...
    def sample(
        ...
        against that of the case where one region is removed or two regions
        are merged.
        If the Bayes factor (more regions over fewer) is smaller than
        ...
        check : float or None
            threshold of Bayes factor to check if smaller number of regions
        ...
        if initial_points is None:
            ...
            raise ValueError("len(initial_points) must equal 2 * num_regions.")
        if initial_points is None:
            evi_max = -np.inf
            evi_for_plot = []
        else:
        ...
        for j in range(random_attempts):
            init = self._gibbs_generate_initial_guess(num_regions)
    ...
    def _gibbs_checker(self, boundaries, check):
        """check whether a smaller number of regions has a higher evidence."""
        evi0 = self._gibbs_get_evidence(boundaries)
        evi_max = -np.inf
        boundaries_max = []
        num_regions = len(boundaries) // 2
