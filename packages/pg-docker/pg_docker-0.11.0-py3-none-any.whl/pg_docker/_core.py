...
class DatabasePool:
    ...
    def stop(self):
        for cleaner in self._cleaners:
            cleaner.stop()
        for process in self._cleanup_processes:
            process.join(timeout=_LONG_TIMEOUT)
...
def postgres_docker_running(
    ...
    container_id = docker_process.stdout.splitlines()[-1]
    try:
        yield
    finally:
        subprocess.run(
...
def database_pool(
    *,
    ...
    """
    port = get_free_port()
    with postgres_docker_running(port=port, postgres_image_tag=postgres_image_tag, docker_command=docker_command):
        pool = DatabasePool(
            DatabaseParams(
                host="0.0.0.0",
                port=port,
