...
class EsOp():
    ...
    def index_search_absolute_match(self,index_name,query_dict,pn=1,size=10,doc_type='',request_timeout=10):
        ...
        iterator = Search(using=self.client, doc_type=doc_type, index=index_name,request_timeout=request_timeout).query("term", **new_dict)[(pn - 1) * size:pn*size]
    ...
    def index_search_fuzzy_match(self,index_name,query_dict,pn=1,size=10,doc_type='',request_timeout=10):
        iterator = Search(using=self.client, doc_type=doc_type,index=index_name,request_timeout=request_timeout).query("match", **query_dict)[(pn - 1) * size:pn*size]
        iterator = iterator.execute().to_dict()
        return self.index_result_dict(iterator)
    ...
    def index_MultiMatch(self,index_name,query_text,query_field_list,doc_type='',request_timeout=10):
        ...
        iterator= Search(using=self.client,doc_type=doc_type,index=index_name,request_timeout=request_timeout).query(multi_match)
        result = []
        for i in iterator:
