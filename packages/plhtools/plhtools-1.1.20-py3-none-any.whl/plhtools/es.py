...
def get_es_client(label='', host='', user='', password=''):
    if label:
        ...
        label = 'default'
    if user and password:
        return Elasticsearch([host],http_auth=(user, password), timeout=50000)
...
class EsOp():
    def __init__(self, label='', host='', user='', password='', size=2000):
        ...
        self.client =get_es_client(label=label, host=host, user=user, password=password)
        self.size = size
    ...
    def index_search(self,index_name,body,size=1000,timeout=10):
        result =  self.conn.search(index=index_name,size=size,body=body,timeout=timeout)
        return self.index_result_dict(result)
    def index_search_all(self,index_name,pn=1,size=10000,doc_type='',timeout=10):
        body = {
            "query": {
                "match_all": {}
            },
        ...
        result = self.conn.search(index=index_name,doc_type=doc_type, body=body,timeout=timeout)
        return self.index_result_dict(result)
    def index_search_absolute_match(self,index_name,query_dict,pn=1,size=10,doc_type='',timeout=10):
        new_dict= {}
        ...
        iterator = Search(using=self.client, doc_type=doc_type, index=index_name,timeout=timeout).query("term", **new_dict)[(pn - 1) * size:pn*size]
        iterator = iterator.execute().to_dict()
        return self.index_result_dict(iterator)

    def index_search_fuzzy_match(self,index_name,query_dict,pn=1,size=10,doc_type='',timeout=10):
        iterator = Search(using=self.client, doc_type=doc_type,index=index_name,timeout=timeout).query("match", **query_dict)[(pn - 1) * size:pn*size]
        iterator = iterator.execute().to_dict()
        return self.index_result_dict(iterator)

    def index_MultiMatch(self,index_name,query_text,query_field_list,doc_type='',timeout=10):
        multi_match = MultiMatch(query=query_text, fields=query_field_list)
        iterator= Search(using=self.client,doc_type=doc_type,index=index_name,timeout=timeout).query(multi_match)
        result = []
        for i in iterator:
...
if __name__ == '__main__':
    ...
    # print(s)
    s= es.index_search_absolute_match('standard_search',{'CnName': '氟硅酸钠的化学分析方法'})
    print(s)
    s = es.index_search_fuzzy_match('title_index_search',{'word': '成品'},pn=1,size=10)
    print(s)
