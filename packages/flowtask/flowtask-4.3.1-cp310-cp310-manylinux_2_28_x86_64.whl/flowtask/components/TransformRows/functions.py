...
def sum(df=None, field: str = '', columns: list = []):
    try:
...
def drop_column(df=None, field=''):
    return df.drop([field], axis=1)


def add_days(df=None, field='', column='', days=1):
    df[field] = df[column] + pandas.DateOffset(days=days)
    return df


def substract_days(df=None, field='', column='', days=1):
    df[field] = df[column] - pandas.DateOffset(days=days)
    return df


def rename_column(df=None, field='', rename=''):
    return df.rename(columns={field: rename})


def regex_match(df=None, field='', column='', regex=''):
    df[field] = df[column].str.extract(regex)
    return df


def zerofill(df=None, field='', num_zeros=1):
    """ Fill with zeroes an string until the length """
    df[field] = df[field].astype(str).str.zfill(num_zeros)
    return df


def pad(df=None, field='', num_chars=4, side='left', char='0'):
...
def concat(df=None, field='', columns=[], separator=' '):
    try:
        if columns and isinstance(columns, list):
...
def split_array(df=None, field='', column='', separator=' ', trim=False):
    try:
        col = column if column else field
...
def split(df=None, field='', column='', separator=' ', index=0, is_integer=False):
    try:
...
def slice(df=None, field='', column='', start=0, end=1, is_integer: bool = False):
...
def case(df=None, field='', column='', condition: Any = None, match=None, notmatch=None):
    """Case.
...
def divide(df=None, field='', divisor=100):
...
def nullif(df=None, field='', chars=[]):
...
def uppercase(df=None, field='', from_column: str = None):
...
def lowercase(df=None, field='', from_column: str = None):
    if from_column is not None:
        column = from_column
    else:
        column = field
...
def both_strip(df=None, field='', character=' '):
    df[field] = df[field].str.strip(character)
    return df


def trim(df=None, field='', characters=' ', remove_empty: bool = False):
...
def right_strip(df=None, field='', character=' '):
    df[field] = df[field].str.rstrip(character)
    return df


def replace_regex(df=None, field='', to_replace='', value=''):
    try:
...
def ereplace(df=None, field='', columns=[], newvalue=''):
    col1 = columns[0]
    col2 = columns[1]
...
def convert_to_object(df=None, field: str = '', remove_nan: bool = False):
...
def convert_to_string(df=None, field: str = '', remove_nan: bool = False, avoid_empty: bool = True):
    try:
...
def to_string(df, field: str, remove_nan: bool = False):
...
def convert_to_array(df=None, field='', separator=',', remove_empty: bool = False):
    df[field] = df[field].astype(str)
...
def convert_to_datetime(df=None, field='', remove_nat=False):
    try:
...
def convert_to_time(df=None, field='', format="%H:%M:%S", not_null=False):
    df[field] = df[field].where(df[field].notnull(), None)
    df[field] = pandas.to_datetime(
...
def column_to_date(df=None, field='', column='', format="%Y-%m-%d"):
    if not column:
        column = field
...
def datetime_format(df=None, field='', column='', format="%Y-%m-%d"):
    try:
        if not column:
            column = field
...
def column_to_integer(df=None, field='', column=''):
    df[field] = pandas.to_numeric(df[column], errors='coerce')
    df[field] = df[field].astype('Int64')
    return df


def convert_to_date(df=None, field='', format="%Y-%m-%d", remove_nat=False):
...
def string_to_date(df=None, field='', column='', format="%Y-%m-%d"):
    df[field] = pandas.to_datetime(df[column], format=format, errors='coerce')
...
def convert_to_integer(df=None, field='', not_null=False, fix_negatives: bool = False):
    try:
        if fix_negatives is True:
            # mask = df[field].str.endswith('-')
            # df.loc[mask, field] = '-' + df.loc[mask, field].str[:-1]
...
def convert_to_boolean(df=None, field='', boolDict={'True': True, 'False': False}, nan: bool = False):
