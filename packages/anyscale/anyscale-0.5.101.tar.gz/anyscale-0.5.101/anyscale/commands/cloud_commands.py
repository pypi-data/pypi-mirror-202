...
@click.option(
    "--provider-id",
    help="Provider ID in Workload Identity Pools for GCP clouds.",
    required=False,
    type=str,
)
@click.option(
...
def register_cloud(  # noqa: PLR0913
    ...
    if provider == "aws":
        ...
        if (
            ...
            raise click.ClickException(
                "Please provide a valid, fully qualified provider ID. Example: projects/<project number>/locations/global/workloadIdentityPools/<pool name>/providers/<provider id>"
            )
