...
def base64decode(value: str) -> str:
    ...
    'foobar'
    """
    return str(base64.b64decode(value), encoding='utf-8')
...
def read_bytes(filepath, encoding='utf-8', not_exists_ok: bool = False):
    if not_exists_ok and not Path(filepath).exists():
        return b''
    with open(filepath, 'rb', encoding=encoding, newline='') as f:
        content = f.read()
    return content


def read_text(filepath, encoding='utf-8', not_exists_ok: bool = False):
    if not_exists_ok and not Path(filepath).exists():
        return ''
...
def read_lines(filepath, encoding='utf-8', not_exists_ok: bool = False):
    lines = []
    if not_exists_ok and not Path(filepath).exists():
        return lines
    with open(filepath, 'r', encoding=encoding, newline='') as f:
...
def write_lines(filepath, lines, mode='w', encoding='utf-8'):
    with open(filepath, mode, encoding=encoding) as f:
        for l in lines:
            f.write(l + '\n')
