"""
websocket support - PyDEEN as Websocket client and listener
"""
...
class WebSocketService(Service):
    ...
    async def ws_listen(self):   
        ...
        url = self.backend.get_url() #get_property(Backend.BACKEND_PROP_URL)
                    ...
                    if action.parse(response) == True:
                        ...
                        if action.handle(response, self) == False:
                            ...
                            if self.answer != None and self.answer != "":
                                self.trace(f"> send answer for action handler {type(action)}") 
