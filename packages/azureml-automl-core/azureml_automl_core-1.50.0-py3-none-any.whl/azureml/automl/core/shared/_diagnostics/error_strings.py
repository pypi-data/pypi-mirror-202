...
class AutoMLErrorStrings:
    ...
    NLP_MALFORMED_NER_LINE = "Each line in {split_type:log_safe} set should either contain a single token and its " \
                             "label separated by whitespace, or it should be an empty line. Each label should " \
                             "either start with 'I-' or 'B-' or be exactly 'O'. Instead, we got \"{malformed_line}\"" \
                             " on line {line_no:log_safe}. More information about NER input formats can be found" \
                             " at {info_link:log_safe}."
