...
Many docstrings in this file are based on PEP-249,
...
class DBAPICursor(with_metaclass(abc.ABCMeta, object)):
    ...
    def _fetch_more(self):
        """Get more results, append it to ``self._data``,
        and update ``self._state``."""
        raise NotImplementedError  # pragma: no cover
    ...
    def executemany(self, operation, seq_of_parameters):
        """Prepare a database operation (query or command)
         and then execute it against all parameter
    ...
    def iter_result(self):
        ...
        An :py:class:`~pyhive.exc.Error` (or subclass)
        exception is raised if the previous call to
        :py:meth:`execute` did not produce any result
    ...
    def fetchone(self):
        ...
        returning a single sequence, or ``None``
        when no more data is available.
        An :py:class:`~pyhive.exc.Error` (or subclass)
        exception is raised if the previous call to
        :py:meth:`execute` did not produce any
    ...
    def fetchmany(self, size=None):
        ...
        If it is not given, the cursor's arraysize determines the
        number of rows to be fetched. The method should try to
        fetch as many rows as indicated by the size parameter.
        If this is not possible due to the specified number of rows
    ...
    def fetchall(self):
        ...
        returning them as a sequence of sequences
    ...
    @property
    def rownumber(self):
        """This read-only attribute should provide the
        current 0-based index of the cursor in the
        result set.
        The index can be seen as index of the cursor
        in a sequence (the result set). The next fetch
        operation will fetch the row indexed by
        ``rownumber`` in that sequence.
        """
        return self._rownumber

    def __next__(self):
        """Return the next row from the currently
        executing SQL statement using the same semantics
        as :py:meth:`fetchone`. A ``StopIteration``
        exception is raised when the result set is
        exhausted.
        """
        one = self.fetchone()
        if one is None:
    ...
    def __iter__(self):
        """Return self to make cursors compatible
...
class ParamEscaper(object):
    ...
    def escape_string(self, item):
        ...
        # Newer SQLAlchemy checks dialect.supports_unicode_binds
        # before encoding Unicode strings
        # as byte strings. The old version always encodes
        # Unicode as byte strings, which breaks
        # string formatting here.
        if isinstance(item, bytes):
            item = item.decode("utf-8")
        # This is good enough when backslashes are literal,
        # newlines are just followed, and the way
    ...
    def escape_sequence(self, item):
        map_escape_items = map(str, map(self.escape_item, item))
        return "(" + ",".join(map_escape_items) + ")"
