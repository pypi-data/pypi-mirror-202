...
class FilterBlueScreen:
    ...
    previous = None
    ''' Previous pixel filter. * ~bge.texture.FilterBGR24 * ~bge.texture.FilterBlueScreen * ~bge.texture.FilterColor * ~bge.texture.FilterGray * ~bge.texture.FilterLevel * ~bge.texture.FilterNormal * ~bge.texture.FilterRGB24 * ~bge.texture.FilterRGBA32'''
...
class FilterRGB24:
    ''' Returns a new input filter object to be used with ~bge.texture.ImageBuff object when the image passed to the :meth: ~bge.texture.ImageBuff.load function has the 3-bytes pixel format BGR.
    '''
...
class ImageBuff:
    ...
    filter = None
    ''' Pixel filter. * ~bge.texture.FilterBGR24 * ~bge.texture.FilterBlueScreen * ~bge.texture.FilterColor * ~bge.texture.FilterGray * ~bge.texture.FilterLevel * ~bge.texture.FilterNormal * ~bge.texture.FilterRGB24 * ~bge.texture.FilterRGBA32'''
...
class ImageRender:
    ...
    background: typing.List[float] = None
    ''' Background color. Use :py:attr: bge.texture.ImageRender.horizon or :py:attr: bge.texture.ImageRender.zenith instead.
...
class ImageViewport:
    ''' Image source from viewport rendered by the active camera. To render from a non active camera see ~bge.texture.ImageRender .
    '''
...
class Texture:
    ''' Class that creates the Texture object that loads the dynamic texture on the GPU. :arg gameObj: Game object to be created a video texture on. :type gameObj: ~bge.types.KX_GameObject :arg materialID: Material ID default, 0 is the first material. (optional) :type materialID: int :arg textureID: Texture index in case of multi-texture channel, 0 = first channel by default. In case of UV texture, this parameter should always be 0. (optional) :type textureID: int :arg textureObj: Reference to another Texture object with shared bindId which he user might want to reuse the texture. If this argument is used, you should not create any source on this texture and there is no need to refresh it either: the other Texture object will provide the texture for both materials/textures.(optional) :type textureObj: ~bge.texture.Texture
    '''
    ...
    source = None
    ''' Source of texture. * ~bge.texture.VideoFFmpeg * ~bge.texture.VideoDeckLink * ~bge.texture.ImageFFmpeg * ~bge.texture.ImageBuff * ~bge.texture.ImageMirror * ~bge.texture.ImageMix * ~bge.texture.ImageRender * ~bge.texture.ImageViewport'''
...
class VideoFFmpeg:
    ...
    filter = None
    ''' An additional filter that is applied on the video before sending it to the GPU. * ~bge.texture.FilterBGR24 * ~bge.texture.FilterBlueScreen * ~bge.texture.FilterColor * ~bge.texture.FilterGray * ~bge.texture.FilterLevel * ~bge.texture.FilterNormal * ~bge.texture.FilterRGB24 * ~bge.texture.FilterRGBA32'''
