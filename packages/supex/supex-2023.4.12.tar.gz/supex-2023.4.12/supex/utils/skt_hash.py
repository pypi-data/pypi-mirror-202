def skt_hash(ss, df, before, after, key_column=None):
    ...
    def mapping_table_load(ss):
        mapping_df = spark.sql(""" select raw, ye_hashed, lake_hashed from aidp.svc_mgmt_num_mapping where dt = (SELECT Max(dt) FROM aidp.svc_mgmt_num_mapping) """)
        return mapping_df
    ...
    def fill_for_mapping_table(ss, df):
        spark = ss
        mapping_path = "/data/aidp/svc_mgmt_num_mapping/"
        output_path = "/data/aidp/svc_mgmt_num_mapping_fill/"
