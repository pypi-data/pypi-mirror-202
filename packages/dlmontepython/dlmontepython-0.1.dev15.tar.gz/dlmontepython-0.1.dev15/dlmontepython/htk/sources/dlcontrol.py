        ...
        try:
            ...
            for line in lines:
                ...
                if key in UseBlock._use_keys:
                    if key == "repexch":
                        nr = int(float(tokens[2]))
                        dt = float(tokens[3])
                        n = int(float(tokens[4]))
...
class MainBlock(object):
    ...
    def _parse_single_line_statement(dlstr):
        ...
        int_float = lambda x : int(float(x))

        _simple_key_values = {"temperature": float,
                              "equilibration": int_float,
                              "acceptatmmoveupdate": int_float,
                              "acceptmolmoveupdate": int_float,
                              "acceptmolrotupdate": int_float,
                              "acceptvolupdate": int_float,
                              "check": int_float,
                              "maxatmdist": float,
                              "maxmolrot": float,
                              "maxvolchange": float,
                              "maxnonbondnbrs": int_float,
                              "print": int_float,
                              "revconformat": str,
                              "stack": int_float,
                              "steps": int_float,
        ...
        try:
            ...
            if key in _simple_key_values:
                ...
                kmax1 = int(float(tokens[3]))
                kmax2 = int(float(tokens[4]))
                kmax3 = int(float(tokens[5]))
    ...
    def _sample_from_string(dlstr):
        ...
        try:
            ...
            if key.startswith("coord"):
                name = "coords"
                linedict.update({"nfreq": int(float(tokens[2]))})
                ...
                name = "rdfs"
                linedict.update({"ngrid": int(float(tokens[2]))})
                linedict.update({"rcut": float(tokens[3])})
                linedict.update({"nfreq": int(float(tokens[4]))})
            elif key.startswith("zden"):
                name = "zdensity"
                linedict.update({"ngrid": int(float(tokens[2]))})
                linedict.update({"nfreq": int(float(tokens[3]))})
            elif key.startswith("vol"):
                name = "volume"
                linedict.update({"mfreq": int(float(tokens[2]))})
...
class CONTROL(object):
    ...
    def get_ewald_sum(self):
        ...
        alpha = float(words[0])
        kx = int(float(words[1]))
        ky = int(float(words[2]))
        kz = int(float(words[3]))
