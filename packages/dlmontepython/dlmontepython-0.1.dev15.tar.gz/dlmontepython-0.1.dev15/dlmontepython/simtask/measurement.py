...
class Measurement(task.Task):
    ...
    value and standard error one or more specified observables via block averaging, using
    the simulation program linked to the object via the `task_interface` argument to the
    constructor. Here 'observables' represent physical quantities which the simulation
    program can output. The nature of the observables is, in general, dependent on the 
    simulation program used with this class. 
    ...
    * The standard errors for all observables are less than threshold values specified 
      in `precisions`
    ...
    Output via the `logging` module
    ---------------------------

    While performing the simulation and analysis, this class can output information via
    the `logging` module. The `measurement.logger` variable housed in this module can be used to
    determine the level information output. With this in mind, adding the following code to 
    a script using this class will result in output to stdout:
    ...
    For more detailed information use `logging.DEBUG` instead of `logging.INFO`; for less use, e.g.
    `logging.WARNING`; see the `logging` module for other options.

    Examples
    --------

    Initiate a measurement of the energy to a precision `eprec` using DL_MONTE, with
    the DL_MONTE input files in the directory `dir1` containing the input parameters
    for the simulations.
    ...
    >>> m = measurement.Measurement(dlmlinker, ("energy",), { ("energy",) : eprec })
    >>> m.run()

    The same as above, but without the assumption of equilibration from the outset of the
    simulation - an equilibration period is calculated and accounted for in the data
    analysis. Moreover, only perform at most 20 back-to-back simulations.

    >>> import dlmontepython.simtask.dlmonteinterface as dlmonteinterface
    >>> dlmlinker = dlmonteinterface.DLMonteInterface('DLMONTE-SRL.X')
    >>> m = measurement.Measurement(dlmlinker, ("energy",), { ("energy",) : eprec }, 
    >>>                             maxsims=20, learn_equilibration_period=True)
    >>> m.run()

    Similar to above, but performing up to 10 simulations (which is the default limit)
    up to a maximum total time of 5 minutes = 300 seconds.

    >>> import dlmontepython.simtask.dlmonteinterface as dlmonteinterface
    >>> dlmlinker = dlmonteinterface.DLMonteInterface('DLMONTE-SRL.X')
    >>> m = measurement.Measurement(dlmlinker, ("energy",), { ("energy",) : eprec }, 
    >>>                             maxtime=300, learn_equilibration_period=True)
    >>> m.run()

    The same as above, but do not terminate the simulation if the threshold precision
    in the energy is met.

    >>> import dlmontepython.simtask.dlmonteinterface as dlmonteinterface
    >>> dlmlinker = dlmonteinterface.DLMonteInterface('DLMONTE-SRL.X')
    >>> m = measurement.Measurement(dlmlinker, ("energy",), 
    >>>                             maxtime=300, learn_equilibration_period=True)
    >>> m.run()
    ...
    equiltest_confint : float
        (see `analysis.equilibration_test`) (default = 0.95)
    ...
    def __init__(self, task_interface, 
                 ...
                 equiltest_confint=0.95, equiltest_corrtime=6.0
                ):
        ...
        self.equiltest_confint = equiltest_confint
    ...
    def __str__(self):
        ...
        r"""Return a readable string representation of a Measurement object
        ...
        string += "  equiltest_confint : "+str(self.equiltest_confint)+"\n"
    ...
    def run(self):
        ...
        r"""Perform the measurement task
        ...
        for i in range(0,self.maxsims):
            ...
            for obs in self.observables:
                ...
                if self.equilibrated[obs]:
                    ...
                    # The inefficiency could be NaN. This occurs if the data is perfectly 'flat',
                    # i.e. all elements of the data array are identical. However, other spurious 
                    # data arrays could cause this I think.
                    # In this case assume an inefficiency of 1, which corresponds to a correlation 
                    # time of 0 (see below).
                    if np.isnan(inefficiency):
                        logger.warning("WARNING: 'NaN' detected for a statistical inefficiency for time series for "
                                      +"observable '"+str(obs)+"' in sim "+str(i)+".\nProceeding assuming no correlation in "
                                      +"time series: setting inefficiency to 1.0. Recommend checking the data is okay!" )
                        inefficiency = 1.0
                    ...
                    # In this case set the block size to block_factor (rounded to the nearest integer above)
                    if inefficiency < 1.0000000001:
                        tau = 0.0                    
                        self.blocksize[obs] = int(self.block_factor)+1
                    else:
                    ...
                    if len(self.blockavgs[obs]) >= 1:
                        ...
                        if len(self.blockavgs[obs]) >= 2:
                            ...
                            # The inefficiency 's' here could be NaN. This occurs if all block averages take
                            # exactly the same value, i.e. all elements of the blockavgs array are identical. 
                            # However, other spurious blockavgs arrays could cause this I think.
                            # In this case assume an inefficiency of 1.
                            if np.isnan(s):
                                logger.warning("WARNING: 'NaN' detected for a statistical inefficiency for block averages for "
                                              +"observable '"+str(obs)+"' in sim "+str(i)+".\nProceeding assuming no correlation in "
                                              +"block averages: setting inefficiency to 1.0. Recommend checking the data is okay!" )
                                s = 1.0
...
class MeasurementSweep(task.Task):
    ...
    def __str__(self):
        ...
        r"""Return a readable string representation of a MeasurementSweep object
