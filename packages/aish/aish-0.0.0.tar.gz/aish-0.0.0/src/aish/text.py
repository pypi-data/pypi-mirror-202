...
def truncate_context(message_list: List[Dict[str, str]], context_limit: int) -> List[Dict[str, str]]:
    context = []
...
def text_options(model, num, temp):
    def decorator(function):
        @click.option("-m", "--model", type=str, default=model, help=f"Model selection. Default is {model}.")
        @click.option("-n", "--num", type=int, default=num, help=f"Number of completions to generate. Default is {num}")
        @click.option("-t", "--temp", type=float, default=temp, help=f"Sampling temperature selection. Default is {temp}")
...
@click.command(help="Chat with ChatGPT")
@click.argument("prompt", type=utils.Prompt(), default="")
@text_options(DEFAULT_CHAT_MODEL, DEFAULT_CHAT_NUM, DEFAULT_CHAT_TEMPERATURE)
@click.option("-c", "--context", type=int, default=DEFAULT_CHAT_CONTEXT, help=f"Context length, for chat. Maximum is {MAXIMUM_CONTEXT-SYSTEM_MESSAGE_LEN}. Default is {DEFAULT_CHAT_CONTEXT}")
@click.option("-ch", "--clearhistory", is_flag=True, help="Clear entire chat history")
@click.option("-mt", "--maxtokens", type=int, default=DEFAULT_CHAT_MAX_TOKENS, help=f"Maximum number of tokens in response, set to -1 to remove limit. Default is {DEFAULT_CHAT_MAX_TOKENS}")
@click.option("-s", "--stop", type=str, multiple=True, help=f"Sequences where the API will stop generating further tokens. Up to four are allowed. The returned text will not contain the stop sequence.")
    ...
    try:
        ...
        if num > 1:
            if no_stream:
                ...
                for index, content in messages.items():
                    print(f"({index}) {content}")
        else:
            if no_stream:
...
@click.command(help="Use ChatGPT to complete text")
@click.argument("prompt", type=utils.Prompt(), default="")
@text_options(DEFAULT_COMPLETE_MODEL, DEFAULT_COMPLETE_NUM, DEFAULT_COMPLETE_TEMPERATURE)
@click.option("-mt", "--maxtokens", type=int, default=DEFAULT_COMPLETE_MAX_TOKENS, help=f"Maximum number of tokens in response, set to -1 to remove limit. Default is {DEFAULT_COMPLETE_MAX_TOKENS}")
@click.option("-s", "--stop", type=str, multiple=True, help=f"Sequences where the API will stop generating further tokens. Up to four are allowed. The returned text will not contain the stop sequence.")
@click.option("-ns", "--no-stream", is_flag=True, help=f"Disable response streaming.")
@click.option("-sf", "--suffix", type=str, default=DEFAULT_COMPLETE_SUFFIX, help=f"Suffix that comes after a completion of inserted text. Default is '{DEFAULT_COMPLETE_SUFFIX}'.")
@click.option("--echo", is_flag=True, help=f"Echo back the prompt in addition to the completion")
def complete(
    prompt: str,
    model: str,
    suffix: str,
...
@click.command(help="Edit text with ChatGPT")
