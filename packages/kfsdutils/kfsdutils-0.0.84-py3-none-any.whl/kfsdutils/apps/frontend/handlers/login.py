...
class LoginHandler:
    ...
    def getLoggedInHttpHeaders(self):
        return {
            'Content-Type': 'application/json',
            'X-CSRFToken': self.getRequest().COOKIES.get('csrftoken'),
            'Cookie': HTTPUtils.cookieToHeaderStr(self.getRequest(), ["access_token", "refresh_token", "csrftoken"])
        }
    
    def getNonLoggedInHttpHeaders(self):
        return {
            'Content-Type': 'application/json',
            'X-CSRFToken': self.getRequest().COOKIES.get('csrftoken'),
            'Cookie': HTTPUtils.cookieToHeaderStr(self.getRequest(), ["csrftoken"])
        }
    ...
    def getVerifyTempTokens(self, code):
        ...
        return self.httpGet(verifyTokensUrl, HTTPStatus.OK, self.getNonLoggedInHttpHeaders())
    ...
    def sendVerifyEmail(self, email, emailType):
        ...
        return self.httpPost(verifyEmailUrl, [HTTPStatus.CREATED, HTTPStatus.BAD_REQUEST], payload, self.getNonLoggedInHttpHeaders())
    ...
    def genUserTokens(self, username):
        ...
        return self.httpGet(userTokensUrl, status.HTTP_200_OK, self.getNonLoggedInHttpHeaders())
    ...
    def authenticate(self, username, password):
        ...
        resp_status, resp = self.httpPost(loginUrl, HTTPStatus.OK, data, self.getNonLoggedInHttpHeaders())
    ...
    def register(self, **kwargs):
        ...
        resp_status, resp = self.httpPost(registerUrl, HTTPStatus.CREATED, data, self.getNonLoggedInHttpHeaders())
        return resp_status, resp
    ...
    def refreshAccessToken(self, refreshToken):
        ...
        resp_status, resp = self.httpPost(refreshAccessTokenUrl, HTTPStatus.OK, payload, self.getNonLoggedInHttpHeaders())
