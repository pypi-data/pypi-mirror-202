...
def get_ticker_expection(zqdm_list,date_list):
    ...
    sql="select EST_DT,SUBSTR(S_INFO_WINDCODE,1,6) as zqdm ,EST_OPER_REVENUE as EST_OPER_REVENUE_YOY,NET_PROFIT as EST_NET_PROFIT_YOY,EST_ROE AS ROE_FY1 ,EST_PE AS PE_FY1,EST_PEG AS PEG_FY1,ROLLING_TYPE from wind.AShareConsensusRollingData where  SUBSTR(S_INFO_WINDCODE,1,6)  in ({0}) and  EST_DT in ({1}) and ROLLING_TYPE in ('FY1','YOY','FY2','FY3')   "\
        .format(util.list_sql_condition(zqdm_list),util.list_sql_condition(date_list))
...
def get_ticker_financial_info(zqdm_list,start_date,end_date,with_stock_price=False):
    ...
    for i in range(int(np.ceil(len(zqdm_list) / max_length))):
        ...
        if(len(df1)>0):
            ...
            sql = "select InnerCode,DividendRatio,TotalMV,PE,PB,PEG,PCFTTM as PCF,TradingDay from  hsjy_gg.LC_DIndicesForValuation where InnerCode in ({0}) and TradingDay in ({1})  " \
                .format(inner_code_con, trading_date_con)
            df3 = hbdb.db2df(sql, db='readonly')
            if(len(df3)==0):
                df3=pd.DataFrame(columns=['INNERCODE','TOTALMV','DIVIDENDRATIO','TRADINGDAY','PE','PB','PCFTTM','PEG'])
            ...
            style_financial_info = pd.concat([style_financial_info, df[['SECUCODE', 'ROE',
                                                                        'NETPROFITGROWRATE', 'OPERATINGREVENUEYOY',
                                                                         'DIVIDENDRATIO', 'TOTALMV',
                                                                        'PE', 'PB','PCF','PEG',
                                                                        'ENDDATE']]], axis=0)

        ##read financial info for kechuagnban stock
        sql = """select b.CompanyCode,b.InnerCode,a.ROETTM as ROE,b.SecuCode,a.ORComGrowRateThreeY as OperatingRevenueYOY,a.NPPCCGrowRateThreeY as NetProfitYOY,a.EndDate 
        ...
        if(len(df1)>0):
            ...
            sql = """select InnerCode,TotalMV,DividendRatio,TradingDay,PETTM as PE,PB,PCFTTM as PCF,PEG from hsjy_gg.LC_STIBDIndiForValue 
            ...
            if(len(df2)>0):
                ...
                df2=pd.DataFrame(columns=['INNERCODE','TOTALMV','DIVIDENDRATIO','TRADINGDAY','PE','PB','PCF','PEG'])
...
class General_holding:
    ...
    @staticmethod
    def read_hld_fromdb(start_date,end_date,jjdm,keyholdonly=False):
    ...
    def fund_holding_date_manufacture(self,jjdm_list,start_date,end_date,if_zstockbl=True
                                      ,if_hldcom=False,keyholdonly=False):
        ...
        for jjdm in jjdm_list:
            # print(jjdm)
            try:
                temphld=self.read_hld_fromdb(start_date,end_date,jjdm,keyholdonly)
    ...
    def get_holding_trading_analysis(asofdate):
        ...
        #get the hsl data
        sql="select jjdm,hsl,tjqj,jsrq from st_fund.t_st_gm_jjhsl where jjdm in ({0}) and jsrq in ({1}) and tjqj!=2"\
            .format(util.list_sql_condition(jjdm_list),
                    util.list_sql_condition(rawdata['in_date'].unique()[1:]))
        ...
        for col in ['平均持有时间（出重仓前）','平均持有时间（出持仓前）','出重仓前平均收益率','出全仓前平均收益率',
                    ...
                    '新股概率（出重仓前）','新股概率（出持仓前）','次新股概率（出重仓前）','次新股概率（出持仓前）','换手率']:
    ...
    @staticmethod
    def read_hld_ind_fromstock(hld,start_date,end_date,hfbz=38):
    ...
    @staticmethod
    def read_hld_style_fromstock(hld,start_date,end_date,if_prv=False):
        if(not if_prv):
        ...
        style_financial_info=get_ticker_financial_info(zqdm_list,start_date[0:6]+'01',end_date)
        ...
        #                                       ['SECUCODE', 'ENDDATE', 'ROE',  'DIVIDENDRATIO',
        #                                       'PE', 'PB', 'PCF', 'PEG', 'TOTALMV']],
    ...
    def save_style_exp2db(self,jjdm_list,start_date,end_date,if_885001=False):
        ...
        hld=self.read_hld_style_fromstock(hld,start_date,end_date)
    ...
    def adjusted_hldquant(self,jjdm_list,start_date,end_date):
        ...
        style_financial_info = get_ticker_financial_info(list(set(top10_list)), '20170630', with_stock_price=False)
...
if __name__ == '__main__':
    ...
    buydf, selldf, buydfgs, selldfgs, buypicdf, sellpicdf, buypicdfgs, sellpicdfgs =\
        stt.data_factory('300750', threshold=3,time_length=7)
    print('')
    # ba=Barra_analysis()
    ...
    #gh.save_industry_financial_stats2db(start_date='20211231',end_date='20220630')
    jjdm_list=util.get_potient_mutual_stock_funds('20220630')
    # jjdm_list=util.get_885001_funds('20220831')

    trunk_size=1000
    #
    # sql="delete from hbs_hld_sl_history"
    # localdb.execute(sql)

    # for i in range(3):
    #     #check if data already exist
    #     sql="delete from hbs_industry_class{2}_exp where jsrq>='{0}' and jsrq<='{1}'"\
    #         .format('20211231','20220630',i+1)
    #     localdb.execute(sql)
    # #
    for i in range(0,int(np.floor(len(jjdm_list)/trunk_size)+1)):
        ...
        # industry_hld_list=gh.save_industry_exp2db(temp_jjdm_list,start_date='20211231',end_date='20220630')
        #
        # for j in range(len(industry_hld_list)):
        #     industry_hld_list[j].to_sql('hbs_industry_class{0}_exp'.format(j + 1),
        #                                                             index=False, if_exists='append', con=localdb)
        #
        # print('{} done'.format(i))

    # sql = "select jjdm,jjmc,clrq  from st_fund.t_st_gm_jjxx where cpfl='2' and (ejfl='37') and jjzt='0' and clrq<='{0}'  ".format(
    #     '20211231')
    ...
    #
    # jjdm_list = util.get_mutual_stock_funds('20181231')
    # gh.save_style_exp2db(jjdm_list,start_date='20211231'
    #                      ,end_date='20220630',if_885001=False)
    # jjdm_list=['540007','530003','519752']
    # jjdm_list=util.get_mutual_stock_funds('20210630')
    # jjdm_list.sort()

    # gh.save_holding_trading_2db(jjdm_list, '20180630', '20220831')
    # gh.get_holding_trading_analysis('20220630')
    # gh.get_hld_property(jjdm_list,'20190831','20220831')
    #gh.ticker_contribution(jjdm_list, '20111230', '20220831')
