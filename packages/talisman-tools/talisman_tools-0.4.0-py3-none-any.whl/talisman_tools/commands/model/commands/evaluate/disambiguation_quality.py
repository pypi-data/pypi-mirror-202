...
from .metrics import ir_micro_avg_score, ir_recall_score, mean_reciprocal_rank
...
def filter_with_predicate(doc: TalismanDocument, fact_predicate: _Predicate):
    return doc.filter_facts(ConceptFact, fact_predicate)
...
def is_ambiguous(doc: TalismanDocument) -> bool:  # TODO: move it to helpers/tdm
    return any(isinstance(fact.value, tuple) and fact.value for fact in doc.filter_facts(ConceptFact))
...
def evaluate_dmb(
        predicted: Sequence[TalismanDocument], gold: Sequence[TalismanDocument], at_ks: Sequence[int],
