...
class DobissAPI:
    def __init__(self, secret, host, secure: bool):
        ...
        logger.debug(
            f"Init DobissAPI object to {host} using secret starting with {secret[0:6]} (secure = {secure})"
        )
        self._host = host
        self._secure = secure
        self._token = ""
    ...
    async def auth_check(self):
        ...
        auth_ok = False
        logger.debug(
            f"Authenticating DobissAPI {self._host} using secret starting with {self._secret[0:6]} (secure = {self._secure})"
        )
        try:
            self.start_session()
    ...
    async def listen_for_dobiss(self):
        while not self._stop_monitoring:
            ...
            logger.debug("Restarting dobiss websocket connection")
