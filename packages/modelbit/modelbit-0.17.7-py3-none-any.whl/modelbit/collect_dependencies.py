    ...
    if maybeFuncVarName in globalsDict:
      ...
      if "__module__" in dir(maybeFuncVar):
        if _isMainModuleFunction(maybeFuncVar):  # the user's functions
          ...
          _collectClassDefDeps(maybeFuncVar, collection)
          ...
          if collectedSpecialObj(maybeFuncVar, maybeFuncVarName, collection):
            ...
            raise Exception(
                f"Instances of user defined classes like {maybeFuncVar.__class__.__name__}'s '{maybeFuncVarName}' are not yet supported."
            )
            # _collectClassDefDeps(maybeFuncVar.__class__, collection)
...
def _collectClassDefDeps(cls: type, collection: NamespaceCollection):
  firstAncestor = [c for c in cls.__mro__ if not _isMainModule(c)][0]
  for superCls in cls.__mro__:
    if _isMainModule(superCls) and superCls != cls:
      _collectClassDefDeps(superCls, collection)
  collection.userClasses[cls.__name__] = getClassSource(cls)
  if firstAncestor != object:
    collection.froms[firstAncestor.__name__] = firstAncestor.__module__
    collection.allModules.append(firstAncestor.__module__)
