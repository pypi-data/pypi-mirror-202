...
def SampledTestValid(model, test_data_loader, device=torch.device("cuda:0")):
    ...
    with torch.no_grad():
        for (
            ...
            batch_score = model(
                b_ori_rev_tensor, b_feature_3Mer, b_feature_3Mer_rev_com, b_feature_4Mer, b_feature_4Mer_rev_com, batch_texts.to(device)
            )
...
def train(
    ...
    trainLoader = DataLoader(trainDataSet, batch_size=batchSize, shuffle=True, num_workers=10, pin_memory=True, drop_last=True)
    samTestLoader = DataLoader(samTestDataSet, batch_size=8, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)

    device = torch.device(device)
    model.to(device=device)
    model = nn.DataParallel(module=model, output_device=device)
    if finetune:
        optimizer = optim.AdamW(params=list(model.module.visionEncoder.parameters()), lr=learningRate, weight_decay=reguLambda)
    else:
