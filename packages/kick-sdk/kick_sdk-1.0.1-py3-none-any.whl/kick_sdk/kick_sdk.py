...
class KickSDK:
    ...
    def ban_user(self, channel_name: str, user_id: int) -> bool:
        request = self.session.post(f"https://kick.com/api/v1/channels/{channel_name}/mute-user", headers=AUTH_HEADERS, json={"user_id": user_id})
    ...
    def unban_user(self, username: str)  -> bool:
        request = self.session.delete(f"https://kick.com/api/v2/channels/devvince/bans/{username}", headers=AUTH_HEADERS)
