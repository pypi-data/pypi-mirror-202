...
def jupy(
    ...
    app = easy(
        identifier, version, url, headless, allow_sync_in_async=True, log_level="ERROR", instance_id=instance_id
    )
    app.enter()
    return app
...
def qt(identifier: str, version: str, parent, instance_id: str = "main") -> Arkitekt:
    """
...
def flussi(
    identifier: str, version: str = "latest", url: str = "http://localhost:8000", instance_id: str = "main"
):
    """
    A simple way to create an Arkitekt app with a device code grant
    it will cache the configuration in a file called `identifier`_token.json
