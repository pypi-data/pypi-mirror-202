...
@run.command()
@click.option(
    "--url", help="The fakts url for connection", default="http://localhost:8000", envvar="FAKTS_URL",
)
@with_instance_id
...
@run.command()
@click.option("--url", help="The fakts url for connection", envvar="FAKTS_URL", default="http://lok:8000")
@with_token
@with_instance_id
def port(url, token, instance):
    ...
    asyncio.run(
        run_port(manifest.entrypoint, manifest.identifier, manifest.version, url, token, instance_id=instance)
    )
