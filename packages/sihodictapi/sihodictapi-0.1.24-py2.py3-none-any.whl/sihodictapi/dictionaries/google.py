...
class Google:
    ...
    @classmethod
    def translate(cls, text: str, source_lang: Lang = None, target_lang: Lang = Lang.ZH_CN) -> str:
        return google_translator(proxies=utils.requests.get_proxies()).translate(text, target_lang, source_lang)

    @classmethod
    def detect(cls, text: str) -> Lang:
        resp = google_translator(proxies=utils.requests.get_proxies()).detect(text)
        return cls.Lang(resp[0])
