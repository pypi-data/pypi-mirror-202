...
class ExtensionOptionDynamicFeeTx(betterproto.Message):
    """
    ExtensionOptionDynamicFeeTx is an extension option that specify the
...
class ExtensionOptionsWeb3Tx(betterproto.Message):
    ...
    """
    typed data chain id used only in EIP712 Domain and should match Ethereum
    ...
    """
    fee payer sig is a signature data from the fee paying account, allows to
...
class TxResult(betterproto.Message):
    ...
    """
    eth tx index, the index in the list of valid eth tx in the block,  aka. the
    transaction list returned by eth_getBlock api.
    """
    ...
    """
    gas used by tx, if exceeds block gas limit, it's set to gas limit which is
    what's actually deducted by ante handler.
    """
