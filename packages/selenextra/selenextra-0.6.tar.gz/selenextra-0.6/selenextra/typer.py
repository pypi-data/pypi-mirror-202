...
class Typer():
    ...
    def getTuple(self, char):
        k = [(index, row.index(char))
             for index, row in enumerate(self.qwertyKeyboardArray) if char in row]
        arr = self.qwertyKeyboardArray
        if len(k) == 0:
            k = [(index, row.index(char)) for index, row in enumerate(
                self.qwertyShiftedKeyboardArray) if char in row]
    ...
    def getAllNeighbors(tup, arr, dist):
        ...
        for k in range(len(xs)):
            tups += [(xs[k], ys[i]) for i in range(len(ys))
                     if ys[i] <= bounds[xs[k]] and ys[i] >= 0]
        return tups
