...
def get_cache_dir(ensure_existence=False):
    ...
    if specified:
        ...
        p = Path(xdg, 'winpacker')

    elif sys.platform == 'darwin':
        p = Path(os.path.expanduser('~'), 'Library/Caches/winpacker')
        ...
        if local.startswith('~'):
            logger.warning(
                "Could not find cache directory. Please set any of "
                "these environment variables: "
                "LOCALAPPDATA, HOME, USERPROFILE or HOMEPATH")
        p = Path(local, 'winpacker')
