...
def find_disp(start_id, byte_array):
    disp = 0
    for _ in byte_array:
        b = struct.unpack('c', byte_array[start_id + disp : start_id + disp + 1])
        if is_hex_char(b):
            return disp
        disp += 1
    return disp

def str_filter(ln: str) -> str:
    for ch in ['b', '\\n', "'"]:
        ln = ln.replace(ch, '')
    return ln

def is_regular_line(ln: str) -> bool:
    if ('(' in ln) and (')' in ln):
        return True
    else:
        return False    

def make_regular_face(ln: str) -> list:
    return [int(s) for s in re.findall("\d+", ln)[1:]]

def find_ending_idx(content: list) -> int:
    for i, ln in enumerate(content[::-1]):
        ln = str_filter(str(ln))
        if ln == ')':
            return len(content) - (i + 1)
    return -1

def create_ascii_data(content: list, n: int) -> list:
    data = []
    idx_end = find_ending_idx(content)
    idx_start = n + 2
    L = len(content[idx_start:idx_end])
    i = 0

    while i < L:
        ln = content[idx_start + i]
        ln = str_filter(str(ln))
        
        if is_regular_line(ln):
            data.append(make_regular_face(ln))
            i += 1
            continue
        ...
        if ln.isdigit():
            n = int(ln)
            next_ln = str_filter(str(content[idx_start + i + 1]))
            if next_ln == '(':
                tmp_data = []
                for j in range(n):
                    id = str_filter(str(content[idx_start + i + 2 + j]))
                    tmp_data.append(int(id))
                data.append(tmp_data)
            i += n + 3
            continue
        else:
            raise ValueError(f"Unknown type of lines:\n {content[idx_start + i + 1:idx_start + i + 10]}")
        
        raise ValueError(f"Unknown type of line:\n {ln}")
    return data
    ...
    def parse_faces_content(cls, content, is_binary, skip=10):
        ...
        while n < len(content):
            ...
            if is_integer(lc):
                num = int(lc)
                if not is_binary:
                    data = create_ascii_data(content, n + 2)
                else:
                    buf = b''.join(content[n+1:])
                    disp_1 = find_disp(0, buf)
                    end_1 = num * struct.calcsize('i') + disp_1
                    disp_2 = find_disp(end_1, buf) + disp_1

                    idx = struct.unpack(f"{num}i", buf[disp_1 : num * struct.calcsize('i') + disp_1])
                    pp = struct.unpack(f"{idx[-1]}i", 
                                       buf[disp_2 + num * struct.calcsize('i') : disp_2 + (num + idx[-1]) * struct.calcsize('i')])
                    data = []
