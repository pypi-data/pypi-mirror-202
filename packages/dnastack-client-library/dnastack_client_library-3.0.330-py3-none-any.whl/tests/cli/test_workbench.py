...
class TestWorkbenchCommand(WorkbenchCliTestCase):
    ...
    def test_workflows_list(self):
        ...
        def test_source():
            ...
            if len(result) == 0:
                ...
                self.assertGreater(len(result), 0, 'Expected at least one workflow.')
                self.assertTrue(all(workflow.source == 'CUSTOM' for workflow in result),
    ...
    def test_workflows_create_and_add_version(self):
        ...
        def test_name_and_description():
            ...
            self.assertTrue('TITLE' in created_workflow.description and 'DESCRIPTION' in created_workflow.description,
        ...
        def test_edit_workflow():
            edited_workflow = Workflow(**self.simple_invoke(
                'alpha', 'workbench', 'workflows', 'update',
                '--name', 'UPDATED',
                '--authors', 'foo,bar',
                '--description', 'updated',
            ...
            self.assertEqual(edited_workflow.authors, [])

        test_edit_workflow()

        def test_add_version():
            created_workflow_version = WorkflowVersion(**self.simple_invoke(
                'alpha', 'workbench', 'workflows', 'versions', 'add',
        ...
        def test_edit_version():
            edited_workflow_version = WorkflowVersion(**self.simple_invoke(
                'alpha', 'workbench', 'workflows', 'versions', 'update',
                '--workflow', created_workflow.internalId,
                '--name', 'UPDATED',
                '--authors', 'foo,bar',
                '--description', 'updated',
    ...
    def test_workflow_version_list(self):
        ...
        result = [WorkflowVersion(**workflow_version) for workflow_version in self.simple_invoke(
            'alpha', 'workbench', 'workflows', 'versions', 'list', '--workflow', workflow_id
        )]
    ...
    def test_workflow_version_describe(self):
        ...
        workflow = [Workflow(**w) for w in self.simple_invoke(
            'alpha', 'workbench', 'workflows', 'describe', workflow_id
        )][0]

        result = [WorkflowVersion(**workflow_version) for workflow_version in self.simple_invoke(
            'alpha', 'workbench', 'workflows', 'versions', 'describe', '--workflow', workflow_id,
