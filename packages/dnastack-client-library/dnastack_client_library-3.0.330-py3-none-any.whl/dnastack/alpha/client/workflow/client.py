...
class WorkflowClient(BaseWorkbenchClient):
    ...
    def list_workflows(self, source: Optional[WorkflowSource]) -> Iterator[Workflow]:
        with self.create_http_session() as session:
            response = session.get(
    ...
    def list_workflow_versions(self, workflow_id: str) -> Iterator[WorkflowVersion]:
        with self.create_http_session() as session:
            response = session.get(
    ...
    def get_workflow_version(self, workflow_id: str, version_id: str) -> WorkflowVersion:
        with self.create_http_session() as session:
            response = session.get(
