...
def list_workflows(context: Optional[str],
    ...
    show_iterator(output_format=OutputFormat.JSON, iterator=workflows_client.list_workflows(source=source))
...
def update_workflow(context: Optional[str],
    ...
    if patch_list:
        ...
        raise ValueError("Must specify at least on attribute to update")
...
def describe_version(context: Optional[str],
    ...
    click.echo(to_json(normalize(
        [workflows_client.get_workflow_version(workflow_id=workflow, version_id=version_id) for version_id in versions]
    )))
