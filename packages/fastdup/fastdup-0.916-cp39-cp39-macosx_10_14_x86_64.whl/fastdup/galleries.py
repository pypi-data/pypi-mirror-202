    ...
    if (get_label_func is not None):
        if isinstance(get_label_func, str):
            if os.path.exists(get_label_func):
                ...
                df.loc[:,out_col] = generate_vqa_labels(df[in_col], "Is the photo taken indoors or outdoors", kwargs)
            else:
            ...
            #    df[out_col] = df['__id' if '__id' in df.columns else ('index' if 'index' in df.columns else in_col)].apply(lambda x: get_label_func(x))
            #else:
...
if __name__ == "__main__":
    ...
    if False:
        ...
        import fastdup
        fastdup.init_search(10, '/Users/dannybickson/visual_database/cxx/unittests/search_out', license='magical')
        import cv2
