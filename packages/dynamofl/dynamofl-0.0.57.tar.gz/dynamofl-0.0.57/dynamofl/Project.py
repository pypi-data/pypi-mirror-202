...
class _Project:
    def __init__(self, key=None, params=None, api: ProjectAPI = None):
        """
        ...
        if key:  # prioritizing the presence of a key
            self.key = key
        elif params:
            self.key = self._api.create_project(params)['key']
        else:
            raise Exception(
                'Either key or params is required to create a project')
    ...
    def push_model(self, path, datasource_key, params=None):
        self._api.push_model(project_key=self.key, path=path,
                             datasource_key=datasource_key, params=params)
    ...
    def push_ids(self, base_file):
        self._api.push_ids(base_file=base_file, project_key=self.key)

    def pull_model(self, filepath, datasource_key=None, round=None, federated_model=None):
        self._api.pull_model(project_key=self.key, filepath=filepath,
                             datasource_key=datasource_key, round=round, federated_model=federated_model)
