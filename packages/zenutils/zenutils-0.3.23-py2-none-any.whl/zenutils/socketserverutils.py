...
class ServerEngineBase(object):
    ...
    def __init__(self, config):
        ...
        self._services = {} # hold all registered services, categoried by service namespace.
    ...
    def register_function(self, method, name):
        _logger.debug("Registering method {name}: {method}".format(name=name, method=method))
...
class NStreamExchangeProtocolBase(object):
    ...
    def setup(self):
        self.max_request_size = self.config.select("server.max_request_size", 2**20*4) # default to 4M
        self.buffer_size = self.config.select("server.buffer_size", 4096)
        self.rfile_buffer_size = self.config.select("server.rfile_buffer_size", self.buffer_size)
        self.wfile_buffer_size = self.config.select("server.wfile_buffer_size", self.buffer_size)
        self.rfile = self.client_socket.makefile("rb", self.rfile_buffer_size) # the second parameter named buffering in new version python, bug named bufsize in old version python
        self.wfile = self.client_socket.makefile("wb", self.wfile_buffer_size) # so we just using positional parameters here...
    ...
    def handle(self):
        try:
            ...
            _logger.info("socket server process info: client_address={address}, info=connection closed by client.".format(address=self.client_address))
        except Exception as error:
            error = errorutils.BizError(error)
            _logger.exception("socket server process error: client_address={address}, error_code={code}, error_message={message}...".format(
            ...
            try:
                ...
                _logger.info("socket server process info: client_address={address}, info=connection closed by server.".format(address=self.client_address))
            except Exception as error:
                _logger.info("socket server process error: client_address={address}, error_message=close client socket failed, system_error_message={error}...".format(address=self.client_address, error=error))
    ...
    def get_request_size(self):
        ...
        if size > self.max_request_size:
            raise errorutils.TooLargeRequestError(size=size, maxsize=self.max_request_size)
        return size
    ...
    def send_response(self, response_bytes):
        ...
        response_size_bytes = struct.pack(self.package_size_struct_pack_sign, response_size)
...
class ServerHandle(object):
    ...
    def __init__(self, server_engine, config, exchange_protocol_class=None):
        ...
        if exchange_protocol_class:
            ...
            exchange_protocol_class_name = self.config.select("server.exchange_protocol", None)
            ...
            self.exchange_protocol_class = importutils.import_from_string(exchange_protocol_class_name)

    def __call__(self, socket, address):
        protocol = self.exchange_protocol_class(self.server_engine, self.config, socket, address)
        protocol.handle()
