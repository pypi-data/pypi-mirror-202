...
class CertoraBuildGenerator:
    ...
    def instrument_imports(self, build_arg_contract_file: str, instr: Dict[str, Dict[int, Instrumentation]]) -> bool:
        ...
        for file_name, import_id in imports_to_rewrite:
            # we're only working with abstract paths
            file_name_abs = abs_posix_path(file_name)
            if file_name_abs not in instr:
                instrumentation_logger.debug(f"Adding {file_name} (abs: {file_name_abs}) "
                                             f"to instrumentation from import {import_id}")
                instr[file_name_abs] = dict()
            per_file_instr = instr[file_name_abs]
