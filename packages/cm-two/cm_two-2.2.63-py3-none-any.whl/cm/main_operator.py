...
class Operator:
    ...
    def tara_too_little_react(self, too_little):
        if too_little and not self.currentPage.blockImgDrawn:
            self.currentPage.initBlockImg('tare_too_little_win',
    ...
    def gross_too_little_react(self, too_little):
        if too_little and not self.currentPage.blockImgDrawn:
            self.currentPage.initBlockImg('brutto_too_little_win',
                                          'tare_too_little_btns')
        if not too_little:
            self.currentPage.destroyBlockImg('total')
    ...
    def operate_get_history(self, status, records):
        ...
        if status == 'success':
            ...
            for record in records:
                ...
                self.statPage.tar.insertRec(id=act_number,
    ...
    def update_status_operate(self, info):
        ...
        try:
            ...
            if info['status'] == 'Начало взвешивания' and self.currentPage.orupState:
                self.currentPage.destroyORUP('total')
            if (info[
            ...
            if (info['protocol'].strip() == 'Машина заезжает.' or info[
                ...
                self.updateMainTree()
            self.drawStatus()
            if (self.current == 'MainPage' or self.current == 'ManualGateControl') \
