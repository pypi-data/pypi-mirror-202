...
from ..component import SplEvent
...
class _Coverage:
    ...
    def __init__(self, allele_num: Optional[int], isoform_num: Optional[int], event: SplEvent, plot_gene: bool = False):
        data = []
        determined_map = {}
        ...
        self.valid_start = event.iv.start
        self.valid_end = event.iv.end

        if plot_gene:
            all_exons = event.get_all_support_gene_exons()
            self.valid_start = all_exons[0].start
            self.valid_end = all_exons[-1].end
    ...
    def name(self, n: int):
        ...
        isoform_num = ['A', 'B'][self.isoform_num] if isoform_determined else '?'
        return 'Allele {} - {}'.format(allele_num, isoform_num)
    ...
    def add(self, determined_status, ivs: List[HTSeq.GenomicInterval]):
        ...
        for iv in ivs:
            iv.start = max(iv.start, self.valid_start)
            iv.end = min(iv.end, self.valid_end)
            if iv.start >= iv.end:
                continue
            ga[iv] += 1
...
class TaskPlot(TaskAlign, TaskResultMeta):
    ...
    def _generate_coord_offset_map(self, plot_gene: bool = False) -> HTSeq.GenomicArray:
        """Generate display coord offset map."""
        all_exons = self.event.get_all_exons()
        if plot_gene:
            all_exons = self.get_all_support_gene_exons_containing_snps_and_overlap_with_event_isoforms()
    ...
    def _load_coverage(self, plot_gene: bool = False):
        ...
        for align_stat in self.align_stats_iterator():
            ...
            idxes1 = set(self.event.gene_isoform_idxes_support_isoform1)
            idxes2 = set(self.event.gene_isoform_idxes_support_isoform2)
            idxes = idxes1.union(idxes2)

            def _compatible_event_isoforms(compatible_isoforms):
                if compatible_isoforms.issubset(idxes1):
                    return [0]
                if compatible_isoforms.issubset(idxes2):
                    return [1]
                if compatible_isoforms.issubset(idxes):
                    return [0, 1]
                return []

            isoform_determined, allele_determined = False, False
            if len(_compatible_event_isoforms(align_stat.compatible_isoforms)) == 1:
            ...
            for iso_num in _compatible_event_isoforms(align_stat.compatible_isoforms):
    ...
    def _plot_coverage(self, isoform_num: int, y_lim: int, plot_gene: bool = False):
        """Plot coverage."""
        out_path = self._coverage_path(allele_num=None, isoform_num=isoform_num)

        fig_size = (6, 3)
        if plot_gene:
            fig_size = (9, 3)

        with TaskFig(fig_size=fig_size, out_path=out_path) as task_fig:

            x_lim = (self._get_coord(self.event.iv.start) - 0.5,
                     self._get_coord(self.event.iv.end - 1) + 0.5)

            if plot_gene:
                all_exons = self.get_all_support_gene_exons_containing_snps_and_overlap_with_event_isoforms()
                x_lim = (self._get_coord(all_exons[0].start) - 0.5,
                         self._get_coord(all_exons[-1].end - 1) + 0.5)

            # Plot isofrom.
            ax = task_fig.fig.add_axes([0.1, 0.05, 0.8, 0.1])
            # ax = task_fig.fig.add_axes([0.1, 0.8, 0.8, 0.1])
            ax.set_axis_off()

            if isoform_num == 0:
                isoform_nums = self.event.gene_isoform_idxes_support_isoform1
                exons = self.event.isoform1.exons
            else:
                assert isoform_num == 1
                isoform_nums = self.event.gene_isoform_idxes_support_isoform2
                exons = self.event.isoform2.exons

            snps, _ = self.support_gene_isoform_snps_and_exons(isoform_nums=isoform_nums)
            exons = sorted(exons, key=lambda _iv: _iv.start)

            if plot_gene:
                exons = self.get_support_gene_exons_containing_snps_and_overlap_with_event_isoforms(isoform_nums)
            ...
            for exon_num, exon in enumerate(exons):
                ...
                ax.add_patch(Rectangle((s, 0), e - s, 1, facecolor=(0, 0, 0, 0.1), linewidth=0))

                if prev_exon_end is not None:
                    ax.add_line(Line2D([prev_exon_end, s], [0.5, 0.5], linestyle=':', color='C4', alpha=0.2))
                prev_exon_end = e

            for allele_num in range(2):
                for snp in snps:
                    x = self._get_coord(snp.pos)
                    ax.add_line(Line2D([x, x], [0+0.5*(1-allele_num), 0.5+0.5*(1-allele_num)],
                                       color=_nt_color(snp.alleles[allele_num]), alpha=0.6))

            if plot_gene:
                s = self._get_coord(self.event.iv.start)
                e = self._get_coord(self.event.iv.end)

                ax.add_patch(Rectangle((s+0.05, 0.05), e-s-0.1, 1-0.1, facecolor='none', edgecolor='red',
            ...
            for allele_num in range(2):
                if allele_num == 0:
                    ax = task_fig.fig.add_axes([0.1, 0.55, 0.8, 0.35])
                    # ax = task_fig.fig.add_axes([0.1, 0.425, 0.8, 0.35])
                else:
                    assert allele_num == 1
                    ax = task_fig.fig.add_axes([0.1, 0.175, 0.8, 0.35])
                    # ax = task_fig.fig.add_axes([0.1, 0.05, 0.8, 0.35])

                title = 'Isoform: ' + [self.event.isoform1, self.event.isoform2][isoform_num].name
                # colors = [_COLOR_BLUE, _COLOR_ORANGE, _COLOR_GREEN, _COLOR_RED]
                # alphas = [0.8, 0.65, 0.5, 0.35]
                # colors.append(_COLOR_GREY)
                # alphas.append(0.2)

                colors = [[_COLOR_BLUE, _COLOR_ORANGE, _COLOR_GREEN], [_COLOR_RED, _COLOR_ORANGE, _COLOR_GREEN]]
                alphas = [0.8, 0.5, 0.5]

                cov = self.cov_dict[(allele_num, isoform_num)]
                for i, ga in enumerate([cov.data[0], cov.data[1], cov.data[2]]):
                    x = []
                    y = []
                    for iv, v in ga.steps():
                        if v != 0:
                            ...
                            for k in [0, v, v, 0]:
                                y.append(k)
                    ax.fill_between(x, 0, y, lw=0, color=colors[allele_num][i], alpha=alphas[i],
    ...
    def _plot_histogram(self, var_name: str, title: str):
        ...
        with TaskFig(fig_size=(3, 3), out_path=out_path) as task_fig:
            ...
            ax.hist(trace, bins=np.arange(-2, 2, 0.02), density=True, rwidth=0.75, histtype='bar', color=_COLOR_BLUE)
    ...
    def _generate_html(self):
        ...
        if self.plot_type is TaskPlotType.ASAS:
            for isoform_num in range(2):
                html += '<tr class="plot {}">\n'.format('g' if isoform_num is None else 'i{}'.format(isoform_num))
                # for allele_num in range(2):
                #     var = self.get_var_expr(allele_num=allele_num)
                ...
                img_path = self._coverage_path(allele_num=None, isoform_num=isoform_num)
                td_class = 'plot cov'
                html = _add_td(html, td_class, img_path)
    ...
    def plot(self, plot_gene) -> str:
        """Plot."""
        if os.path.exists(self._html_path):
            return self._html_path

        self.coord_offset_map = self._generate_coord_offset_map(plot_gene=plot_gene)
        self.cov_dict = self._load_coverage(plot_gene=plot_gene)

        self.mc.handle_progress('Plot histograms...')
        if self.plot_type is TaskPlotType.ASAS:
            # for allele_num in range(2):
            #     var = self.get_var_expr(allele_num=allele_num)
            #     title = 'Isoform A - Allele {}'.format(allele_num+1)
            #     self._plot_histogram(var, title)

            var = self.get_var_expr_diff()
            title = 'Isoform A - Difference'
            self._plot_histogram(var, title)
        ...
        if self.plot_type is TaskPlotType.ASAS:
            max_coverage = 0
            for allele_num in range(2):
                for iso_num in range(2):
                    tmp_max = self.cov_dict[(allele_num, iso_num)].get_max()
                    max_coverage = max(max_coverage, tmp_max)

            for iso_num in range(2):
                self._plot_coverage(isoform_num=iso_num, y_lim=max_coverage, plot_gene=plot_gene)
