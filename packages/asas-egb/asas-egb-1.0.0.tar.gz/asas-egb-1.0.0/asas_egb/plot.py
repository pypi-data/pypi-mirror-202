...
def plot_task(args):
    ...
    task_plot_dir = os.path.join(plot_dir, ('' if not plot_gene else '[gene-view] ')+task_id)
    for dir_path in [plot_dir, task_plot_dir]:
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)

    plot_type = TaskPlotType.ASAS
    ...
    html_path = task_plot.plot(plot_gene=plot_gene)
    return html_path
