...
def _add_gen_annotation_parser(subparsers):
    parser = subparsers.add_parser('build-database', help='Build database for tasks.')

    parser.add_argument('-e', '--event-gff', required=True,
                        help='The path of the GFF splicing events annotation file.')
    parser.add_argument('-G', '--event-gene-feature', default='gene',
                        help='The feature type name of genes of the splicing events annotation file, '
                             'in the 3rd column of the GFF file. (DEFAULT: gene)')
    parser.add_argument('-I', '--event-isoform-feature', default='mRNA',
                        help='The feature type name of isoforms of the splicing events annotation file, '
                             'in the 3rd column of the GFF file. (DEFAULT: mRNA)')
    parser.add_argument('-A', '--event-gene-id-attr', default='gene_id',
                        help='The attribute that describes gene ID of events, '
                             'extracted from the 9th column of the GFF file. (DEFAULT: gene_id)')

    parser.add_argument('-f', '--gene-gff', required=True,
                        help='The path of the GFF gene annotation file.')
...
def _add_inference_parser(subparsers):
    ...
    parser.add_argument('-d', '--database', required=True, help='The path of database.')
...
def _add_plot_parser(subparsers):
    ...
    parser.add_argument('-g', '--gene-view', action='store_true', help='Plot results in gene isoform view.')
