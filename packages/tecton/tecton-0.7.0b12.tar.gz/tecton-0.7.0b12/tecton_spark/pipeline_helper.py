        ...
        if transformation.transformation_mode == TransformationMode.TRANSFORMATION_MODE_PYSPARK:
            ...
            try:
                if isinstance(self, _ODFVPipelineBuilder):
                    ...
                    @pandas_udf(self._output_schema)
                    def transform_rows(df_group: pd.DataFrame) -> pd.DataFrame:
                        # Apply your desired transformation on the input group of rows
                        transformed_df_group = user_function(df_group)
