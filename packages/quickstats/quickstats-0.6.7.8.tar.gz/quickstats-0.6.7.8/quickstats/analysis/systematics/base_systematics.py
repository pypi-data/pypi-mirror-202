...
from quickstats import AbstractObject, DescriptiveEnum, ConfigurableObject, semistaticmethod
from quickstats.utils.common_utils import filter_dataframe_by_index_values
...
class BaseSystematics(ConfigurableObject):
    ...
    def resolve_samples(self, samples:Optional[List]=None):
        ...
        if self.dataframe is not None:
            ...
            sample_list = pd.unique(sample_list)
    ...
    def resolve_syst_themes(self, syst_themes:Optional[List]=None):
        ...
        if self.dataframe is not None:
            ...
            syst_theme_list = pd.unique(syst_theme_list)
    ...
    def resolve_syst_names(self, syst_names:Optional[List]=None):
        ...
        syst_name_list = pd.unique(syst_name_list)
    ...
    def resolve_categories(self, categories:Optional[List]=None):
        ...
        if self.dataframe is not None:
            ...
            category_list = pd.unique(category_list)
    ...
    def _get_summary(self, prefix:str='',
        ...
        is_pass = self.sanity_check(check_dataframe=True,
                                    check_rel_effect=True,
                                    strict=False)
        ...
        for category in categories:
            ...
            for sample in samples:
                ...
                for syst_name, val_up, val_down in zip(data['systematics'],
                    ...
                    summary_data[category][sample][f"{prefix}{syst_name}"] = {
                        'Constr': 'asym',
                        'CentralValue': 1,
                        'Mag': [val_up/100., val_down/100.]
                    }
        return summary_data