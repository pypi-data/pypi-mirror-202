...
class RedisStorage(AbstractStorage):
    ...
    def load_cookie(self, request):
        ...
        cookie = request.query.get('jwt') or request.query.get('apikey') or request.query.get('sessionid') or request.query.get('token')
        # 登入时，jwt == sessionid
        if cookie is None:
    ...
    async def load_session(self, request):
        ...
        if cookie is None:
            ...
            data = await self._redis.get('session:' + key)
            if data is None:
                # 经过了kong 的 apikey认证
                if request.headers.get('X-Consumer-Username') and request.headers.get('X-Consumer-Custom-Id'):
                    session_data = {
                        'created':  int(time.time()),
                        'session': {"uid": request.headers.get('X-Consumer-Custom-Id'),
                                               "uname": request.headers.get('X-Consumer-Username')}
                    }
                    return Session(key, data=session_data,
