...
def sign_and_request(url: str,
    ...
    if has_jwt_package and auth_type == AuthType.AuthSDK:
        ...
        headers['YYCtoken'] = sign(url, params)
    elif auth_type == AuthType.YHT:
        headers['cookie'] = f'yht_access_token={sign_yht()}'
...
def sign_yht():
    ...
    if expire != 0 and time.time() < expire:
        ...
        AuthType.AuthCache['yhtToken'] = {'expire': time.time() + 900, 'token': token}
        return token
...
def __do_request(url: str,
    ...
    for i in range(1, 5):
        ...
        try:
            ...
            time.sleep(i * 2)
            print(
                f"[request] request retry time: {i}, url: {url}, body: {e.read()}, error: {e}")
