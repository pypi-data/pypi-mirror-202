...
class DataSourceNLPCorpora(AbstractDataSource):
    ...
    def __init__(self, source_address: str, ds_id: str, source_train_type: int, reader_type: str = CorpusInputType.local, download_dir='./tmp_local_nlp_corpora_origin_data/'):
    ...
    def __download_file(self, source_address, ds_id, source_train_type, download_dir):
        ...
        request_data = {'dsIds': ds_id,
                        'type': source_train_type, 'tenantId': config.TENANT_ID}
    ...
    def _get_reader(self, pagesize=5000):
        if self.reader_type == "row":
            return DataSourceNLPCorpora.__OnlineReader(self.source_address, self.ds_id, pagesize)
        else:
            return self.__localReader(self._file_list, pagesize)

    def __localReader(self, file_paths, limit=5000, encoding="utf-8"):
    ...
    class __OnlineReader:
        def __init__(self, input_address, ds_id, limit=5000):
            """
