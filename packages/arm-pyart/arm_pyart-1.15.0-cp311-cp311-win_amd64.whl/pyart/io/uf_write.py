...
def _find_field_mapping(radar, uf_field_names, radar_field_names, exclude_fields):
...
class UFRayCreator:
    ...
    def _calc_record_length(radar, field_mapping, field_write_order):
        ...
        data_types = [
            field_mapping[field].encode("ascii") for field in field_write_order
        ]
    ...
    def _set_field_header(self):
        ...
        range_start -= range_step / 2.0  # range bin center to edge
        ...
        if iparams is not None and "radar_receiver_bandwidth" in iparams:
            ...
            header["bandwidth"] = int(round(bandwidth / 1.0e6 * 16))
        else:
    ...
    def make_field_header(self, data_offset, ray_num, scale_factor):
        ...
        if iparams is not None and "prt" in iparams:
            ...
            field_header["prt_ms"] = int(round(prt * 1.0e6))
        else:
    ...
    def make_data_array(self, field, ray_num, scale=100.0):
