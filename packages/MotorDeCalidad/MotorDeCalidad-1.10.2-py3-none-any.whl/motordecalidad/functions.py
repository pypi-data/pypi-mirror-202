# Installation of the azure-storage-blob library through the pip install subprocess
import subprocess
libreria = "azure-storage-blob"
...
# Configuration of the logging handler
logging.basicConfig(filename='output.log',level=logging.INFO,format='%(asctime)s %(levelname)s %(message)s')
logging.getLogger("py4j").setLevel(logging.ERROR)
conn_string = "DefaultEndpointsProtocol=https;AccountName=adlseu2edthdev001;AccountKey=T1RZsgj62zrRWcsYRW3QGr3+TEhtalj8o/fU3Zqmh4ef3TYxZw0P7+neqmgOPmbFOoVPZhLFT9GV+AStAj2YpA==;EndpointSuffix=core.windows.net"
...
inform("Motor de Calidad Version Release 1.10.2")
...
def startValidation(inputspark,config,dfltPath=""):
    try:
        ...
        blob_client = blob_service_client.get_blob_client(container='metadata', blob=f"/log/{project}/{entity}/{dataDate}/otput.log")
        with open('output.log', "rb") as data:
...
def validateRules(object:DataFrame,rules:dict,registerAmount:int, entity: str, project:str,country: str,domain: str,subDomain: str,segment: str,area: str,error,dataDate:str,validData:str, Data,email,output):
            ...
            if code[0:3] == Rules.Pre_Requisites.code:
                ...
                for field in columnName:
                    ...
                    if formatDate in PermitedFormatDate:
                        ...
                        inform("Los formatos permitidos son: ", PermitedFormatDate)
                        inform("El formato solicitado fue: ", formatDate)
                        inform("regla de formato: %s segundos" % (time.time() - t))
    ...
    blob_client = blob_service_client.get_blob_client(container='metadata', blob=f"/log/{project}/{entity}/{dataDateWidget}/otput.log")
    with open('output.log', "rb") as data:
        blob_client.upload_blob(data, overwrite = True)
