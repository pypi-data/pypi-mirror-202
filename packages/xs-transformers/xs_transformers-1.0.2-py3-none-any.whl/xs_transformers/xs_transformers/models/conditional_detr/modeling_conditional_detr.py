...
class ConditionalDetrAttention(nn.Module):
    ...
    def _v_shape(self, tensor: torch.Tensor, seq_len: int, batch_size: int):
        return (
            tensor.view(batch_size, seq_len, self.num_heads, self.v_head_dim)
            .transpose(1, 2)
            .contiguous()
        )
    ...
    def forward(
        ...
        query_states = self._qk_shape(query_states, target_len, batch_size).view(
            *proj_shape
        )
        key_states = key_states.view(*proj_shape)
...
class ConditionalDetrForSegmentation(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        features, position_embeddings_list = self.conditional_detr.model.backbone(
