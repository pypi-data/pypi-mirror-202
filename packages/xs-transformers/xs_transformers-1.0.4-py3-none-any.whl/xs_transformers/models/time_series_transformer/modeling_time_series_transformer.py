...
class TimeSeriesTransformerPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(
            module, (TimeSeriesTransformerDecoder, TimeSeriesTransformerEncoder)
        ):
...
class TimeSeriesTransformerModel(TimeSeriesTransformerPreTrainedModel):
    ...
    def create_network_inputs(
        ...
        inputs_length = (
            self._past_length + self.config.prediction_length
            if future_values is not None
    ...
    @replace_return_docstrings(
        output_type=Seq2SeqTimeSeriesModelOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        past_values: torch.Tensor,
    ...
    def generate(
        ...
        for k in range(self.config.prediction_length):
            ...
            repeated_past_values = torch.cat(
                (repeated_past_values, next_sample / repeated_scale), dim=1
            )
