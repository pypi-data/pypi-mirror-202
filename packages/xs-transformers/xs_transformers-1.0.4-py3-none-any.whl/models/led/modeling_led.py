...
class LEDEncoderSelfAttention(nn.Module):
        ...
        if hidden_states.size(1) == window_overlap * 2:
            ...
            for chunk in range(chunk_size[1]):
                overlapping_chunks[:, chunk, :, :] = hidden_states[
                    :,
                    chunk * window_overlap : chunk * window_overlap
                    + 2 * window_overlap,
                    :,
                ]
    ...
    def _sliding_chunks_query_key_matmul(
        ...
        query = self._chunk(
            query, window_overlap, self.config.__dict__.get("onnx_export", False)
        )
        key = self._chunk(
            key, window_overlap, self.config.__dict__.get("onnx_export", False)
        )
