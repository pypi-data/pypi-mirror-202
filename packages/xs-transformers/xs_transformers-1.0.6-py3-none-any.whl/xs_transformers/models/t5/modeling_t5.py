...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        # print("Is this use for beam search?")
        # print("hidden_states: ", hidden_states)
        # print("present_key_value_states:", present_key_value_states)
        # print("hidden_states: ", hidden_states)
        # print("attentions: ", all_attentions)
...
class T5Model(T5PreTrainedModel):
    ...
    def forward(
        ...
        # print("KH chnage the encode algorithm, remove byT5 encoder and accept the perceiver encoder's embedding")
        if encoder_outputs is None and input_hidden_state is not None:
            # print("Load perceiver's hidden state")
            encoder_ouputs = input_hidden_state
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        # Encode if needed (training, first prediction pass)
        # print("modeling t5.py: KH changes T5's encoder to perceiver's hidden state embedding")
        # print("encoder_outputs: ", encoder_outputs)
        # print("input_hidden_state: ", input_hidden_state)
        if encoder_outputs is None and input_hidden_state is not None:
            # print("Load perceiver embedding...")
            encoder_outputs = BaseModelOutputWithPastAndCrossAttentions(
                last_hidden_state=input_hidden_state,
            )
        elif encoder_outputs is None:
            # print("Original encoder_outputs")
            ...
            # print("this is for beam search?")
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
                hidden_states=encoder_outputs[1] if len(encoder_outputs) > 1 else None,
                attentions=encoder_outputs[2] if len(encoder_outputs) > 2 else None,
            )
        # print("encoder_outputs: ", encoder_outputs, type(encoder_outputs))

        hidden_states = encoder_outputs[0]
        # print("hidden stateds: ", hidden_states.shape)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if "input_hidden_state" in kwargs:
            return {
                "decoder_input_ids": input_ids,
                ...
                "cross_attn_head_mask": cross_attn_head_mask,
                "use_cache": use_cache,
                "input_hidden_state": kwargs["input_hidden_state"],
            }
