        ...
        **model_kwargs,
        ...
        if input_ids is not None:
            if isinstance(input_ids, tf.Tensor) and input_ids.dtype.is_floating:
                pass
            elif isinstance(input_ids, np.ndarray) and np.issubdtype(
                input_ids.dtype, np.floating
            ):
                pass
            else:
        ...
        if not self.config.is_encoder_decoder:
            if pad_token_id is not None and tf.math.reduce_any(
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        expanded_return_idx = tf.reshape(
            tf.tile(
                tf.reshape(tf.range(tf.shape(input_ids)[0]), (-1, 1)), (1, expand_size)
            ),
            (-1,),
        )
