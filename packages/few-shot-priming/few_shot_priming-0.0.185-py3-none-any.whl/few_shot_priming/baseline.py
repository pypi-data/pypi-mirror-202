...
from transformers import BertTokenizer, DebertaModel, DebertaTokenizer
...
class DeBERTaClassifier(nn.Module):
    def __init__(self, hyperparameters, path=None):
        ...
        self.linear = nn.Linear(512, self.head_size)
        self.classifier_head = nn.Linear(self.head_size, 1)
        if path:
            self.deberta = DebertaModel.from_pretrained(path)
        else:
