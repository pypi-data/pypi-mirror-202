...
from transformers import BertTokenizer, DebertaForSequenceClassification, DebertaTokenizer
from few_shot_priming.few_shot_stance import *

class Dataset(torch.utils.data.Dataset):
    def __init__(self,df, labels, path=None, model='bert-base-cased'):
        if path:
            if model.startswith("bert"):
                tokenizer = BertTokenizer.from_pretrained(path)
            else:
                tokenizer = DebertaTokenizer.from_pretrained(path)
                print("deberta tokenizer")
        else:
            if model.startswith("bert"):
                tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
            elif model.startswith("microsoft/deberta-base"):
        ...
        for i, record in df.iterrows():
            if model.startswith("bert"):
                self.texts.append(tokenizer( record["topicText"], record['claims.claimCorrectedText'], padding="max_length", max_length=768,
                                        truncation=True, return_tensors="pt"))
            else:
                self.texts.append(tokenizer( record['claims.claimCorrectedText'],truncation=True,  padding="max_length", max_length=512, return_tensors="pt"))
...
class DeBERTaClassifier(nn.Module):
    def __init__(self, hyperparameters, path=None):
        ...
        if path:
            self.deberta = DebertaForSequenceClassification.from_pretrained(path)
        else:
            self.deberta = DebertaForSequenceClassification.from_pretrained('microsoft/deberta-base')
        self.labels = {"CON": 0, "PRO":1}

    def forward(self, input_ids, mask):
        print(input_ids.shape)
        print(mask.shape)
        _, pooled_output = self.deberta(input_ids=input_ids, attention_mask=mask, return_dict=False)
...
def run_finetuning_experiment_baseline(params=None, offline=True, validate=False):
    splits = load_splits()
    splits["validation"] = splits["validation"].sample(10)
    ...
    train_dataset = Dataset(splits["training"], {"PRO": 1, "CON": 0}, path=path, model=model_name)
    if validate:
        experiment_type = "validate"
        test_dataset = Dataset(splits["validation"], {"PRO": 1, "CON": 0}, path=path, model=model_name)
    else:
        experiment_type = "test"
        test_dataset = Dataset(splits["test"], {"PRO": 1, "CON": 0}, path=path, model=model_name)
    ...
    for epoch in range(epochs):
        ...
        con_f1 = f1_score(all_test_labels, all_test_labels,average='binary', pos_label=0)
