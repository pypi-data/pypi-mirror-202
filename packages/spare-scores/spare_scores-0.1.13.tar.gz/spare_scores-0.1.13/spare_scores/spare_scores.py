...
def spare_test(df: Union[pd.DataFrame, str],
  ...
  logging.info('Model Info: training N =', len(meta_data['cv_results'].index), end=' / ')
  logging.info('ages =', int(np.floor(np.min((meta_data['cv_results']['Age'])))),
               '-', int(np.ceil(np.max((meta_data['cv_results']['Age'])))), end=' / ')
  stats_metric = list(meta_data['stats'].keys())[0]
  logging.info(f'expected {stats_metric} =', '{:.3f}'.format(np.mean(meta_data['stats'][stats_metric])))
