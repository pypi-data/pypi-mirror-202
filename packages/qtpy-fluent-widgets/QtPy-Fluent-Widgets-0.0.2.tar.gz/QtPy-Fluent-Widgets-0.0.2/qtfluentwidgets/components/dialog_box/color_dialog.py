...
from qtpy.QtGui import QBrush, QColor, QPixmap, QPainter, QPen, QIntValidator, QIcon
...
class HexColorLineEdit(ColorLineEdit):
    ...
    def __init__(self, color, parent=None):
        ...
        self.setValidator(
            QRegExpValidator(QRegExp(r"[A-Fa-f0-9]{6}"))
            if qtpy.QT5
            else QRegularExpressionValidator(QRegularExpression(r"[A-Fa-f0-9]{6}"))
        )
...
class ColorDialog(MaskDialogBase):
    ...
    def __onBrightnessChanged(self, color):
        ...
        self.color.setHsv(self.color.hue(), self.color.saturation(), color.value())
