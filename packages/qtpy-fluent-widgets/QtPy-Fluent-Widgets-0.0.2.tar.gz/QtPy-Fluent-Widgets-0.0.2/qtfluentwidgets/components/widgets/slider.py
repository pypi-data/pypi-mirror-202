...
class HollowHandleStyle(QProxyStyle):
    ...
    def subControlRect(self, cc: QStyle.ComplexControl, opt: QStyleOptionSlider, sc: QStyle.SubControl, widget: QSlider):
        ...
        if cc != QStyle.ComplexControl.CC_Slider or widget.orientation() != Qt.Horizontal or sc == QStyle.SubControl.SC_SliderTickmarks:
            return super().subControlRect(cc, opt, sc, widget)

        rect = widget.rect()

        if sc == QStyle.SubControl.SC_SliderGroove:
            h = self.config["groove.height"]
            grooveRect = QRectF(0, (rect.height()-h)//2, rect.width(), h)
            return grooveRect.toRect()

        elif sc == QStyle.SubControl.SC_SliderHandle:
    ...
    def drawComplexControl(self, cc: QStyle.ComplexControl, opt: QStyleOptionSlider, painter: QPainter, widget: QSlider):
        """ draw sub control """
        if cc != QProxyStyle.ComplexControl.CC_Slider or widget.orientation() != Qt.Horizontal:
            return super().drawComplexControl(cc, opt, painter, widget)

        grooveRect = self.subControlRect(cc, opt, QStyle.SubControl.SC_SliderGroove, widget)
        handleRect = self.subControlRect(cc, opt, QStyle.SubControl.SC_SliderHandle, widget)
        ...
        handleColor.setAlpha(255 if opt.activeSubControls !=
                             QStyle.SubControl.SC_SliderHandle else 153)
