...
class PostProcessor(object):
    ...
    def __init__(self, interface_data: dict, interface_info: Interface, data: dict, res: ResData, config: ConfigLoader):
        self.__config = config
        # 期望的实际值和期望值的长度
        self.__length_for_expect = self.__config.get_config("length_for_expect", int)
        # db_info
    ...
    def __work_for_post(self, context: dict, data):
        ...
        ex = "${{{}}}".format(data.if_param)
        ex_res = Express.calculate_str(self.__config, ex, context)
        if not ex_res:
