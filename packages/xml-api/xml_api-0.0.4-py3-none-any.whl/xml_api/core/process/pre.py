...
class PreProcessor(object):
    ...
    def __init__(self, interface_data: dict, interface_info: Interface, data: dict, config: ConfigLoader):
    ...
    def __work_for_pre(self, context: dict, pre):
        ...
        ex = "${{{}}}".format(pre.if_param)
        if not Express.calculate_str(self.__config, ex, context):
            logger.debug("标签未执行，因为其if属性判定为 False")
            return
    ...
    def __process_set(context: dict, names, value, var_type):
        ...
        if len(names) > 1:
            ...
            for i in range(len(names)):
                ...
                logger.debug("标签存储了变量 {} 到局部变量池，其值为 {}".format(names[i], value[i]))
        else:
    ...
    def __process_print(value, desc, var_type):
        ...
        """
        if desc == "打印请求报文":
            res_record.place_data("req_s_data", str(value), "interfaces")
        if not value:
    ...
    def __work_for_interface(self, context: dict, inf):
        ...
        # 处理动作
        if act == "delete":
            self.__delete(context, ty, scope, inf)
        elif act == "replace":
            self.__replace(context, ty, scope, inf)
        elif act == "insert":
            self.__insert(context, ty, scope, inf)
        elif act == "replace or insert":
            self.__replace_or_insert(context, ty, scope, inf)
        else:
            raise InfActIsIncorrectException(act)
        logger.debug("已处理完成")

    def __delete(self, context, ty: str, scope: str, param: InfoData):
        """
        删除参数指定的键,并返回删除后的值
        :param param:
        :return:
    ...
    def __replace_or_insert(self, context: dict, ty: str, scope: str, param: InfoData):
