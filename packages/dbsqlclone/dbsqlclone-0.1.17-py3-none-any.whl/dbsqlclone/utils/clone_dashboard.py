...
def delete_dashboard(client: Client, tags=[], ids_to_skip={}):
    ...
    for d in get_all_dashboards(client, tags):
        if d['id'] not in ids_to_skip:
            ...
            with requests.delete(client.url+"/api/2.0/preview/sql/dashboards/"+d["id"], headers = client.headers) as r:
                r.json()
...
def delete_query(client: Client, q):
    ...
    with requests.delete(client.url+"/api/2.0/preview/sql/queries/"+q["id"], headers = client.headers) as r:
        return r.json()
...
def get_all_item(client: Client, item, tags = []):
    ...
    def get_all_dashboards(dashboards, page):
        with requests.get(client.url+"/api/2.0/preview/sql/"+item, headers = client.headers, params={"page_size": page_size, "page": page}) as r:
            r = r.json()
...
def set_data_source_id_from_endpoint_id(client):
    ...
    with requests.get(client.url+"/api/2.0/preview/sql/data_sources", headers=client.headers) as r:
        data_sources = r.json()
