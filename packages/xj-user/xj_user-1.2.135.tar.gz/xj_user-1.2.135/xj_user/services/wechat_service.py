...
class WechatService:
    ...
    def wechat_login(self, phone_code, login_code, sso_serve_id=None, detail_params=None):
        ...
        if not current_user:
            ...
            user_info = parse_model(user_info_set)

            # 生成登录token
            token = WechatService.set_token(user_info.get('id', None), phone)
            # 用户第一次登录即注册，允许添加用户的详细信息
            try:
                detail_params.setdefault("user_id", user_info.get('id', None))
                detail_params.setdefault("score", "5")  # 用户评分初始化，镖行天下业务逻辑 TODO 后期业务抽离，路程控制
            ...
            # 检验单点登录信息
            sso_set, sso_err = WechatService.sso_verify(sso_serve_id, user_info.get('id', None), sub_appid, openid, unionid)
            if sso_err:
                return None, sso_err
        ...
        # 绑定用户关系 邀请关系和收益关系
        data, relate_err = UserRelateToUserService.bind_bxtx_relate(params=detail_params, user_info=user_info)
        if relate_err:
            logger.error(
        ...
        if not sso:
            if phone:
                ...
                if current_user:
                    ...
                    sso_set, sso_err = WechatService.sso_verify(sso_serve_id, user.get('id', None), sub_appid,
                                                                openid, unionid)
                    if sso_err:
                        return None, sso_err

                    token = WechatService.set_token(user.get('id', None), phone)
                    ...
                    if sso_serve_id:
                        ...
                        if err:
                            return None, err
                    token = WechatService.set_token(base_info.id, phone)
            ...
            token = WechatService.set_token(current_user.get('id', None), current_user.get('phone', None))
            # 修改用户登录信息，绑定token
