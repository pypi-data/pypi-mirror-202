...
class WechatService:
    ...
    def wechat_login(self, phone_code, login_code, sso_serve_id=None, other_params=None):
        """
        过期续约就是重新登录
        :param other_params:
        ...
        if not current_user:
            ...
            try:
                ...
                if group_id:
                    UserGroupService.user_bind_group(user_id=user_info.get('id', None), group_id=group_id)
            except Exception as err:
                write_to_log(prefix="用户手机号登录，绑定部门ID异常", err_obj=err)
