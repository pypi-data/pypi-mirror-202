        ...
        if not sso:
            if phone:
                ...
                if current_user:
                    ...
                    token = WechatService.set_token(user.get('id', None), phone, platform_id)
            ...
            token = WechatService.set_token(current_user.get('id', None), current_user.get('phone', None), platform_id)
            # 修改用户登录信息，绑定token
