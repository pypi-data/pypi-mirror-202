...
class RustBackendGenerated:
    ...
    def get_image_for_occlusion_raw(self, message: bytes) -> bytes:
        return self._run_command(18, 0, message)

    def get_image_for_occlusion(self, path: str) -> anki.image_occlusion_pb2.ImageData:
        message = anki.image_occlusion_pb2.GetImageForOcclusionRequest(path=path)
        raw_bytes = self._run_command(18, 0, message.SerializeToString())
        output = anki.image_occlusion_pb2.ImageData()
        output.ParseFromString(raw_bytes)
        return output


    def add_image_occlusion_note_raw(self, message: bytes) -> bytes:
        return self._run_command(18, 1, message)

    def add_image_occlusion_note(self, *, image_path: str, occlusions: str, header: str, back_extra: str, tags: Sequence[str]) -> anki.collection_pb2.OpChanges:
        message = anki.image_occlusion_pb2.AddImageOcclusionNoteRequest(image_path=image_path, occlusions=occlusions, header=header, back_extra=back_extra, tags=tags)
        raw_bytes = self._run_command(18, 1, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output


    def get_image_cloze_note_raw(self, message: bytes) -> bytes:
        return self._run_command(18, 2, message)

    def get_image_cloze_note(self, note_id: int) -> anki.image_occlusion_pb2.ImageClozeNoteResponse:
        message = anki.image_occlusion_pb2.GetImageOcclusionNoteRequest(note_id=note_id)
        raw_bytes = self._run_command(18, 2, message.SerializeToString())
        output = anki.image_occlusion_pb2.ImageClozeNoteResponse()
        output.ParseFromString(raw_bytes)
        return output


    def update_image_occlusion_note_raw(self, message: bytes) -> bytes:
        return self._run_command(18, 3, message)

    def update_image_occlusion_note(self, *, note_id: int, occlusions: str, header: str, back_extra: str, tags: Sequence[str]) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(18, 3, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output
