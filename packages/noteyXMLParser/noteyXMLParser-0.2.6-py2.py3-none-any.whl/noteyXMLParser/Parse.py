        ...
        for neighbor in self.root.iter('note'):
            if (neighbor.find('chord') != None):
                for pitch in neighbor.iter('pitch'):
                    pitch = str(pitch.find('step').text) + "_" + str(int(pitch.find('octave').text)  -1 )
                ...
                for pitch in neighbor.iter('pitch'):
                    if (neighbor.find('accidental') != None):  
                        # check if key is in sharpMap
                        if str(neighbor.find('accidental').text) in self.sharpMap:
                            pitch = str(pitch.find('step').text) + self.sharpMap[str(neighbor.find('accidental').text)] + "_" + str(int(pitch.find('octave').text) - 1)
                        else:
                            pitch = str(pitch.find('step').text) + str(neighbor.find('accidental').text) + "_" + str(int(pitch.find('octave').text) - 1)
                    else:
                        pitch = str(pitch.find('step').text) + "_" + str(int(pitch.find('octave').text) - 1)
                    duration = int(neighbor.find('duration').text)
                    type = str(neighbor.find('type').text)
