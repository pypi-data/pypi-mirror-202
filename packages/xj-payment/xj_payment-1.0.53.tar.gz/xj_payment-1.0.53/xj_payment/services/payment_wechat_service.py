...
class PaymentWechatService:
    ...
    def payment_logic_processing_v2(param):
        ...
        try:
            ...
            if not payment:
                logger.info(">>>>payment_logic_processing" + "支付记录不存在")
                logging.debug(">>>>payment_logic_processing" + "支付记录不存在")
            ...
            if err_txt:
                print(">>>>payment_logic_processing_err", err_txt)
                logger.info(">>>>payment_logic_processing" + "写入资金模块失败")
                logging.debug(">>>>payment_logic_processing" + "写入资金模块失败")
                raise Networkerror("Bad hostname")
            # 根据唯一交易id 查询主键id
            finance_data, err = FinanceTransactService.finance_transact_detailed(transact_no + "-1")
            if finance_data:
                ...
                if pay_call_back_err:
                    ...
                    logging.debug(">>>>报名修改失败" + pay_call_back_err)
                return None
        except Networkerror as t:
            logger.info("支付回调异常1：", t)
            print("支付回调异常1：", t)
            transaction.savepoint_rollback(sid)
            return t
        except Exception as e:
            logger.info("支付回调异常2：", e)
            print("支付回调异常2：", e)
            transaction.savepoint_rollback(sid)
            logging.debug("payment_logic_processing" + e)
            return e
