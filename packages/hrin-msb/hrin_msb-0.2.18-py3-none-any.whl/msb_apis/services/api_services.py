...
class ApiService(CustomExceptionHandler, metaclass=Singleton):
	...
	def list(self, limit: int = 0, offset: int = 0) -> QuerySet:
		try:
			offset = int(offset if int(offset) > 0 else DEFAULT_QUERY_OFFSET)
			limit = int(limit if int(limit) > 0 else DEFAULT_QUERY_LIMIT) + offset
