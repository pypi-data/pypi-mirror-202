...
def files_diff(src_dir: str, tgt_dir: str) -> pd.DataFrame:
    ...
    Example:
        | ${comparison_results}= | Files Diff | C:/SourceDir | C:/TargetDir |
        | Log | ${comparison_results.to_string()} |
    ...
    for file in common_files:
        try:
            ...
            if filecmp.cmp(src_path, tgt_path, shallow=False):
                ...
                diff = difflib.ndiff(src_file, tgt_file)
                diff_percentage = round(sum([1 for line in diff if line[0] != ' ']) / float(len(src_file)), 2) * 100
