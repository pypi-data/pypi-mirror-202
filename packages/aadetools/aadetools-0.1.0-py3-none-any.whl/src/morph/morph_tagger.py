...
import re
from morph.lemmatizeSentence import lemmatize_sentence
...
def load_ALMA_dic():
    ...
    Returns:
    - dict: A dictionary containing the ALMA data.
...
def tag(token, lang, task):
    """
    Given a token, this method retrives all the possible morphological solutions (lemma, pos, and frequency) filterd by spesific
    language and task.
     
    Args:
        token (str): The Arabic word to be morphologcaly tagged.
    ...
    if token in settings.div_dic.keys():
        ...
        if task =='full' and soluation[-2] == lang:
            return  [token, soluation[3], soluation[2], soluation[2] ,soluation[-2], soluation[-1]]
...
def lemmatize_sentence(text ,lang, task):
   ...
   token = simple_word_tokenize(text)
   # for each word 
   for word in token:
         result_word =[]
         # Trim spaces 
