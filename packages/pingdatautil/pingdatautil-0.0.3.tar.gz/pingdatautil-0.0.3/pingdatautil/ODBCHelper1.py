from . import _LogUtil as logutil
import pyodbc
import sys
import time
...
class ODBCHelper():
    ...
    def log(self, input_str, level=None):
        if not self.logger:
            logutil.log(input_str, ident=self.ident)
        else:
            if self.ident:
                self.logger.log(f"[{self.ident}] {str(input_str)}", level)
            else:
                self.logger.log(str(input_str), level)
