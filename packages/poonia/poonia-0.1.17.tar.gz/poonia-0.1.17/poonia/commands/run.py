...
@click.option('--iterations', '-i', help='number of successful runs', type=int, default=1)
...
def run(command, threads, timeout, iterations):
    ...
    global pool
    finished = 0
    tasks = list((command, timeout, i,)
                 for i in range(iterations + threads - 1))
