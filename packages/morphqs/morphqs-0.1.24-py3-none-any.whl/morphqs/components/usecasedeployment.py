...
def deploy_usecase_tools(branch,integration_id):
    logger = loghandler.get_logger(__name__)
    cl = RequestsApi()
    logger.info("Deploying usecase tools")
    logger.info("Deploying Roles that are needed for the MSP Use Case")
    base_clone_role = urllib.parse.quote("System Admin")
    role_id = cl.get(f"/api/roles?name={base_clone_role}").json()["roles"][0]["id"]
    payload = {"role": {
        "roleType": "user",
        "multitenant": True,
        "multitenantLocked": False,
        "authority": "MultiTenantRole",
