...
@click.option('--feature','-f', show_default=True,default=[DEFAULTS['feature']], multiple=True, help='Feature to install on the produced image. e.g: "cuda"')
@click.option('--user','-u', show_default=True, default=DEFAULTS['user'], help='User environment where installations will take place')
...
def builder(iam_api_key, output_folder, version, region, yes, base_image_name, installation_type, feature, user, cleanup, compute_iam_endpoint):
    if cleanup:
        clean_up(cleanup)
        exit(0)
    ...
    logger.info(color_msg(f"\n\nBase Image Name: {base_config['user_image_name']}"
                          ...
                          f"\nImage Creation Retries: {DEFAULTS['image_create_retries']}"
                          , color=Color.YELLOW))
