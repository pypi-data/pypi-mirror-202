    ...
    def run(self) -> Dict[str, Any]:
        ...
        def _run_remote_script(script):
            ...
            while self.inst_retries:  
                self.inst_retries -= 1          
                stdout_log = get_unique_file_name("installation_log", self.base_config['output_folder'])
                stderr_log = stdout_log + '_error'
                logger.info(color_msg(f"\nInstalling {feature} in newly created VSI"
                                      f"\n- stdout log: '{stdout_log}'\n- stderr log: '{stderr_log}'."
                                      f"\nProcess might take a while.", color=Color.YELLOW))

                client.exec_command(f'chmod 777 {remote_destination}/{script_name}')
                _, stdout, stderr = client.exec_command(f'{remote_destination}/{script_name}') # returns the tuple (stdin,stdout,stderr)
