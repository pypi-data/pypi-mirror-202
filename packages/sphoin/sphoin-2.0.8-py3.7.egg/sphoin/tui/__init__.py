...
"""
	...
	BINDINGS = [
		...
		("up", "height_stretch","Height Stretch"),
		("down", "height_squeeze","Height Squeeze"),
	]
	...
	def action_height_stretch(self) -> None:
		c = 0
		for layout in ["studies","signals"]:
			graphs = self.query(f"#{layout}")
			if len(graphs):
				graphs.first().styles.height = int(graphs.first().styles.height[0]) +1
				c+=1

		graphs = self.query("#line")
		if len(graphs)>0:
			graphs.first().styles.height = int(graphs.first().styles.height[0]) - c

	def action_height_squeeze(self) -> None:
		c = 0
		for layout in ["studies","signals"]:
			graphs = self.query(f"#{layout}")
			if len(graphs)>0:
