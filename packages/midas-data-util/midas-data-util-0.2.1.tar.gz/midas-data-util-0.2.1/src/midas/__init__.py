...
# def test():
# 	print("read csv")
# 	df = pd.read_csv("test/in/test.csv")

# 	print("loading")
# 	events, sessions, users = load(
# 		df,
# 		sessions_must_include_exit_and_enter_events=True,
# 	)
...
# 	event_df = DataFrame(dump(events))
# 	event_df.to_json("test/out/events.json", indent=4, orient="records")

# 	print("constructing session table")
# 	session_df = DataFrame(dump(sessions))
# 	session_df.to_json("test/out/sessions.json", indent=4, orient="records")

# 	print("constructing user table")
# 	session_df = DataFrame(dump(users))
# 	session_df.to_json("test/out/users.json", indent=4, orient="records")
