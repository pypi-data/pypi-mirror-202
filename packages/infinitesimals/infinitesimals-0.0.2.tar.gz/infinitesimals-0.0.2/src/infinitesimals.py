...
def repr_polynomial(coefficients, variable, mode):
    ...
    for i, v in enumerate(coefficients):
        ...
        s += (str(abs(v)) if i == 0 or v != 1 else '') + ('*' if i > 0 and p else '')
        s += enclose(str(variable) + (superscript_int(i) if not p else f'**{i}'), p) if i else ''
        c += 1
...
class HyperReal(tuple):
    ...
    def __mul__(self, other):
        return HyperReal(*(x for x in convolve(self, other)).tolist())
    ...
    def __truediv__(self, other):
        try:
            ...
            if isinstance(other, HyperReal):
                ...
                while not (_self.st and _other.st):
                    _self <<= 1
                    _other <<= 1
    ...
    def from_matrix(matrix):
        ...
        return HyperReal(*(x for x in array(matrix[0, :]).flatten()))
