    ...
    if "overrides" in config:
        ...
        if "roleGroupAccessOverride" in config['overrides']:
            ...
            for item in config['overrides']['roleGroupAccessOverride']:
                ...
                resp = cl.post(f'/api/roles/{roleId}/update-group', data = json.dumps(payload)).json()
                if resp['success'] is True:
                    logger.info(f"Successfully created role {item['name']}")
                else:
                    logger.error(f"Failed to create role {item['name']}")
                    logger.error(f"Error: {resp}")
                    sys.exit(1)
