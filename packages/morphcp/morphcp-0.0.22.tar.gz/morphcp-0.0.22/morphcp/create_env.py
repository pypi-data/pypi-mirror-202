...
def process_content_pack_higher_ed(config: dict):
    ...
    if "overrides" in config:
        ...
        if "roleGroupAccessOverride" in config['overrides']:
            ...
            for item in config['overrides']['roleGroupAccessOverride']:
                ...
                # try: 
                # need to add if it detects a group that does not exist. 
                ...
                #     logger.error(f"Failed to update role {item['rolename']}")
    injector.inject_optionlist()
    injector.inject_inputs()
    injector.inject_tasks()
    ...
    if "overrides" in config:
        logger.info(f"Overrides Found!!")
        logger.debug(config)
        if "whitelabel" in config["overrides"]:
            logger.info(f"Whitelabel Overrides Found!!")
            payload = {"whitelabelSettings": {
            "headerBgColor": config["overrides"]["whitelabel"]["headerBgColor"],
            "headerFgColor": config["overrides"]["whitelabel"]["headerFgColor"],
            "footerBgColor": config["overrides"]["whitelabel"]["footerBgColor"],
            "footerFgColor": config["overrides"]["whitelabel"]["footerFgColor"],
            "loginBgColor": config["overrides"]["whitelabel"]["loginBgColor"],
            }}
            resp = cl.put(f'/api/whitelabel-settings', data = json.dumps(payload)).json()
            if resp['success'] is True:
                logger.info(f"Successfully updated whitelabel settings")
            else:
                logger.error(f"Failed to update whitelabel settings")
                logger.error(f"Error: {resp}")
