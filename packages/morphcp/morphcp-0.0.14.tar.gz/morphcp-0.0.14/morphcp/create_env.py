...
def process_content_pack_higher_ed(config: dict):
    ...
    #         f.close()
    #         for item in ovrconfig["schools"]:
    #             if "overrides" in item:
    #                 logger.info(f"Overrides Found!!")
    #                 if "groups" in item["overrides"]:
    #                     logger.info(f"Group Overrides Found!!")
    #                     for group in item["overrides"]["groups"]:
    #                         # group_name = urllib.parse.quote(group['name'])
    #                         # resp = cl.get(f"/api/groups?name={group_name}").json()["groups"][0]["id"]
    #                         # if resp is not None:
    #                         #     group['id'] = resp['groups'][0]['id']
    #                         json_paylod = {'group': {'name': group['name'], "code": group['code'], "location": group['location']}}
    #                         logger.info("Attempting to Create the group")
    #                         resp = cl.put(f"/api/groups/{group['id']}", data = json.dumps(json_paylod)).json()
    #                         if resp['success'] is True:
    #                             logger.info(f"Successfully updated group {group['name']}")
    #                         elif resp['success'] is False:
    #                             logger.error(f"Failed to update group {group['name']}")
