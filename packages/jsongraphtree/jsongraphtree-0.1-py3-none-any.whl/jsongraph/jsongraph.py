...
def add_nodes(graph, data, dict_alias,parent_node=None, label=None):
    """
    添加节点和边的递归函数
    """
    ...
    if isinstance(data, dict):
        for key,value in data.items():
            ...
            if parent_node is None:
                  graph.node(node_id,label=node_label)
                  add_nodes(graph,value,dict_alias,node_id)
            if parent_node is not None:
                    node_id = str(id(key))
                    # print("key:"+key+" "+"parent_node:"+parent_node)
                    # print("node_id: "+node_id)
...
def main():
    ...
    for key, value in data.items():
        # print("开始的key："+key)  
        if key=='pipeline':
            add_nodes(graph,value,dict_alias)
    # 添加节点和边

    # add_nodes(graph, data)

    # 将图形保存为图像文件
    graph.render('json_tree_test', format='png')
