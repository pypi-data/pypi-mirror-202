...
@click.option(
    "--auth-mode",
    help="Authentication Mode. We support service principal and master user two auth types",
    required=True,
    type=click.Choice(["SERVICE_PRINCIPAL", "MASTER_USER"]),
)
@click.option(
    "--client-id",
...
@click.option(
    "--username",
    help="Username for accessing the Power BI. Set only when auth-mode is MASTER_USER.",
    cls=AdvancedOptions,
    required=False,
...
@click.option(
    "--password",
    help="Password for accessing the Power BI. Set only when auth-mode is MASTER_USER.",
    cls=AdvancedOptions,
    required=False,
    mutually_exclusive_options=["client-secret"],
    prompt_if_requested=True,
...
@click.option(
    "--webhook",
    help="Use a dbt Cloud webhook for near real time collection of run results",
    is_flag=True,
    default=False,  # TODO: hopefully we can change the default to True in the not too distant future
    show_default=True,
)
@add_common_options(warehouse_select_option())
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
def add_dbt_cloud(ctx, name, webhook, **kwargs):
    """
    Onboard a dbt cloud connection
    """
    DbtCloudOnboardingService(config=ctx["config"]).onboard_dbt_cloud(
        warehouseName=name, webhook=webhook, **kwargs
    )
