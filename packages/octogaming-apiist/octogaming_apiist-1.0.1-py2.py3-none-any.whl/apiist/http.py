...
class HTTP(object):
    ...
    def get(self, address, **kwargs):
        return self.request("GET", address, **kwargs)

    def post(self, address, **kwargs):
        return self.request("POST", address, **kwargs)

    def put(self, address, **kwargs):
        return self.request("PUT", address, **kwargs)

    def delete(self, address, **kwargs):
        return self.request("DELETE", address, **kwargs)

    def patch(self, address, **kwargs):
        return self.request("PATCH", address, **kwargs)

    def head(self, address, **kwargs):
        return self.request("HEAD", address, **kwargs)

    def options(self, address, **kwargs):
        return self.request("OPTIONS", address, **kwargs)
...
class smart_transaction(transaction_logged):
    def __init__(self, name):
        ...
        self.driver, self.func_mode, self.controller = get_from_thread_store(
            ("driver", "func_mode", "controller")
        )
...
class RequestFailure(Request):
    def __init__(self, method, address, request, exc, session):
        ...
        super(RequestFailure, self).__init__(
            method, address, request, HTTPResponse(response), session
        )
        self.method = method
        self.address = address
        self.request = request
...
class AssertionFailure(Event):
    ...
    def __repr__(self):
        return "Assertion(name=%r, failure_message=%r)" % (
            self.name,
...
class _EventRecorder(object):
    ...
    def record_transaction_start(self, tran):
        ...
        if isinstance(tran, transaction_logged):
            self.log.info(
                "Transaction started:: start_time=%.3f,name=%s",
                tran.start_time(),
                tran.name,
            )
...
class HTTPTarget(object):
    def __init__(
        ...
        self.__request = http_client or requests
        self.__http = http_instance or http
...
class HTTPResponse(object):
    ...
    def assert_ok(self, msg=None):
        if self.status_code >= 400:
            msg = msg or "Request to %s didn't succeed (%s)" % (
                self.url,
                self.status_code,
            )
    ...
    def assert_status_code(self, code, msg=None):
        ...
        expected = str(code)
        if actual != expected:
            msg = msg or "Actual status code (%s) didn't match expected (%s)" % (
                actual,
                expected,
            )
            raise AssertionError(msg)
    ...
    def assert_has_header(self, header, msg=None):
        if header not in self.headers:
            msg = msg or "Header %s wasn't found in response headers: %r" % (
                header,
                self.headers,
            )
            raise AssertionError(msg)
    ...
    def assert_header_value(self, header, value, msg=None):
        ...
        if actual != value:
            msg = msg or "Actual header value (%r) isn't equal to expected (%r)" % (
                actual,
                value,
            )
            raise AssertionError(msg)
    ...
    def assert_in_headers(self, member, msg=None):
        ...
        if member not in headers_text:
            msg = msg or "Header %s wasn't found in response headers text: %r" % (
                member,
                headers_text,
            )
            raise AssertionError(msg)
    ...
    def assert_xpath(self, xpath_query, parser_type="html", validate=False, msg=None):
        parser = (
            etree.HTMLParser()
            if parser_type == "html"
            else etree.XMLParser(dtd_validation=validate)
        )
        tree = etree.parse(BytesIO(self.content), parser)
        matches = tree.xpath(xpath_query)
        if not matches:
            msg = msg or "XPath query %r didn't match response content: %s" % (
                xpath_query,
                self.text[:BODY_LIMIT],
            )
            raise AssertionError(msg)
    ...
    def assert_not_xpath(self, xpath_query, parser_type="html", validate=False, msg=None):
        ...
        matches = tree.xpath(xpath_query)
        if matches:
            msg = msg or "XPath query %r did match response content: %s" % (
                xpath_query,
                self.text[:BODY_LIMIT],
            )
            raise AssertionError(msg)
    ...
    def assert_cssselect(self, query, expected_value=None, attribute=None, msg=None):
        ...
        if not matches:
            msg = msg or "CSSSelect query %r didn't match response content: %s" % (
                query,
                self.text[:BODY_LIMIT],
            )
            raise AssertionError(msg)
    ...
    def assert_not_cssselect(self, query, expected_value=None, attribute=None, msg=None):
        ...
        msg = msg or "CSSSelect query %r did match response content: %s" % (
            query,
            self.text[:BODY_LIMIT],
        )
        raise AssertionError(msg)
