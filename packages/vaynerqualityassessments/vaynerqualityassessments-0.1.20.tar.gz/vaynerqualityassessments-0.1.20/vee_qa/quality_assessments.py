...
qual_logger = utility_functions.Logger('Indeed','QualityAssessments')
...
class QualityAssessments:
    ...
    def boosted_function_qa(self,paid_df, organic_df, gsheet_name,tab_name='OrganicWithBigImpressions',impressions_threshold = 100000):
        ...
        if (missing_boosted) != 0:
            ...
            qual_logger.logger.warning(error_message)
    ...
    def comparison_with_previous_data(self,df,name_of_df,cols_to_check=['impressions','likes'],perc_increase_threshold=20,
                                   perc_decrease_threshold=0.5, check_cols_set=True,raise_exceptions=False, date_col=None):
        ...
        new_dict['num_rows'] = df.shape[0]
        # if date_col != None:
        #     new_dict['max_date'] = df[date_col].max()
        #     new_dict['min_date'] = df[date_col].min()
        ...
        if os.path.exists(f'Historic df Comparison (Do Not Delete)/{name_of_df}_previous_totals.json') ==False:
            ...
            qual_logger.logger.info(f"Creation of {name_of_df}_previous_totals")
            qual_logger.logger.info(new_dict)
            return
        else:
        ...
        for key, value in old_dict.items():
            if key == 'Columns':
                ...
                if new_dict[key] < value:
                    error_occured = True
                    error_message = error_message + '  ' + f'The number of rows has decreased from last time\n'\
                                            f' Prev Value = {old_dict[key]} , New Value = {new_dict[key]}\n'
            elif new_dict[key] *(1+perc_decrease_threshold/100) < value:
                error_occured = True
                error_message = error_message + '  ' + f'The total of {key} seems to have decreased by more than {perc_decrease_threshold} from last time\n'\
                                        f' Prev Value = {old_dict[key]} , New Value = {new_dict[key]}\n'
        ...
        if error_occured:
            ...
            qual_logger.logger.info('ERROR - ' + error_message) #if error messages has been added to then log it
    ...
    def duplicates_qa(self, df: pd.DataFrame, df_name: str,subset= None, drop_duplicates: bool = True, cols_to_check = None):
        ...
        if num_duplicates >= 0:
            qual_logger.logger.warning(f'{num_duplicates} duplicates found in the {df_name}. Subset = {subset}')
            if drop_duplicates:
