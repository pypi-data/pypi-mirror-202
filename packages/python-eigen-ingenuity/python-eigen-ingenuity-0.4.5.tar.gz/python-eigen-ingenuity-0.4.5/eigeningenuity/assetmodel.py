...
class AssetModel (object):
    ...
    def getRelatedDocuments(self, node:Union[str,list], match=None, output="json", directory:str=None):
        ...
        if output == "download":
            ...
            for doc in documents:
                with requests.get(doc["url"], stream=True,timeout=1000) as r:
                    r.raise_for_status()
    ...
    def getLabels(self):
        """
        List all labels used in an assetmodel instance

        Args:
            N/A
        Returns:
            A list of all node labels/types present in the model
        """
...
def get_assetmodel(eigenserver:EigenServer=None):
    ...
    return AssetModel(eigenserver.getEigenServerUrl() + "ei-applet" + "/")
