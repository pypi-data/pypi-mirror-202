...
from t2iadapter.ldm.models.diffusion.ddpm import LatentDiffusion, disabled_train
from t2iadapter.ldm.util import instantiate_from_config
...
class T2IAdapterStyleV3(LatentDiffusion):
    ...
    def __init__(self, adapter_config, extra_cond_key, noise_schedule, *args, **kwargs):
        ...
        self.clip_vision_model = CLIPVisionModel.from_pretrained('/root/.cache/t2iadapter/openai/clip-vit-large-patch14')
    ...
    def shared_step(self, batch, **kwargs):
        ...
        if self.extra_cond_key in self.ucg_training:
            idx = np.random.choice(
                self.adapter.num_token, np.random.randint(1, self.adapter.num_token + 1), replace=False)
