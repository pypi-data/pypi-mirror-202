...
from t2iadapter.ldm.models.diffusion.plms import PLMSSampler
from t2iadapter.ldm.modules.encoders.adapter import Adapter, StyleAdapter, Adapter_light
from t2iadapter.ldm.modules.extra_condition.api import ExtraCondition
from t2iadapter.ldm.util import fix_cond_shapes, load_model_from_config, read_state_dict
...
def get_adapters(opt, cond_type: ExtraCondition):
    ...
    if cond_type == ExtraCondition.style:
        ...
        adapter['model'] = Adapter_light(
            cin=64 * get_cond_ch(cond_type), channels=[320, 640, 1280, 1280], nums_rb=4).to(opt.device)
    else:
