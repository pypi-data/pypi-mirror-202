...
from t2iadapter.ldm.util import log_txt_as_img, exists, default, ismap, isimage, mean_flat, count_params, instantiate_from_config
from t2iadapter.ldm.modules.ema import LitEma
from t2iadapter.ldm.modules.distributions.distributions import normal_kl, DiagonalGaussianDistribution
from t2iadapter.ldm.models.autoencoder import IdentityFirstStage, AutoencoderKL
from t2iadapter.ldm.modules.diffusionmodules.util import make_beta_schedule, extract_into_tensor, noise_like
...
class LatentDiffusion(DDPM):
    ...
    def progressive_denoising(self,
        ...
        iterator = tqdm(
            reversed(range(0, timesteps)), desc='Progressive Generation', total=timesteps) if verbose else reversed(
                range(0, timesteps))
    ...
    def p_sample_loop(self,
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)
        iterator = tqdm(
            reversed(range(0, timesteps)), desc='Sampling t', total=timesteps) if verbose else reversed(
                range(0, timesteps))
