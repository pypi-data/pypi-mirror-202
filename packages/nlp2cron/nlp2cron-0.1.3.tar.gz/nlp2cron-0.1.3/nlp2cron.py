...
class NLPCron:
    ...
    def a2ora1(self):
        ## 词汇中没有 每 什么，但有定时或者提醒二字
        test = self.a2 and not self.a1
        if not test: return

        self.freq = 'F' ## 意味着不重复执行，只执行一次

        today = re.findall(r'(今)(天|日)', self.message)

        if today: 
            self.dict['day'] = str(self.now.day)
            return 

        tomorrow = re.findall(r'(明)|(天|日)', self.message)

        if tomorrow: 
            self.dict['day'] = str((self.now + timedelta(days=1)).day)
            return

        after_tomorrow = re.findall(r'(后)|(天|日)', self.message)

        if after_tomorrow: 
            self.dict['day'] = str((self.now + timedelta(days=2)).day)
            return

        aafter_tomorrow = re.findall(r'(大后)|(天|日)', self.message)

        if aafter_tomorrow: 
            self.dict['day'] = str((self.now + timedelta(days=3)).day)
            return

        weekday = re.findall(r'下周(\d)', self.message)
        if '下周' in self.message and not weekday: 
            wd = self.now.weekday() ## 今天是周几，算出来
            # 今天到星期天还有几天
            last_days = 6 - wd 
        ...
        if '下个月' in self.message:
            tmp = re.findall(r'下个月(\d{1,2})', self.message)
    ...
    def extract_freq(self):
        ...
        if '点' in self.message:
                ...
                else: self.dict['hour'] = '12'

        if '分' in self.message:
                min = re.findall(r'(\d+)分', self.message)
                if min: self.dict['min'] = str(min[0])
                else: self.dict['min'] = '0'


        if ':' in self.message:
            s = re.findall(r'(\d+)\:(\d+)', self.message)
    ...
    def check(self, min, hour, day, month, week):
        ...
        if (hour != '*' and int(hour) > 23): return None

        if (day not in ['*', 'L']) and (month == '*' or month == '2'):
            day = int(day)
            if self.is_leap_year(self.now.year) and day > 28: return None
...
if __name__ == '__main__':
    tasks = [
                '明天要去医院看病，要带上病历和身份证。',
                '下周二是最后一天交作业，要抓紧时间写作。',
                '今天晚上要去参加公司晚宴，别忘了准备礼服和名片。',
                '明天是国庆节，要提前准备好旅游计划和行程安排。',
                '今天晚上要去机场接人，别忘了带上车钥匙和行李架。',
                '明天是父亲的生日，要准备份礼物。',
                '下周一要提交报告，要抓紧时间写作。',
                '今天晚上要去参加朋友的生日聚会，别忘了带礼物。',
                '明天要去旅游，要提前准备好行李和路线规划。',
                '今天晚上要去健身房锻炼，别忘了带运动鞋和水杯。',
                '明天要去超市购物，要带上购物清单和购物袋。',
                '下周五要参加面试，要准备好简历和面试资料。',
                '今天晚上要去参加音乐会，别忘了带上门票和身份证。',
                '明天要去旅游，要提前准备好护照和行李箱。',
                '今天晚上要去参加生日派对，别忘了准备礼物和祝福语。',
                '每天晚上8点发送课程业绩给张明',
                '后天晚上10点同学聚会',
                '每月末10点叫我去上班',
                '下周5, 10点叫我去公园遛娃',
