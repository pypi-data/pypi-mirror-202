...
def check_type(item, item_type):
    ...
    if not isinstance(item, item_type):
        if item:
            logger.warning("%r is not a %s - setting to an empty %s", item, item_type.__name__, item_type.__name__)
        return item_type()
    return item
        ...
        for party in all_parties:
            ...
            for unique_party in unique_parties:
                ...
                if party["party_fingerprint"] == unique_party["party_fingerprint"]:
                    ...
                    break
            if found_party:
                found_party["party"]["roles"] = list(
                    set(
                        check_type(found_party["party"].get("roles"), list)
                        + check_type(party["party"].get("roles"), list)
                    )
                )
            else:
...
def contract_status(state):
    ...
    for compiled_release, contracting_process in zip(
        ...
        if any(
            cast_string(period.get("startDate"))
...
def budget(state):
    ...
    if len(state.compiled_releases) == 1:
        ...
        if len(budget_currencies) > 1:
            logger.warning(
                "Can't get budget total, %s different currencies found.",
                len(budget_currencies),
            )
        else:
            state.output["budget"] = {
                "amount": {
                    "amount": sum(budget_amounts),
    ...
    for compiled_release in state.compiled_releases:
        ...
        if project_description:
            if output_description and output_description != project_description:
                logger.warning(
                    "Multiple differing planning/project/description found e.g. %s, %s, skipping transform",
                    project_description,
                    output_description,
                )
                return
