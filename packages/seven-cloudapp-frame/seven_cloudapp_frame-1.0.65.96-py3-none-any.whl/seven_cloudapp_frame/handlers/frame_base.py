...
@LastEditTime: 2023-04-12 19:08:58
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def get_param_int(self, param_name, default=0, strip=True, filter_sql=False, filter_special_key=False, contain_list=[], range_list=[]):
        """
        ...
        :param filter_sql: 是否过滤sql关键字
        :param filter_special_key: 是否过滤sql特殊字符
        :param contain_list: 包含列表
        :param range_list: 区间列表
        :param filter_special_key: 是否过滤sql特殊字符
        :return: 转换后的参数值
        :last_editors: HuangJianYi
        """
        ...
        if status == False:
            self.param_error_list.append({"param": param_name, "type": "int", "info": "参数格式错误"})
        else:
            if len(contain_list) > 0 and not param in contain_list:
                self.param_error_list.append({"param": param_name, "type": "int", "info": "参数不在包含范围内"})
            if len(range_list) == 2 and (param < range_list[0] or param > range_list[1]):
                self.param_error_list.append({"param": param_name, "type": "int", "info": "参数不在区间范围内"})
        return param

    def get_param_decimal(self, param_name, default=0, strip=True, filter_sql=False, filter_special_key=False):
        """
        ...
        if status == False:
            self.param_error_list.append({"param": param_name, "type": "decimal", "info": "参数格式错误"})
        return param

    def get_param_datetime(self, param_name, default='1900-01-01 00:00:00', strip=True, filter_sql=False, filter_special_key=False, fmt='%Y-%m-%d %H:%M:%S'):
        """
        ...
        if status == False:
            self.param_error_list.append({"param": param_name, "type": "datetime", "info": "参数格式错误"})
        return param
