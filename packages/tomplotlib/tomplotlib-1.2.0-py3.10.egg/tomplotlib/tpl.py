    ...
    if isinstance(colorscheme, int):
        ...
        if colorscheme in ["viridis", "plasma", "inferno", "magma", "cividis"]:
            ...
            if divisions is None:
                print(
                    "Colorscheme NOT set. For this continuous colormap please specify a number of divisions with argument `divisions=...`"
                )
                return
            else:
                d = divisions
                colorlist = [
                    colorscheme_.__call__(f)
                    for f in np.linspace(0 + 1 / (2 * d), 1 - (1 / (2 * d)), d)
                ]
                alpha = 0.3
                colorlist_1 = [
                    tuple(
                        alpha * np.array(color[:3]) + (1 - alpha) * np.array([1, 1, 1])
                    )
