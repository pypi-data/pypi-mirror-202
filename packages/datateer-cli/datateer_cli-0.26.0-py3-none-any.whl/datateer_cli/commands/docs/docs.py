...
def run_dbt_docs_command(target: str) -> None:
    """
...
def run_aws_s3_sync_command(client_code: str) -> None:
    ...
    """
    command = f"aws s3 cp dbt/target/ s3://dbt-docs-666090538019/{client_code} --recursive --profile AdministratorAccess-666090538019".split()
    try:
        process = subprocess.Popen(
            command, stderr=subprocess.STDOUT, stdout=subprocess.PIPE
        )
