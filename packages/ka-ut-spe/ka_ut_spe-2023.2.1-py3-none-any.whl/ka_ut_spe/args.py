...
from ka_ut_spe.str import Str as UtsStr
from ka_ut_obj.str import Str as UtoStr
        ...
        class SysArgv:
            ...
            def sh(key, value):
                if key.startswith('sw_'):
                    return UtoStr.sh_boolean(value)
                elif key.startswith('d_'):
                    return UtsStr.sh_dic(value)
                elif key.startswith('a_'):
                    return UtsStr.sh_arr(value)
                elif key.startswith('aoa_'):
                    return UtoStr.sh_aoa(value)
                elif key.startswith('f_'):
                    return value
                else:
                    if UtoStr.is_integer(value):
                        return int(value)
                    else:
                        return value
