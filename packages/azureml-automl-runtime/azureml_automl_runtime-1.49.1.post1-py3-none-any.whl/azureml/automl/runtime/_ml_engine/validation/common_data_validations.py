...
def check_data_nan_inf(data: DataInputType, input_data_name: str, check_nan: bool,
                       check_inf: bool = True, exception_for_nan: bool = False) -> None:
    """
    ...
    :param exception_for_nan: raise an exception rather than a warning for Nans
    :return:
    """
    try:
        if check_nan:
            _check_nan_in_data(data, input_data_name, exception_for_nan)
...
def _check_nan_in_data(data: DataInputType, input_data_name: str, raise_exception: bool) -> None:
    """
    Raise an error or warning if the data contains nan

    :param data: data to check
    :param input_data_name: name of the data
    :param raise_exception: raise an exception rather than a warning
    :return:
    """
    if _is_nan_in_data(data):
        if raise_exception:
            raise DataException._with_error(
                AzureMLError.create(DatasetContainsNan, target=input_data_name, data_object_name=input_data_name),
            )
        else:
            msg = (
...
def check_dimensions(
    ...
    try:
        ...
        MaterializedTabularData(X, y, sample_weight, reduce_mem=False)
        if X_valid is not None and y_valid is not None:
            MaterializedTabularData(X_valid, y_valid, sample_weight_valid, reduce_mem=False)
    except Exception as e:
