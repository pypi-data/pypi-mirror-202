...
from azureml.automl.core.shared.constants import AutoMLJson, TimeSeriesInternal
...
class VerifiedFaultsTypes:
    ...
    TIMESERIES_ID_DETECTION_UTILITIES = "_timeseries_id_detection_utilities"
    TIMESERIES_NON_STATIONARY_HANDLING = "_timeseries_non_stationary_detection_and_handling"
    INSUFFICIENT_DATA_FORECAST_TCN = "_insufficient_data_forecast_tcn"
    TIMESERIES_UNIQUE_TARGET_GRAINS = "_timeseries_unique_target_grains"
    TIMESERIES_AUTO_CV = "_timeseries_automatic_cross_validation"
...
class ValidParameterKeys:
    ...
    ROW_COUNTS_AFTER_PREPROCESSING = "row_counts_after_preprocessing"
    ...
    SERIES_DROPPED_NAMES = "series_dropped_names"
    SERIES_PADDED_NAMES = "series_padded_names"
    SERIES_FREQ = "series_freq"
    TIME_SERIES_AGG_FUN = "aggregation_function"
    NUM_OF_DUPLICATE_TIME_INDEX = "number_of_duplicate_time_index"
    DETECTED_TIME_SERIES_ID_COLUMN_NAMES = "detected_time_series_id_column_names"
    UNIQUE_TARGET_SERIES = "unique_target_series"
...
class VerifierManager:
    ...
    def update_data_verifier_for_high_cardinal_features(
            self, stats_and_column_purpose: Optional[List[StatsAndColumnPurposeType]]
    ) -> None:
        """
        ...
        for col in stats_and_column_purpose:
            ...
            if column_purpose == _FeatureType.Hashes:
                ...
                self.add_data_fault_parameter(
                    VerifiedFaultsTypes.DETECT_HIGH_CARDINAL_FEATURES, {
                        ValidParameterKeys.COLUMN_NAME: str(col[2]),
                        ValidParameterKeys.CONTENT_TYPE: ValidParameterValues.HASHES
                    }
                )
    ...
    def update_data_verifier_insufficient_data_forecast_tcn(
            self,
            number_of_data_points: int,
            number_of_data_points_after_preprocessing: int,
            is_multi_grain: bool,
            enable_dnn: bool) -> None:
        """
        Detect if the number of data points is sufficient for
        ForecastTCN to be suggested as a valid model

        :param number_of_data_points before preprocessing: int
        :param number_of_data_points_after_preprocessing: int
        :param is_multi_grain: True if multiple time series columns are passed, otherwise False
        :param enable_dnn: if True, guardrail will be skipped
        """
        guardrail_passed = False
        if not enable_dnn:
            return
        else:
            tcn_minimal_points = TimeSeriesInternal.TCN_MULTI_SERIES_MIN_POINTS if is_multi_grain \
                else TimeSeriesInternal.TCN_SINGLE_SERIES_MIN_POINTS
            if number_of_data_points_after_preprocessing >= tcn_minimal_points:
                guardrail_passed = True
        if guardrail_passed:
            self.add_data_fault(VerifiedFaultsTypes.INSUFFICIENT_DATA_FORECAST_TCN, VerifierResults.PASSED)
        else:
            self.add_data_fault(VerifiedFaultsTypes.INSUFFICIENT_DATA_FORECAST_TCN, VerifierResults.ALERTED)
        self.add_data_fault_parameter(
            VerifiedFaultsTypes.INSUFFICIENT_DATA_FORECAST_TCN, {
                ValidParameterKeys.ROW_COUNTS: str(number_of_data_points),
                ValidParameterKeys.ROW_COUNTS_AFTER_PREPROCESSING: str(number_of_data_points_after_preprocessing)})
    ...
    def update_data_verifier_data_non_stationary(self, is_non_stationary: bool) -> None:
        """
        Detect if the data is non-stationary and if it is processed to make non-stationary data stationary.
        :param is_non_stationary: If True, guard rails needs to be printed.
        """

        result = VerifierResults.FIXED if is_non_stationary else VerifierResults.PASSED
        self.add_data_fault(VerifiedFaultsTypes.TIMESERIES_NON_STATIONARY_HANDLING, result)

    def update_data_verifier_time_series_id_detection_handling(self,
                                                               time_series_id_column_names: List[str],
                                                               duplicate_num: int,
                                                               is_time_series_id_column_names_detected: bool) -> None:
        """
        Detect if the data set has time series identifiers.

        :param time_series_id_column_names:
            List of the time_series_id_column_names. It is either user defined or automatically detected.
        :param duplicate_num:
            Number of duplicate time index rows in the data.
        :param is_time_series_id_column_names_detected:
            True if time_series_id_column_names is detected automatically in dataset.
        """
        if is_time_series_id_column_names_detected:
            self.add_data_fault(VerifiedFaultsTypes.TIMESERIES_ID_DETECTION_UTILITIES, VerifierResults.FIXED)
            self.add_data_fault_parameter(
                VerifiedFaultsTypes.TIMESERIES_ID_DETECTION_UTILITIES,
                {ValidParameterKeys.NUM_OF_DUPLICATE_TIME_INDEX: str(duplicate_num),
                 ValidParameterKeys.DETECTED_TIME_SERIES_ID_COLUMN_NAMES: ', '.join(time_series_id_column_names)})

        # In this case, there is no duplicate time index in dataset, w/o user defined time series id column names.
        elif duplicate_num == 0:
            self.add_data_fault(VerifiedFaultsTypes.TIMESERIES_ID_DETECTION_UTILITIES, VerifierResults.PASSED)

    def update_data_verifier_short_grain_handling(self, padded: List[GrainType],
                                                  dropped: List[GrainType]) -> None:
        """
        Detect if the data set contains the short grains and was corrected.

        :param padded: The list of short series fixed by padding.
        :param dropped: The list of short series dropped.
        :param grain_names: The name of processed grains.
        """

        if not padded and not dropped:
            ...
            if dropped:
                self.add_data_fault_parameter(
                    ...
                    self._get_short_grain_dict(ValidParameterKeys.SERIES_DROPPED,
                                               ValidParameterKeys.SERIES_DROPPED_NAMES, dropped))
            if padded:
                self.add_data_fault_parameter(
                    VerifiedFaultsTypes.TIMESERIES_SHORT_SERIES_HANDLING,
                    self._get_short_grain_dict(ValidParameterKeys.SERIES_PADDED,
                                               ValidParameterKeys.SERIES_PADDED_NAMES, padded))

    def update_data_verifier_forecasting_auto_cv(self, num_of_folds: Optional[int],
                                                 cv_step_size: Optional[int]) -> None:
        """
        Update VerifierManager with number of folds and cv step size if set by forecasting auto-cv.

        :param number_of_folds: Number of folds in cross validation.
        :param cv_step_size: Number of periods of stagger between two consecutive cross-validation folds.
        :return: None
        """
        self.add_data_fault(
            VerifiedFaultsTypes.TIMESERIES_AUTO_CV,
            result=VerifierResults.DONE)
        if num_of_folds is not None and cv_step_size is not None:
            self.add_data_fault_parameter(
                VerifiedFaultsTypes.TIMESERIES_AUTO_CV,
                {ValidParameterKeys.NUMBER_OF_FOLDS: str(num_of_folds),
                 ValidParameterKeys.CV_STEP_SIZE: str(cv_step_size)})
        elif num_of_folds is not None and cv_step_size is None:
            self.add_data_fault_parameter(
                VerifiedFaultsTypes.TIMESERIES_AUTO_CV,
                {ValidParameterKeys.NUMBER_OF_FOLDS: str(num_of_folds)})
        elif num_of_folds is None and cv_step_size is not None:
            self.add_data_fault_parameter(
                VerifiedFaultsTypes.TIMESERIES_AUTO_CV,
                {ValidParameterKeys.CV_STEP_SIZE: str(cv_step_size)})

    def _get_short_grain_dict(self,
                              grain_number_key: str, grain_names_key: str,
                              grain_names: List[GrainType]
                              ) -> Dict[str, str]:
        """
        Populate the dictionary with parameters for short grains.

        :param grain_number_key: The key in the fault parameters dictionary for the number of processed grains.
        :param grain_name_key: The key in the fault parameters dictionary for the names of processed grains.
        :param grain_names: The series, which were dropped or padded.
        :return: The populated dictionary with parameters.
        """
        return {
            grain_number_key: str(len(grain_names)),
            grain_names_key: ', '.join(list(map(str, grain_names)))
        }

    def update_data_verifier_for_unique_target_grains(
            self,
            unique_target_grains: Optional[List[GrainType]] = None,
            is_all_grains_unique_target: bool = False
    ) -> None:
        """
        Update data verifier with unique target grains.

        :param unique_target_grains: The grains that has unique target values.
        :param is_all_grains_unique_target: If all grains are unique.
        """
        if is_all_grains_unique_target:
            self.add_data_fault(
                VerifiedFaultsTypes.TIMESERIES_UNIQUE_TARGET_GRAINS, VerifierResults.FIXED)
        elif unique_target_grains:
            self.add_data_fault(
                VerifiedFaultsTypes.TIMESERIES_UNIQUE_TARGET_GRAINS, VerifierResults.ALERTED,
                [{ValidParameterKeys.UNIQUE_TARGET_SERIES: str(unique_target_grains)}]
            )
