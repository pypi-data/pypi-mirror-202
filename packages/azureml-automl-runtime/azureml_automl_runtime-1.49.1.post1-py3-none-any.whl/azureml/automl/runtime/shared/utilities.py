...
def _get_num_unique(col: DataSingleColumnInputType, ignore_na: bool = False, ignore_inf: bool = False) -> int:
    """
    ...
    """
    non_inf = np.repeat(True, len(col))
    non_na = np.repeat(True, len(col))
    if ignore_inf:
        if ignore_inf and (isinstance(col, pd.Series) or isinstance(col, np.ndarray)):
            col_arr = col
            if isinstance(col, pd.Series):
                col_arr = col.values
            if np.issubdtype(col_arr.dtype, np.floating):
                non_inf = np.isfinite(col)
    if ignore_na:
        non_na = ~pd.isna(col)
    col_clean = col[np.logical_and(non_inf, non_na)]
    n_unique = _get_unique(col_clean).shape[0]
