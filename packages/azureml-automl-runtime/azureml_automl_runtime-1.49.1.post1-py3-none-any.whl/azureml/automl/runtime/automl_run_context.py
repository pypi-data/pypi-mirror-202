...
# Package repository url for torch 1.11.0+cu113 and torchvision 0.12.0+cu113 packages, needed
# for vision runs.
...
class AutoMLAbstractRunContext(ABC):
    ...
    def _save_mlflow_model(self,
        ...
        if inference_deps is not None:
            ...
            # TODO: replace the following hack with a systematic solution. For vision models, add extra index url to
            # the mlflow-model/conda.yml file because the torch and torchvision package versions used by the vision
            # package as of 4/4/2023 are not on the PyPi repository. They are installed via `pip install
            # --extra-index-url` in the Dockerfile and the mlflow-model/conda.yml file must follow suit and use
            # --extra-index-url too.
            if self._mlflow_custom_pyfunc_flavor_workflow(options) == constants.MLFlowLiterals.WRAPPER:
