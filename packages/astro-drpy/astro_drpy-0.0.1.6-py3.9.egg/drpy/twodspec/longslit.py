...
def trace(ccd, slit_along, fwhm, method, n_med=3, reference_bin=None, interval=None, 
    ...
    if show | save:
        ...
        # Subplot 1
        ax[0].step((idx_row + 1), count_ref, 'k-', lw=1.5, where='mid')
        ax[0].axvline(x=(center_ref + 1), color='r', ls='-', lw=1.5)
        ...
        ax[1].plot((fitted_trace + 1), (idx_col + 1), 'r-', lw=1.5)
...
def background(ccd, slit_along, trace1d, distance=50, aper_width=50, degree=1, 
    ...
    if show | save:
        ...
        # Subplot 1
        ax[0].step(
            (idx_row + 1), np.nanmedian(data_arr, axis=1), 'k-', lw=1.5, where='mid')
        ax[0].axvline(x=(idx_lbkg_min.mean() + 1), color='y', ls='--', lw=1.5)
        ax[0].axvline(x=(idx_lbkg_max.mean() + 1), color='y', ls='--', lw=1.5)
        ax[0].axvline(x=(idx_rbkg_min.mean() + 1), color='b', ls='--', lw=1.5)
        ax[0].axvline(x=(idx_rbkg_max.mean() + 1), color='b', ls='--', lw=1.5)
        ...
        if slit_along == 'col':
            ...
            _plot2d(
                ...
                cbar=False, aspect='auto')
        (xmin, xmax), (ymin, ymax) = ax[1].get_xlim(), ax[1].get_ylim()
        ax[1].plot((idx_lbkg_min + 1), (idx_col + 1), 'y--', lw=1.5)
        ax[1].plot((idx_lbkg_max + 1), (idx_col + 1), 'y--', lw=1.5)
        ax[1].plot((idx_rbkg_min + 1), (idx_col + 1), 'b--', lw=1.5)
        ax[1].plot((idx_rbkg_max + 1), (idx_col + 1), 'b--', lw=1.5)
