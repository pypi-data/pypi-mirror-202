...
class CertoraBuildGenerator:
    ...
    def collect_funcs(self, data: Dict[str, Any], contract_file: str,
        ...
        def collect_func_source_code_signatures_from_abi() -> List[str]:
            ...
            for f in filter(lambda x: self.is_imported_abi_entry(x), abi):
                func_signature = self.get_func_signature(f)
                ...
                lambda node_id: "nodeType" in contract_file_ast[node_id] and
                                ...
                                (("kind" in contract_file_ast[node_id] and
                                  (contract_file_ast[node_id]["kind"] == "function" or contract_file_ast[node_id][
                                      "kind"] == CertoraBuildGenerator.CONSTRUCTOR_STRING)) or
    ...
    def collect_func_source_code_signatures_source(self, contract_file: Path,
                                                   contract_name: str, solc: str) -> List[Tuple[str, List[str],
                                                                                                List[str], str]]:
        """
        returns a list of function signatures for a given contract
        ...
        @return: List of str containing the function signature for each contract
        """
        func_signatures = []
        file_abs_path = (Path() / contract_file).resolve()
        if file_abs_path.suffix != ".sol":
            build_logger.error(f'May only collect functions from Solidity files: {file_abs_path} received')
            sys.exit(1)
        ...
        data = storage_data["contracts"][str(file_abs_path)][contract_name]
        abi = data["abi"]  # ["contracts"][contract_file][contract_name]["abi"]
        ast_logger.debug(f"abi is: \n{abi}")
        for f in filter(lambda x: self.is_imported_abi_entry(x), abi):
    ...
    @staticmethod
    def get_full_func_signature(f: Dict[str, Any]) -> Tuple[str, List[str], List[str], str]:
        """
        ...
        outputs = f["outputs"]
        func_outputs = [output["internalType"] if "internalType" in output else output["type"] for output in outputs]
