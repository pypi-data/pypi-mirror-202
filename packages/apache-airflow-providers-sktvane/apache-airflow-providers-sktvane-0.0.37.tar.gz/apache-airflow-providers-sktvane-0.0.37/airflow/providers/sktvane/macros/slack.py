...
def _get_user_id_by_email(email: str = None) -> str:
    if not email:
        return "UNDEFINED"
    try:
        response = slack_hook.client.users_lookupByEmail(email=email)
...
def get_aidp_fail_alert(email: str = None):
    def callback(context: Context) -> None:
        user_id = _get_user_id_by_email(email=email)
        slack_hook.client.chat_postMessage(
            channel="#aidp-airflow-monitoring",
            username=f"Airflow-AlarmBot-{env}",
            blocks=_generate_fail_alert_blocks(context=context, user_id=user_id),
        )
