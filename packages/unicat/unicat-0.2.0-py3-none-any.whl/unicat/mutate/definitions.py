...
def create_definition(
    ...
    *,
    ...
    if classes is not None and type(classes) is not str:
        properties["classes"] = [
            class_.gid if hasattr(field, "gid") else class_ for class_ in classes
        ]
    ...
    if childdefinitions is not None and childdefinitions(fields) is not str:
...
def modify_definition_remove_field(unicat, definition, field_):
    success, result = unicat.api.call(
