...
def _wait_output(popen, is_slow):
    ...
    try:
        proc.wait(const.wait_slow_command if is_slow
                  else const.wait_command)
        return True
...
def get_output(script, expanded):
    ...
    """
    env = dict(os.environ)
    env.update(const.env)


    split_expand = shlex.split(expanded)
    is_slow = split_expand[0] in const.slow_commands if split_expand else False
