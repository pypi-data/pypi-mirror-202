...
from ansible_collections.f5networks.f5_modules.plugins.module_utils.constants import PLATFORM
...
def print_banner():
    banner = generate_banner(
        app_name="RPC",
        author=f"jinwoo \n\n - Platform    : {pconf().data.args.platform.upper()}",
...
def fill_sign_params_from_args(payload: dict):
    ...
    if payload.get('params'):
        for args_key in args_list:
            if  getattr(args, args_key, NONE_STRING) != NONE_STRING:

                if args_key == "value":
                    value = hex(int(getattr(args, args_key) * 10 ** 18))
                else:
                    value = getattr(args, args_key, NONE_STRING)

                if value != NONE_STRING and value is not None :
...
class RpcCommand:
    ...
    def initialize_arguments(self):
        ...
        if self.args.platform not in PLATFORM_LIST:
            raise ValueError(f'not supported platform, allowed = {PLATFORM_LIST}')
    ...
    def set_network_info(self):
        ...
        PromptWithArgument(
            ...
            choices=NetworkInfo().get_network_list(platform=self.args.platform),
            type="list",
            argument="network",
            default="vega"
        ).prompt()
        self.network_info = NetworkInfo(platform=self.args.platform, network_name=self.args.network)
    ...
    def generate_tx_payload(self):
        ...
        if self.icon_tpl.is_required_sign():
            ...
            # payload = fill_sign_params_from_args(self._payload)
            self.print_balance()
